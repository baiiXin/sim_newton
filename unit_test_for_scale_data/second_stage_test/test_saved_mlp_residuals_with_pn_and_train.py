...
def build_training_grid_from_config(
    *,
    y_star: torch.Tensor,
    points_per_axis: int,
    ...
    """
    根据训练配置重建完整规则训练网格，返回 CPU 张量 [N, 3]。
    """
    ...
    axes = [
        torch.linspace(
            float(y_star_cpu[i] - sampling_radius),
            float(y_star_cpu[i] + sampling_radius),
    ...
    grid = torch.stack(mesh, dim=-1).reshape(-1, 3).contiguous()
    metadata = {
        "mode": "regular_cartesian_training_grid",
        "points_per_axis": int(points_per_axis),
        "num_total_points": int(grid.shape[0]),
...
def plot_final_residual_vs_dataset_size(records: Sequence[dict[str, Any]], save_path: Path) -> None:
    """
    将训练集和测试集的最终残差图拼在同一张大图中。

    布局：2 x 4
        第一行：训练集 mean / median / p95 / max
        第二行：测试集 mean / median / p95 / p_n
    """
    fig, axes = plt.subplots(2, 4, figsize=(22, 10.8))

    train_metrics = [
        (
            "train_final_mean_residual",
            "Training set final mean residual",
            r"$\frac{1}{|S_{train}|}\sum_{y_0\in S_{train}} r_K(y_0)$",
        ),
        (
            "train_final_median_residual",
            "Training set final median residual",
            r"$\mathrm{median}_{y_0\in S_{train}}\, r_K(y_0)$",
        ),
        (
            "train_final_p95_residual",
            "Training set final p95 residual",
            r"$\mathrm{p95}_{y_0\in S_{train}}\, r_K(y_0)$",
        ),
        (
            "train_final_max_residual",
            "Training set final max residual",
            r"$\max_{y_0\in S_{train}} r_K(y_0)$",
        ),
    ]
    test_metrics = [
        (
            "final_mean_residual",
            "Held-out test set final mean residual",
            r"$\frac{1}{|T|}\sum_{y_0\in T} r_K(y_0)$",
        ),
        (
            "final_median_residual",
            "Held-out test set final median residual",
            r"$\mathrm{median}_{y_0\in T}\, r_K(y_0)$",
        ),
        (
            "final_p95_residual",
            "Held-out test set final p95 residual",
            r"$\mathrm{p95}_{y_0\in T}\, r_K(y_0)$",
        ),
        (
            "pn_final_residual_assuming_first_point",
            r"Original held-out $p_n$ final residual",
            r"$r_K(p_n)$",
        ),
    ]
    ...
    for key in unique_optimizer_keys(records):
        ...
        for ax, (metric_name, title, ylabel) in zip(axes[0], train_metrics):
        ...
        for ax, (metric_name, title, ylabel) in zip(axes[1], test_metrics):
            values = [finite_plot_value(r.get(metric_name)) for r in selected]
            ax.plot(sizes, values, marker="o", label=optimizer_label(key))
            ax.set_xscale("log")
            ax.set_yscale("log")
    ...
    fig.suptitle(
        "Training-set and held-out-test residual statistics vs. training dataset size",
        y=1.01,
    )
    fig.text(
        0.5,
        -0.01,
        (
            r"Definition: $r_k(y_0)=\|\nabla E(y^{(k)}(y_0))\|_2$. "
            r"Top row aggregates over the reconstructed training set $S_{train}$ of each experiment. "
            r"Bottom row aggregates over the shared held-out test set $T$; the last panel is the single-point residual for $p_n$."
        ),
        ha="center",
        va="top",
        fontsize=9,
...
def main() -> None:
    ...
    for exp in experiment_files:
        ...
        train_sampling_radius = float(
            cfg.get("sampling_radius_per_axis", cfg.get("sampling_radius", sampling_radius))
        )
        train_points_cpu, train_set_metadata = build_training_grid_from_config(
            y_star=y_star_cpu,
        ...
        train_eval_result = evaluate_model_on_test_set(
            model=model,
            test_points_cpu=train_points_cpu.to(dtype=dtype),
            p_n=p_n,
            v_n=v_n,
            m=m_i,
            g=g_i,
        ...
        record = {
            "training_set_metadata": train_set_metadata,
            "train_num_points": int(train_eval_result["num_test_points"]),
            "train_mean_residual_by_step": train_eval_result["mean_residual_by_step"],
            "train_median_residual_by_step": train_eval_result["median_residual_by_step"],
            "train_p95_residual_by_step": train_eval_result["p95_residual_by_step"],
            "train_max_residual_by_step": train_eval_result["max_residual_by_step"],
            "train_final_mean_residual": train_eval_result["final_mean_residual"],
            "train_final_median_residual": train_eval_result["final_median_residual"],
            "train_final_p95_residual": train_eval_result["final_p95_residual"],
            "train_final_max_residual": train_eval_result["final_max_residual"],
        ...
        print(
            "  train mean={:.4e}, test mean={:.4e}, test median={:.4e}, test p95={:.4e}, p_n={}".format(
                record["train_final_mean_residual"],
    ...
    summary = {
        ...
        "evaluation": {
            "steps": int(args.steps),
            "batch_size": int(args.batch_size),
            "pointwise_metric": r"r_k(y_0)=||∇E(y^(k)(y_0))||_2",
            "aggregate_metrics": {
                "train_final_mean_residual": r"mean over reconstructed training set at final step K",
                "train_final_median_residual": r"median over reconstructed training set at final step K",
                "train_final_p95_residual": r"95th percentile over reconstructed training set at final step K",
                "train_final_max_residual": r"max over reconstructed training set at final step K",
                "final_mean_residual": r"mean over held-out test set at final step K",
                "final_median_residual": r"median over held-out test set at final step K",
                "final_p95_residual": r"95th percentile over held-out test set at final step K",
                "pn_final_residual_assuming_first_point": r"single-point residual at final step K for the original held-out initial state p_n",
            },
        },
        "experiments": records,
    }
    ...
    print(f"🖼️ 训练+测试总体残差图: {output_dir / 'heldout_final_residual_vs_training_dataset_size.png'}")
