"""
自由落体单帧变分问题：规则网格 Full-Batch 训练数据规模消融实验（float32 全优化器对照）
====================================================
...
4. 输入使用当前规则网格的解析统计量做逐特征归一化；
5. 网络输出乘以 dt，得到最终位置更新量；
6. loss 只使用隐式欧拉变分能量：对展开轨迹中的 K 步能量直接求和；
7. 每个 epoch 使用当前完整规则网格作为一个 Full-Batch；
8. 在同一组参数下完整展开 K 步轨迹，不在步间 detach；
9. 每个 epoch 仅执行一次 backward 和一次 optimizer.step；
10. 训练阶段从 K=1 开始，每 10000 个 epoch 增加 1 次展开迭代，最高 K=5；
11. 总训练轮数为 50000；
12. 训练集以精确解 y_star 为中心，在三个维度分别等距均匀取点；
13. 每个维度的采样范围均为 [y_star - 0.01, y_star + 0.01]；
14. 每个维度使用偶数个采样点，因此规则网格不会包含精确解 y_star；
15. 最终只测试未参与训练的真实初值 p_n；
16. 默认在 cuda:0 上运行。
...
- 6 组优化器与学习率配置复用同一份设备端规则网格缓存。
- 每个 epoch 直接使用当前完整规则网格作为一个 Full-Batch。
- 每个 epoch 按当前 K 完整展开轨迹，并执行一次 backward 和一次 optimizer.step。
- K 从 1 开始，每 10000 个 epoch 增加 1，最高为 5。
...
# float32 对照实验：恢复输入逐特征归一化与输出 dt 缩放。
...
# 训练阶段使用 curriculum：K 从 1 开始，每 10000 个 epoch 增加 1，最高为 5。
DEFAULT_INITIAL_K = 1
...
# 汇总图中每条训练 loss 曲线最多保存/绘制这些点，避免 50000 epoch × 42 组实验让总汇总 JSON 过大。
    ...
    report = {
        "config": {
            ...
            "grid_storage": (
                ...
                "on the training device. All optimizer and learning-rate experiments "
...
def plot_dataset_scale_summary(
    ...
    现在除了 held-out p_n 的最终 gap/residual，也额外输出最终训练 loss gap，
    这样总对比图同时包含测试集数据和训练 loss 数据。
    """
    ...
    for optimizer_config in OPTIMIZER_CONFIGS:
        ...
        train_loss_gaps = [
            finite_plot_value(item["final_mean_training_gap_for_readability"])
            for item in selected
        ]
...
def main() -> None:
    ...
    overall_report = {
        ...
        "grid_storage": (
            ...
            "chunks on the training device, reuse it for all optimizer and learning-rate "
    ...
    print(f"🖼️ 6子图训练 loss 对比: {training_loss_summary_plot_path}")
    for item in experiment_summaries:
        print(
            ...
            f"最终训练 loss gap={format_metric(item['final_mean_training_gap_for_readability'])}, "
