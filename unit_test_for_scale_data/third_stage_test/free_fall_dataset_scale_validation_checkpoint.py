"""
自由落体单帧变分问题：规则网格 Full-Batch 数据规模消融
=========================================================

本脚本以原始“训练数据规模 × 优化器”消融实验为基础，并整合 held-out
集合的批量 residual / energy-gap 评估方式。

核心实验设置
------------
1. 网络固定为当前表现最好的 1×32 MLP：12 -> 32 -> 3；
2. 数值精度固定为 torch.float64；
3. 默认运行设备为 cuda:1；
4. 训练集仍为 y_star 周围的三维规则网格；
5. 数据规模仍为 8、64、216、1,000、10,648、97,336、1,000,000；
6. 优化器仍为 SGD/Adam × lr={1e-2, 1e-3, 1e-4}，共 42 组实验；
7. 每个 epoch 使用完整训练集做一次 Full-Batch 更新；
8. K 从 1 开始，每 10,000 epoch 增加 1，最高 K=5；
9. 最大训练 50,000 epoch，不执行 early stopping；
10. 所有实验共享同一组固定 held-out 点，并显式排除全部训练规则网格点；
11. held-out 点默认拆为 1,024 个验证点和 3,072 个测试点；
12. 每 500 epoch 在验证集上固定展开 50 步；
13. 以最终步 validation residual p95 为主指标选择最佳 checkpoint，
    再依次比较 non-finite 数量、residual median、energy-gap p95；
14. 测试集在训练过程中完全不可见，只在训练完成后评估；
15. 同时保存最后 epoch 模型与验证集最优模型；
16. 最终报告以验证集最优模型为主，并额外报告最后 epoch 模型，便于判断后期退化。

默认运行
--------
    python free_fall_dataset_scale_validation_checkpoint.py

快速 CPU 冒烟测试
-----------------
    python free_fall_dataset_scale_validation_checkpoint.py \
        --device cpu --target-dataset-sizes 8 \
        --optimizer-configs adam:1e-3 \
        --epochs 2 --validation-interval 1 \
        --validation-size 16 --test-size 24 \
        --evaluation-steps 2 --skip-contour
"""
...
def plot_final_heldout_summary(
    ...
    for key in unique_optimizer_keys(records):
        selected = sorted(
            [record for record in records if optimizer_key(record) == key],
            key=lambda record: int(record["dataset_size"]),
        )
        sizes = [int(record["dataset_size"]) for record in selected]
        for ax, (metric, title) in zip(axes[0], residual_metrics):
            values = []
            for record in selected:
                if metric.startswith("pn_"):
                    values.append(finite_plot_value(record[checkpoint_field][metric]))
                else:
                    values.append(finite_plot_value(record[checkpoint_field][metric]))
            ...
            ax.grid(True, alpha=0.3)
        for ax, (metric, title) in zip(axes[1], gap_metrics):
            values = [finite_plot_value(record[checkpoint_field][metric]) for record in selected]
            ax.plot(sizes, values, marker="o", label=optimizer_label(key))
            ax.set_xscale("log")
            ax.set_yscale("log")
            ax.set_title(title)
    ...
    for ax in axes.reshape(-1):
        ax.legend(fontsize=7)
    fig.suptitle("Held-out test metrics using validation-selected checkpoints", y=1.01, fontsize=14)
    plt.tight_layout()
    plt.savefig(save_path, dpi=250, bbox_inches="tight")
    plt.close(fig)
...
def plot_training_loss_summary(
    ...
    fig, axes = plt.subplots(num_rows, num_cols, figsize=(18, 4.8 * num_rows), sharex=True)
    axes_array = np.asarray(axes).reshape(-1)
    for ax, key in zip(axes_array, keys):
        selected = sorted(
            [record for record in records if optimizer_key(record) == key],
            key=lambda record: record["dataset_size"],
        )
        for record in selected:
    ...
    for ax in axes_array[len(keys):]:
        ax.axis("off")
    fig.suptitle("Training curves", y=1.01, fontsize=14)
    plt.tight_layout()
    plt.savefig(save_path, dpi=250, bbox_inches="tight")
    plt.close(fig)
...
def plot_validation_summary(
    ...
    for ax in axes_array[len(keys):]:
        ax.axis("off")
    fig.suptitle("Validation curves used for checkpoint selection", y=1.01, fontsize=14)
    plt.tight_layout()
    plt.savefig(save_path, dpi=250, bbox_inches="tight")
    plt.close(fig)
...
def run_experiment(
    *,
    ...
    print(f"device={device}, dtype={TORCH_DTYPE}, architecture=12->32->3")
    print(f"training_N={dataset_size:,}, validation_N={config.validation_size:,}, test_N={config.test_size:,}")
    print(f"optimizer={optimizer_name}, lr={learning_rate:.0e}")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Free-fall 1x32 MLP dataset-size ablation with validation-selected checkpoints."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Optional subset, e.g. adam:1e-3 sgd:1e-2. Default: all six configs.",
    )
...
def main() -> None:
    ...
    overall_report = {
        "experiment_type": "dataset_size_and_optimizer_ablation_with_validation_checkpoint_selection",
        "purpose": (
            "Use the original full-batch dataset-size/optimizer ablation, replace the network "
            "with the selected 1x32 architecture, select the best epoch using a fixed validation "
            "split, and evaluate only after training on an independent test split."
        ),
    ...
    save_json(overall_report, output_dir / "dataset_scale_validation_checkpoint_summary.json")

    plot_final_heldout_summary(
        summaries,
        output_dir / "best_checkpoint_heldout_test_summary.png",
        checkpoint_field="best_checkpoint_test",
    )
    plot_training_loss_summary(
        summaries,
        output_dir / "training_loss_6_optimizer_settings_summary.png",
    )
    plot_validation_summary(
        summaries,
        output_dir / "validation_residual_p95_6_optimizer_settings_summary.png",
    )

    print("\n" + "=" * 88)
    print("所有实验完成。")
    print(f"汇总 JSON: {output_dir / 'dataset_scale_validation_checkpoint_summary.json'}")
