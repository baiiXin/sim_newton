...
9. K 从 1 开始，每 1000 个 epoch 增加 1，最大增加至 5；
...
def precompute_regular_grid_on_device(
    *,
    ...
    最大默认网格约含 10^8 个 float32 三维点，占用约 1.2 GB。
...
def iterate_shuffled_cached_grid_minibatches(
    training_grid: torch.Tensor,
    batch_size: int,
    generator: torch.Generator,
) -> Iterable[torch.Tensor]:
    """
    从设备端缓存网格中按 mini-batch 取样，遍历一个完整 epoch。

    为避免为约 10^8 个样本创建 randperm，本函数使用仿射置换：
    ...
    for start in range(0, num_points, batch_size):
        ...
        yield training_grid.index_select(0, shuffled_indices)
    ...
    *,
    ...
    for epoch in range(config.epochs):
        ...
        for batch_index, batch in enumerate(
            iterate_shuffled_cached_grid_minibatches(
    ...
    report = {
        "config": {
            ...
            "dataset_mode": "device_cached_regular_cartesian_grid_near_y_star",
            ...
            "grid_storage": (
                ...
                "on the training device. SGD and Adam reuse the same cached grid for "
            ...
            "epoch_shuffle": (
                "Affine permutation of flattened grid indices; visits every grid point "
                "exactly once per epoch without allocating randperm(N). Batch indices "
                "and index_select are evaluated on the training device."
            ),
            "batch_size": config.batch_size,
...
def main() -> None:
    ...
    overall_report = {
        ...
        "grid_storage": (
            ...
            "chunks on the training device, reuse it for SGD and Adam, then release it."
        ),
        "use_input_normalization": USE_INPUT_NORMALIZATION,
        "use_output_dt_scaling": USE_OUTPUT_DT_SCALING,
        "normalization_definition": (
