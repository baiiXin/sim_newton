...
10. 训练集以精确解 y_star 为中心，在三个维度分别均匀取点；
11. 每个维度的采样范围均为 [y_star - 0.01, y_star + 0.01]；
12. 每个维度使用偶数个采样点，因此规则网格不会包含精确解 y_star；
13. 最终只测试未参与训练的真实初值 p_n。

默认测试五个目标数据规模：
    10^4, 10^5, 10^6, 10^7, 10^8

由于三维规则网格的总点数必须是“每轴点数”的三次方，并且每轴点数必须为偶数，
脚本会选择最接近每个目标规模的偶数轴向点数。默认对应关系为：
    22^3  = 10,648
    46^3  = 97,336
    100^3 = 1,000,000
    216^3 = 10,077,696
...
实现说明
--------
- 规则网格通过扁平索引按 mini-batch 动态生成，不会一次性在内存中存储约 10^8 个点。
- 每个 epoch 会遍历当前规则网格中的全部样本一次。
- 为避免创建巨大的 randperm，脚本每个 epoch 使用一个仿射置换打乱扁平索引；
  该置换会无重复地遍历网格中的所有点。
- 10^8 量级样本配合 1000 epochs 的完整训练计算量极大。脚本保留命令行参数，
  可先使用更小的目标规模和更少的 epoch 做快速检查。
"""
...
# 目标训练数据规模。每一个数量级对应一组独立实验。
# 实际网格点数会取最接近目标值的 even_points_per_axis ** 3。
...
def flat_indices_to_grid_points(
    ...
    """
    将规则三维网格中的扁平索引转换为坐标，返回 CPU float32 张量 [B, 3]。

    网格轴坐标使用 linspace(-R, R, points_per_axis) 的等价公式。
    points_per_axis 为偶数，因此任一坐标轴都不会采到 0。
    """
    ...
    n_squared = n * n
    flat_indices = flat_indices.to(dtype=torch.int64, device="cpu")
    ...
    integer_indices = torch.stack([index_x, index_y, index_z], dim=-1)
    offsets = (
        -grid_spec.sampling_radius
        + integer_indices.to(dtype=TORCH_DTYPE) * grid_spec.axis_spacing
    )
...
def iterate_shuffled_grid_minibatches(
    grid_spec: GridSpec,
    y_star_cpu: torch.Tensor,
    batch_size: int,
    generator: torch.Generator,
    ...
    """
    按 mini-batch 动态生成一个规则网格 epoch。

    为避免对约 10^8 个样本创建 randperm，本函数使用仿射置换：
        shuffled_index = (multiplier * index + offset) mod N
    其中 multiplier 与 N 互素，因此每个 epoch 会无重复地遍历全部 N 个网格点。
    """

    num_points = grid_spec.actual_num_points
    multiplier = draw_coprime_multiplier(num_points, generator)
    offset = int(
        torch.randint(
            low=0,
            high=num_points,
    ...
    for start in range(0, num_points, batch_size):
        ...
        sequential_indices = torch.arange(start, end, dtype=torch.int64)
        shuffled_indices = torch.remainder(
            multiplier * sequential_indices + offset,
            num_points,
        )
        yield flat_indices_to_grid_points(
            flat_indices=shuffled_indices,
            grid_spec=grid_spec,
...
def sample_grid_points_for_plot(
    ...
    """从隐式规则网格中均匀抽取少量点，仅用于绘图。"""

    num_points = grid_spec.actual_num_points
    num_shown = min(num_points, max_points)
    if num_shown == num_points:
        flat_indices = torch.arange(num_points, dtype=torch.int64)
    else:
    ...
    return flat_indices_to_grid_points(flat_indices, grid_spec, y_star_cpu)
    ...
    *,
    ...
    for epoch in range(config.epochs):
        ...
        for batch_index, batch_cpu in enumerate(
            iterate_shuffled_grid_minibatches(
                grid_spec=grid_spec,
                y_star_cpu=y_star_cpu,
    ...
    report = {
        "config": {
            ...
            "dataset_mode": "implicit_regular_cartesian_grid_near_y_star",
            ...
            "epoch_shuffle": (
                "Implicit affine permutation of flattened grid indices; visits "
                "every grid point exactly once per epoch without allocating randperm(N)."
            ),
            "batch_size": config.batch_size,
...
def plot_training_dataset_sample(
    ...
    y_star_np = y_star_cpu.detach().cpu().numpy()
