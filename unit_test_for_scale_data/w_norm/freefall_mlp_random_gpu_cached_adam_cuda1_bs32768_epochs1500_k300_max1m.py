"""
自由落体单帧变分问题：随机采样训练数据规模消融实验（Adam）
====================================================
...
实现说明
--------
- 最大规模随机训练集会在 cuda:1 上分块生成一次，并缓存到设备显存中。
- 较小训练集使用最大训练集的前缀，因此扩大数据规模时只新增样本，不替换已有样本。
- 三种 Adam 学习率配置复用同一份设备端随机训练集。
...
- 最大规模为 10^6 个样本；配合 1500 epochs 和三种 Adam 学习率配置，仍建议先进行小规模检查。
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="cuda:1",
        help="Training device. Default: cuda:1. Use cpu for smoke tests.",
    )
    parser.add_argument(
        "--skip-contour",
        action="store_true",
...
def validate_training_device(device: torch.device) -> None:
    """给出比 PyTorch 默认错误更清晰的 cuda:1 可用性检查。"""
...
def main() -> None:
    ...
    overall_report = {
        ...
        "dataset_storage": (
            ...
            "three Adam learning-rate configurations."
        ),
        "use_input_normalization": USE_INPUT_NORMALIZATION,
        "use_output_dt_scaling": USE_OUTPUT_DT_SCALING,
        "normalization_definition": (
