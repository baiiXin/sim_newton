...
#   1. Training-input / result-point distribution plot
#   2. Training-loss plot
#   3. Compressed detailed numerical logs (.npz)
# ============================================================
...
def plot_training_point_distribution(
    ...
    initial_pt = to_plot_coordinates(
        [y0],
        y0=y0,
        relative=relative,
    )[0]
    ...
    for ax in (ax_train, ax_result):
        ax.scatter(
            ...
            c="black",
            linewidths=2.0,
            label=r"Reference initial point $y_0$",
        )
    ...
    fig.suptitle(
        f"{name}: Training Data and Result Distribution ({coordinate_note})",
        fontsize=14,
    )
...
def plot_training_loss_curves(name, history, loss_type):
    """
    Create a three-panel plot for one MLP.
    ...
    Panel 2:
        - Last backprop objective: the exact final objective encountered by the
          training loop in an epoch. This keeps the original order-dependent
          statistic for traceability.
        - Frozen-model worst terminal objective: after the epoch, freeze the
          MLP, roll out all training initial states for K steps, and take the
          maximum terminal objective. This is the strict worst-case statistic.
    ...
    # --------------------------------------------------------
    # 1. Mean objective used during backpropagation
    # --------------------------------------------------------
    ...
    # --------------------------------------------------------
    # 2. Original last loss and strict worst-case terminal loss
    # --------------------------------------------------------
    ...
    axes[1].plot(
        ...
        label="Frozen-model worst terminal objective across training states",
    )
    ...
    axes[1].plot(
        ...
        linestyle="--",
        label="Frozen-model mean terminal objective across training states",
    )
    ...
    # --------------------------------------------------------
    # 3. Frozen-model physical terminal metrics
    # --------------------------------------------------------
    ...
    fig.suptitle(
        f"{name}: Training Loss and Frozen-model Evaluation",
        fontsize=14,
    )
...
def train_model(
    ...
    point_plot_path = plot_training_point_distribution(
        name=name,
    ...
    loss_plot_path = plot_training_loss_curves(
        name=name,
    ...
    return {
        ...
        "point_plot_path": point_plot_path,
...
def save_json_report(results, overview_plot_path):
    ...
    for res in results:
        report["experiments"][res["name"]] = {
            ...
            "point_plot": res["point_plot_path"],
            "training_loss_plot": res["loss_plot_path"],
...
def main():
    ...
    for cfg in configs:
        ...
        print(
            f"Saved point plot: "
            f"{res['point_plot_path']}"
        )

        print(
            f"Saved loss plot: "
            f"{res['loss_plot_path']}"
        )
