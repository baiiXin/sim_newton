...
COVERAGE_NUM_LINE_POINTS = 11
COVERAGE_NUM_LOCAL_POINTS = 32
COVERAGE_LOCAL_STD_DT_UNITS = 1.0
...
def make_training_states(
    ...
    y_star,
    dt,
    num_line_points=COVERAGE_NUM_LINE_POINTS,
    num_local_points=COVERAGE_NUM_LOCAL_POINTS,
    local_std_dt_units=COVERAGE_LOCAL_STD_DT_UNITS,
    seed=123,
):
    train_states = []
...
def main():
    ...
    if USE_COVERAGE:
        training_initial_points = make_training_states(y0, y_star, dt)
    else:
        training_initial_points = [y0]
    ...
    # --------------------------------------------------------
    # 8.5 最终测试：参考初值 y0 的 MLP vs Newton 对比
    # --------------------------------------------------------
    ...
    report = {
        "config": {
            ...
            "coverage_num_line_points": COVERAGE_NUM_LINE_POINTS,
            "coverage_num_local_points": COVERAGE_NUM_LOCAL_POINTS,
            "coverage_local_std_dt_units": COVERAGE_LOCAL_STD_DT_UNITS,
            "y0": y0.tolist(),
