...
# 扰动点数量消融实验：固定采用完整归一化方案。
# 1. 输入使用当前训练集统计量标准化；
# 2. 网络输出乘以 dt；
# 3. 训练集固定为“精确初值 y0 + y0 附近随机扰动点”；
# 4. 扰动点的绝对坐标标准差固定为 sigma = 1e-2。
#
# 本实验只改变随机扰动点的数量：
#     2, 5, 10, 20, 30, 50
#
# 六组实验复用相同随机种子。较小数据集中的扰动点是较大数据集
# 的前缀子集，使实验差异集中在训练数据数量本身。
...
# 固定采用“全部归一化”：输入归一化 + 输出 dt 放缩。
USE_NORMALIZATION = True
USE_DT_SCALING = True

PERTURBATION_NUM_POINTS_VALUES = [
    2,
    5,
    10,
    20,
...
ABLATION_EXPERIMENTS = [
    {
        "name": f"{index:02d}_perturbation_points_{num_local_points:02d}",
        "description": (
            "Full normalization with initial-point perturbations. "
            f"Absolute coordinate perturbation standard deviation: "
            f"sigma={FIXED_PERTURBATION_STD:.0e}. "
            f"Number of random perturbation points: {num_local_points}."
        ),
        "perturbation_std": FIXED_PERTURBATION_STD,
        "num_local_points": num_local_points,
    }
    for index, num_local_points in enumerate(
        PERTURBATION_NUM_POINTS_VALUES,
        start=1,
    )
]
...
def make_training_states_near_initial(
    ...
    六组实验复用同一个随机种子。较小训练集中的扰动点是
    较大训练集的前缀子集，仅训练数据数量不同。
    """
...
def run_experiment(
    ...
    """
    运行一组扰动点数量消融实验，并将该组全部结果写入独立子目录。

    六组实验保持物理问题、扰动范围、噪声生成种子、网络结构、
    初始化种子、优化器、训练策略和完整归一化方案一致，
    只改变初值附近随机扰动点的数量。
    """
    ...
    print("\n" + "=" * 72)
    print(f"🧪 扰动点数量消融组: {experiment_name}")
...
def plot_perturbation_count_ablation_summary(experiment_summaries, save_path):
    """绘制扰动点数量与最终误差指标之间的关系。"""

    ordered = sorted(
        experiment_summaries,
        key=lambda item: item["num_perturbation_points"],
    )

    num_perturbation_points = [
        item["num_perturbation_points"]
        for item in ordered
    ]
    final_reference_gaps = [
        max(item["final_reference_gap_after_fixed_steps"], 1e-12)
        for item in ordered
    ]
    final_reference_residuals = [
        max(item["final_reference_residual_norm_after_fixed_steps"], 1e-12)
        for item in ordered
    ]
    final_worst_eval_gaps = [
        max(item["final_worst_periodic_eval_gap"], 1e-12)
        for item in ordered
    ]
    ...
    axes[0].plot(
        ...
        final_reference_gaps,
        marker="o",
        label="Reference gap after fixed steps",
    )
    axes[0].plot(
        num_perturbation_points,
        final_worst_eval_gaps,
        marker="s",
        linestyle="--",
        label="Worst periodic eval gap",
    )
    axes[0].set_yscale("log")
    axes[0].set_title("Objective Gap vs. Number of Perturbation Points")
    axes[0].set_xlabel("Number of random perturbation points")
    axes[0].set_ylabel("Gap")
    axes[0].set_xticks(num_perturbation_points)
    axes[0].legend()
    ...
    axes[1].plot(
        ...
        final_reference_residuals,
        marker="o",
        label="Reference residual after fixed steps",
    )
    axes[1].set_yscale("log")
    axes[1].set_title("Residual vs. Number of Perturbation Points")
    axes[1].set_xlabel("Number of random perturbation points")
    axes[1].set_ylabel(r"Stationarity residual $\|\nabla E(y)\|_2$")
    axes[1].set_xticks(num_perturbation_points)
    axes[1].legend()
    ...
    print(f"🖼️ 扰动点数量消融汇总图已保存至: {save_path}")


# ============================================================
# 10. 主程序：依次运行六组扰动点数量消融
# ============================================================

def main():
    base_output_dir = create_output_directory()
    print(f"📁 扰动点数量消融总输出目录: {base_output_dir}")

    # 固定物理问题和扰动范围。六组实验只改变随机扰动点的数量。
    m = 1.0
    g = 9.8
    dt = 0.01
    ...
    ablation_summary = {
        ...
        "comparison_principle": (
            "All groups share the same physical problem, absolute coordinate "
            "perturbation standard deviation, standard-normal random seed, "
            "network architecture, model initialization seed, optimizer, "
            "training schedule, input normalization, and output dt scaling. "
            "Only the number of random initial-point perturbations changes. "
            "Smaller datasets are prefix subsets of larger datasets."
        ),
        "fixed_perturbation_std": FIXED_PERTURBATION_STD,
        "perturbation_num_points_values": PERTURBATION_NUM_POINTS_VALUES,
        "experiments": experiment_summaries,
    }

    summary_path = base_output_dir / "perturbation_count_ablation_summary.json"
    with summary_path.open("w", encoding="utf-8") as file:
        json.dump(ablation_summary, file, indent=2, ensure_ascii=False)

    summary_plot_path = base_output_dir / "perturbation_count_ablation_summary.png"
    plot_perturbation_count_ablation_summary(
        experiment_summaries=experiment_summaries,
        save_path=summary_plot_path,
    )

    print("\n" + "=" * 72)
    print("✅ 六组扰动点数量消融实验全部完成。")
    ...
    for item in experiment_summaries:
        print(
            ...
            f"{item['num_perturbation_points']} 个随机扰动点，"
