...
# 数据集消融参数。六组实验的唯一区别是训练初值集合。
# 组 1：只有原始初值 y0。
# 组 2：从 y0 到理论最优解 y* 的完整 line 采样点，包含 y0 和 y*。
# 组 3：原始初值 y0，加上 y* 附近的随机扰动点；不额外加入 line。
# 组 4：原始初值 y0，加上 y0 附近的随机扰动点。
# 组 5：完整 line 的前一半，包含 y0 和 line 中点。
# 组 6：完整 line 的后一半，包含 line 中点和 y*。
LINE_NUM_POINTS = 11
LOCAL_NUM_POINTS = 32
...
ABLATION_EXPERIMENTS = [
    ...
    {
        "name": "04_initial_plus_initial_perturbations",
        "description": "The original initial state y0 plus random perturbations near y0.",
        "dataset_mode": "initial_plus_initial_perturbations",
    },
    {
        "name": "05_initial_plus_first_half_line",
        "description": "The first half of the y0-to-y_star line, including y0 and the midpoint.",
        "dataset_mode": "initial_plus_first_half_line",
    },
    {
        "name": "06_initial_plus_second_half_line",
        "description": "The second half of the y0-to-y_star line, including the midpoint and y_star.",
        "dataset_mode": "initial_plus_second_half_line",
    },
]
...
def make_ablation_training_states(
    ...
    initial_plus_initial_perturbations:
        先加入原始初值 y0，再加入 num_local_points 个 y0 附近扰动点。

    initial_plus_first_half_line:
        完整 line 的前一半，包含 y0 和 line 中点。

    initial_plus_second_half_line:
        完整 line 的后一半，包含 line 中点和 y_star。
    """

    if num_line_points < 3 or num_line_points % 2 == 0:
        raise ValueError("LINE_NUM_POINTS 必须是大于等于 3 的奇数，以便精确划分前后半段。")

    line_states = [
        (1.0 - alpha) * y0 + alpha * y_star
        for alpha in torch.linspace(0.0, 1.0, num_line_points)
    ]
    midpoint_index = num_line_points // 2
    ...
    if dataset_mode == "initial_plus_first_half_line":
        return line_states[: midpoint_index + 1]

    if dataset_mode == "initial_plus_second_half_line":
        return line_states[midpoint_index:]
    ...
    if dataset_mode in {
        ...
        for _ in range(num_local_points):
            ...
            train_states.append(
                perturbation_center + dt * local_std_dt_units * noise
            )
