...
# 归一化消融实验：固定使用“原始初值 y0 + y0 附近随机扰动点”训练集。
# 三组实验只切换输入归一化与输出 dt 放缩，其余条件保持一致。
#
# 组 1：完全没有归一化。
#       输入不归一化，网络输出也不按 dt 放缩。
# 组 2：只对输入做归一化。
#       输入使用训练集统计量标准化，网络输出不按 dt 放缩。
# 组 3：只对输出做放缩。
#       输入不归一化，网络输出乘以 dt。
LOCAL_NUM_POINTS = 32
...
ABLATION_EXPERIMENTS = [
    {
        "name": "01_no_normalization",
        "description": (
            "No normalization: raw input and unscaled network output."
        ),
        "use_normalization": False,
        "use_dt_scaling": False,
    },
    {
        "name": "02_input_normalization_only",
        "description": (
            "Input normalization only: normalize the input using training-set "
            "statistics, without scaling the network output by dt."
        ),
        "use_normalization": True,
        "use_dt_scaling": False,
    },
    {
        "name": "03_output_dt_scaling_only",
        "description": (
            "Output scaling only: use raw input and multiply the network output "
            "by dt."
        ),
        "use_normalization": False,
        "use_dt_scaling": True,
    ...
    {
        "name": "04_full_normalization",
        "description": (
            "Full normalization experiment: normalize the input and scale the network output by dt."
        ),
        "use_normalization": True,
        "use_dt_scaling": True,
    },
...
def make_training_states_near_initial(
    ...
    扰动尺度为：
        dt * local_std_dt_units

    三组消融实验使用完全相同的训练点，因此实验差异只来自：
        1. 是否对网络输入做归一化；
        2. 是否对网络输出乘以 dt。
    """
    ...
    for _ in range(num_local_points):
        ...
        train_states.append(y0 + dt * local_std_dt_units * noise)
...
def run_experiment(
    ...
    """
    运行一组归一化消融实验，并将该组全部结果写入独立子目录。

    三组实验保持物理问题、训练初值数据集、网络结构、初始化种子、
    优化器和训练策略一致，只改变输入归一化和输出 dt 放缩设置。
    """

    experiment_name = experiment["name"]
    experiment_description = experiment["description"]
    use_normalization = experiment["use_normalization"]
    use_dt_scaling = experiment["use_dt_scaling"]

    output_dir = base_output_dir / experiment_name
    output_dir.mkdir(parents=True, exist_ok=True)

    print("\n" + "=" * 72)
    print(f"🧪 归一化消融组: {experiment_name}")
...
def main():
    ...
    print(f"📁 归一化消融总输出目录: {base_output_dir}")

    # 固定物理问题和训练初值数据集。三组实验只改变归一化设置。
    m = 1.0
    g = 9.8
    dt = 0.01
    ...
    ablation_summary = {
        ...
        "comparison_principle": (
            "All groups share the same physical problem, initial-state dataset, "
            "network architecture, model initialization seed, optimizer, and "
            "training schedule. Only input normalization and output dt scaling "
            "settings change."
        ),
        "experiments": experiment_summaries,
    }

    summary_path = base_output_dir / "normalization_ablation_summary.json"
