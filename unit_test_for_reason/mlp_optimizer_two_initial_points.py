...
# 在原始初值 y0 上添加的扰动，用来构造第二个训练初值。
# 可以直接修改这三个数值。
Y0_PERTURBATION = (0.01, 0.01, 0.01)
...
def main():
    ...
    # 对 y0 做扰动，增加第二个训练初值
    y0_perturbation = torch.tensor(Y0_PERTURBATION, dtype=y0.dtype)
    y0_perturbed = y0 + y0_perturbation
    ...
    with torch.no_grad():
        ...
        initial_delta_y0 = mlp(y0, history, params)
        initial_delta_y0_perturbed = mlp(y0_perturbed, history, params)
    ...
    print(f"训练初值 0: {y0.tolist()}")
    print(f"训练初值 1: {y0_perturbed.tolist()}")
    print(f"初值扰动: {y0_perturbation.tolist()}")
    print(f"初始化网络在初值 0 上的输出: {initial_delta_y0.tolist()}")
    print(f"初始化网络在初值 1 上的输出: {initial_delta_y0_perturbed.tolist()}")
    print(
    ...
    report = {
        "config": {
            ...
            "y0": y0.tolist(),
            "y0_perturbation": y0_perturbation.tolist(),
            "training_initial_points": [
                point.tolist()
