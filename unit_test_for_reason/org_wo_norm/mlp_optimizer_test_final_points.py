...
# 在解析解 y_star 周围添加高斯扰动，构造训练初值数据集。
# 扰动标准差设置为 DT_PERTURB_STD_SCALE * dt。
NUM_PERTURBED_INITIAL_POINTS = 15
DT_PERTURB_STD_SCALE = 0.1
...
def main():
    ...
    # 在解析解 y_star 附近做高斯扰动，构造训练初值数据集
    perturb_std = DT_PERTURB_STD_SCALE * dt
    y_star_perturbations = torch.randn(
        NUM_PERTURBED_INITIAL_POINTS,
        *y_star.shape,
        dtype=y_star.dtype,
    ...
    training_initial_points = [
        y0,
        *[y_star + perturbation for perturbation in y_star_perturbations],
    ]
    ...
    print(f"高斯扰动标准差: {perturb_std:.8f}")
    ...
    report = {
        "config": {
            ...
            "num_perturbed_initial_points": NUM_PERTURBED_INITIAL_POINTS,
            "dt_perturb_std_scale": DT_PERTURB_STD_SCALE,
            "perturb_std": perturb_std,
            "y0": y0.tolist(),
            "training_dataset_center": y_star.tolist(),
            "y_star_perturbations": y_star_perturbations.tolist(),
            "training_initial_points": [
                point.tolist()
