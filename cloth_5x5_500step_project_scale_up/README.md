...
本目录由原 scale-up 项目等价迁移而来，并保持原实验逻辑；它在 `cloth_5x5_500step_project` 的基础上，实现一个**不依赖预生成 reference trajectory** 的大规模布料神经迭代求解器训练流程。

项目将场景构造、批量物理、在线训练池、显存测试、双验证、checkpoint 选择和最终测试拆成独立模块。默认使用 `torch.float64` 和 `cuda:0`。

> **5×5 迁移说明：** catalogue、训练池、损失、验证、checkpoint、baseline 和 rollout 逻辑保持不变；仅将默认规则网格从 15×15 改为 5×5，并同步默认输出目录、状态维数、拓扑统计和测试断言。
...
```text
cloth_5x5_500step_project_scale_up/
...
| `cloth03_training_pool.py` | 75D learned optimizer、在线训练池、loss 和训练 step |
...
```bash
cd /data/zhoucy/sim_newton/cloth_5x5_500step_project_scale_up
```
...
```text
cloth_5x5_scale_up_pipeline/pipeline_logs/
├── 01_memory_probe.log
├── 02_training.log
...
```text
cloth_5x5_scale_up_pipeline/pipeline_state.json
```
...
python cloth01_build_scenario_catalogue.py \
  --output-dir cloth_5x5_scale_up_pipeline/data/scenarios
```
...
python cloth10_plot_initial_states.py \
  --root cloth_5x5_scale_up_pipeline \
...
```text
cloth_5x5_scale_up_pipeline/data/figure/
├── train_c1_1024/
├── train_c2_2048/
├── train_c3_3072/
├── validation_128/
...
```bash
--run-dir cloth_5x5_scale_up_pipeline/experiments/train_c2/activation_relu_depth_01_width_2048_no_bias/seed_42
```
...
- `--mode baseline`：只跑 baseline，不需要 checkpoint，结果缓存在 `cloth_5x5_scale_up_pipeline/single_motion_rollout_baseline/`；
...
```text
cloth_5x5_scale_up_pipeline/single_motion_rollout_baseline/test_256_motion_0000_f500_k050/
```
...
python cloth09_rollout_single_motion_compare.py \
  ...
  --seed 42 \
  --device cuda:0 \
  --checkpoint cloth_5x5_scale_up_pipeline/experiments/train_c2/activation_relu_depth_01_width_2048_no_bias/seed_42/latest_checkpoint.pt \
  --split test \
  --motion-index 0 \
  --rollout-frames 500 \
  --inner-steps 50 \
...
```text
cloth_5x5_scale_up_pipeline/experiments/train_c2/activation_relu_depth_01_width_2048_no_bias/seed_42/single_motion_rollouts/
```
...
```text
cloth_5x5_scale_up_pipeline/experiments/train_c2/*/seed_42/best_validation_model.pt
```

每个模型自己的单条 rollout 输出仍然放在模型对应目录下，例如：

```text
cloth_5x5_scale_up_pipeline/experiments/train_c2/<experiment>/seed_42/single_motion_rollout_scan/test_motion_0000_f500_k050_gd5em05_best/
```
...
```text
cloth_5x5_scale_up_pipeline/single_motion_rollout_scans/c2_test_motion_0000_f500_k050_gd5em05_best_seed_42/
```
