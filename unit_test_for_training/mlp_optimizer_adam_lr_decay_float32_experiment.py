...
# 本脚本显式使用 float32，用于复现并诊断 Adam 在最优点附近的抖动。
...
# 5. 优化器固定为 Adam，初始学习率固定为 1e-4；
# 6. 所有张量、模型参数和优化器状态均使用 torch.float32。
#
# 四组实验：
# 1. 固定 lr=1e-4，作为基线；
# 2. epoch 500 起衰减为 1e-5；
# 3. epoch 500 起衰减为 1e-6；
# 4. epoch 500 起衰减为 1e-5，epoch 750 起进一步衰减为 1e-6。
...
ABLATION_EXPERIMENTS = [
    {
        "name": "01_adam_constant_lr_1e-04",
        "description": "Float32 Adam baseline with constant lr=1e-4.",
        "optimizer_name": "adam",
        "learning_rate": 1e-4,
        "lr_milestones": [],
        "schedule_label": "constant 1e-4",
        "perturbation_std": FIXED_PERTURBATION_STD,
        "num_local_points": FIXED_NUM_PERTURBATION_POINTS,
    },
    {
        "name": "02_adam_decay_to_1e-05_at_epoch_500",
        "description": "Float32 Adam: lr=1e-4, then lr=1e-5 from epoch 500.",
        "optimizer_name": "adam",
        "learning_rate": 1e-4,
        "lr_milestones": [{"epoch": 500, "learning_rate": 1e-5}],
        "schedule_label": "1e-4 -> 1e-5 @500",
        "perturbation_std": FIXED_PERTURBATION_STD,
        "num_local_points": FIXED_NUM_PERTURBATION_POINTS,
    },
    {
        "name": "03_adam_decay_to_1e-06_at_epoch_500",
        "description": "Float32 Adam: lr=1e-4, then lr=1e-6 from epoch 500.",
        "optimizer_name": "adam",
        "learning_rate": 1e-4,
        "lr_milestones": [{"epoch": 500, "learning_rate": 1e-6}],
        "schedule_label": "1e-4 -> 1e-6 @500",
        "perturbation_std": FIXED_PERTURBATION_STD,
        "num_local_points": FIXED_NUM_PERTURBATION_POINTS,
    },
    {
        "name": "04_adam_two_stage_decay_500_750",
        "description": (
            "Float32 Adam: lr=1e-4, lr=1e-5 from epoch 500, "
            "then lr=1e-6 from epoch 750."
        ),
        "optimizer_name": "adam",
        "learning_rate": 1e-4,
        "lr_milestones": [
            {"epoch": 500, "learning_rate": 1e-5},
            {"epoch": 750, "learning_rate": 1e-6},
        ],
        "schedule_label": "1e-4 -> 1e-5 @500 -> 1e-6 @750",
        "perturbation_std": FIXED_PERTURBATION_STD,
        "num_local_points": FIXED_NUM_PERTURBATION_POINTS,
    },
]
...
def apply_learning_rate_schedule(optimizer, epoch, lr_milestones):
    """在指定 epoch 开始时应用离散学习率衰减。"""

    for milestone in lr_milestones:
        if epoch == milestone["epoch"]:
            new_learning_rate = milestone["learning_rate"]
            set_optimizer_learning_rate(optimizer, new_learning_rate)
            print(
                f"📉 Epoch {epoch}: Adam learning rate -> "
                f"{new_learning_rate:.1e}"
            )
...
def plot_epoch_diagnostics(epoch_diagnostic_log, save_path):
    """绘制每个 epoch 冻结模型后，从 y0 出发的高频诊断曲线。"""

    epochs = [item["epoch"] for item in epoch_diagnostic_log]
    gaps = [
        max(item["reference_final_gap_after_fixed_steps"], PLOT_FLOOR)
        for item in epoch_diagnostic_log
    ]
    residuals = [
        max(item["reference_final_residual_norm_after_fixed_steps"], PLOT_FLOOR)
        for item in epoch_diagnostic_log
    ]
    learning_rates = [item["learning_rate"] for item in epoch_diagnostic_log]

    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    axes[0].plot(epochs, gaps)
    axes[0].set_yscale("log")
    axes[0].set_title("Frozen y0 Eval Gap after Fixed Steps")
    axes[0].set_xlabel("Epoch")
    axes[0].set_ylabel("Gap")
    axes[0].grid(True, alpha=0.3)

    axes[1].plot(epochs, residuals)
    axes[1].set_yscale("log")
    axes[1].set_title("Frozen y0 Eval Residual after Fixed Steps")
    axes[1].set_xlabel("Epoch")
    axes[1].set_ylabel(r"Stationarity residual $\|\nabla E(y)\|_2$")
    axes[1].grid(True, alpha=0.3)

    axes[2].plot(epochs, learning_rates)
    axes[2].set_yscale("log")
    axes[2].set_title("Adam Learning Rate Schedule")
    ...
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"🖼️ 每 epoch 冻结评估诊断图已保存至: {save_path}")
...
def run_experiment(
    ...
    """
    运行一组 float32 Adam 学习率衰减实验，并将结果写入独立子目录。

    四组实验保持物理问题、训练数据集、扰动范围、噪声生成种子、
    网络结构、初始化种子、训练策略、Adam 优化器和 float32 精度一致，
    只改变后期学习率衰减策略。
    """
    ...
    lr_milestones = experiment["lr_milestones"]
    schedule_label = experiment["schedule_label"]
    ...
    print("\n" + "=" * 72)
    print(f"🧪 float32 Adam 学习率衰减组: {experiment_name}")
    ...
    print(f"initial_learning_rate={learning_rate:.1e}")
    print(f"lr_milestones={lr_milestones}")
    print(f"schedule_label={schedule_label}")
    ...
    for epoch in range(epochs):
        apply_learning_rate_schedule(opt, epoch, lr_milestones)
        current_learning_rate = get_optimizer_learning_rate(opt)
        ...
        # 每个 epoch 结束后冻结模型，从原始初值 y0 做固定步数评估。
        # 这条高频诊断曲线专门用于观察 Adam 在收敛后是否持续抖动。
        reference_diagnostic_steps = evaluate_mlp_steps(
            mlp=mlp,
            initial_y=y0,
            history=history,
            params=params,
        ...
        reference_diagnostic_final = reference_diagnostic_steps[-1]
        epoch_diagnostic_log.append(
            {
                "epoch": epoch,
                "K": K,
                "learning_rate": current_learning_rate,
                "reference_final_loss_after_fixed_steps": (
                    reference_diagnostic_final["loss"]
                ),
                "reference_final_gap_after_fixed_steps": (
                    reference_diagnostic_final["loss"] - E_star
                ),
                "reference_final_residual_norm_after_fixed_steps": (
                    reference_diagnostic_final["residual_norm"]
                ),
            }
        )
    ...
    epoch_diagnostic_rows = np.asarray(
        [
            [
                item["epoch"],
                item["K"],
                item["learning_rate"],
                item["reference_final_loss_after_fixed_steps"],
                item["reference_final_gap_after_fixed_steps"],
                item["reference_final_residual_norm_after_fixed_steps"],
            ]
            for item in epoch_diagnostic_log
        ],
        dtype=np.float64,
    ...
    np.savez_compressed(
        ...
        epoch_diagnostics=epoch_diagnostic_rows,
    ...
    report = {
        "config": {
            ...
            "final_learning_rate": get_optimizer_learning_rate(opt),
        ...
        "epoch_diagnostics": epoch_diagnostic_log,
    ...
    plot_epoch_diagnostics(
        epoch_diagnostic_log=epoch_diagnostic_log,
        save_path=output_dir / "epoch_frozen_reference_diagnostics.png",
    )
...
def plot_adam_lr_decay_summary(experiment_summaries, save_path):
    """绘制不同 Adam 学习率衰减策略对应的最终误差指标。"""

    labels = [item["schedule_label"] for item in experiment_summaries]
    ...
    final_learning_rates = [item["final_learning_rate"] for item in experiment_summaries]

    fig, axes = plt.subplots(1, 3, figsize=(20, 5))
    axes[0].bar(x - width / 2, final_reference_gaps, width, label="Reference y0 gap")
    axes[0].bar(x + width / 2, final_worst_eval_gaps, width, label="Worst eval gap")
    axes[0].set_yscale("log")
    axes[0].set_title("Final Objective Gap")
    axes[0].set_ylabel("Gap")
    axes[0].legend()
    axes[0].grid(True, alpha=0.3)

    axes[1].bar(x, final_reference_residuals)
    axes[1].set_yscale("log")
    axes[1].set_title("Final Residual from Original Initial Point")
    axes[1].set_ylabel(r"Stationarity residual $\|\nabla E(y)\|_2$")
    axes[1].grid(True, alpha=0.3)

    axes[2].bar(x, final_learning_rates)
    axes[2].set_yscale("log")
    axes[2].set_title("Final Adam Learning Rate")
    axes[2].set_ylabel("Learning rate")
    axes[2].grid(True, alpha=0.3)
    ...
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"🖼️ Adam 学习率衰减汇总图已保存至: {save_path}")


def plot_adam_lr_decay_dynamics(experiment_summaries, save_path):
    """跨组对比每个 epoch 的冻结 y0 评估曲线与学习率。"""

    fig, axes = plt.subplots(1, 3, figsize=(20, 5))
    for item in experiment_summaries:
        diagnostics = item["epoch_diagnostics"]
        epochs = [row["epoch"] for row in diagnostics]
        gaps = [
            max(row["reference_final_gap_after_fixed_steps"], PLOT_FLOOR)
            for row in diagnostics
        ]
        residuals = [
            max(row["reference_final_residual_norm_after_fixed_steps"], PLOT_FLOOR)
            for row in diagnostics
        ]
        learning_rates = [row["learning_rate"] for row in diagnostics]
        label = item["schedule_label"]

        axes[0].plot(epochs, gaps, label=label)
        axes[1].plot(epochs, residuals, label=label)
        axes[2].plot(epochs, learning_rates, label=label)
    ...
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
    print(f"🖼️ Adam 学习率衰减动态对比图已保存至: {save_path}")


# ============================================================
# 10. 主程序：依次运行四组 float32 Adam 学习率衰减实验
# ============================================================

def main():
    base_output_dir = create_output_directory()
    print(f"📁 float32 Adam 学习率衰减总输出目录: {base_output_dir}")
    ...
    ablation_summary = {
        ...
        "experiment_type": "float32_adam_learning_rate_decay",
        "torch_dtype": str(TORCH_DTYPE),
        "comparison_principle": (
            "All groups use float32 Adam with initial lr=1e-4. They share the "
            "same physical problem, fixed dataset with 10 random initial-point "
            "perturbations, sigma=1e-2, random seeds, network architecture, "
            "training schedule, input normalization, and output dt scaling. "
            "Only the late-stage learning-rate decay schedule changes."
        ),
    ...
    summary_path = base_output_dir / "adam_learning_rate_decay_summary.json"
    with summary_path.open("w", encoding="utf-8") as file:
        json.dump(ablation_summary, file, indent=2, ensure_ascii=False)

    summary_plot_path = base_output_dir / "adam_learning_rate_decay_summary.png"
    plot_adam_lr_decay_summary(
        experiment_summaries=experiment_summaries,
        save_path=summary_plot_path,
    )

    dynamics_plot_path = base_output_dir / "adam_learning_rate_decay_dynamics.png"
    plot_adam_lr_decay_dynamics(
        experiment_summaries=experiment_summaries,
        save_path=dynamics_plot_path,
    )

    print("\n" + "=" * 72)
    print("✅ 四组 float32 Adam 学习率衰减实验全部完成。")
    print(f"📁 总输出目录: {base_output_dir}")
    print(f"📄 汇总文件: {summary_path}")
    print(f"🖼️ 汇总图片: {summary_plot_path}")
    print(f"🖼️ 动态对比图: {dynamics_plot_path}")
    for item in experiment_summaries:
        print(
            f"- {item['experiment_name']}: "
            f"schedule={item['schedule_label']}，"
            f"final_lr={item['final_learning_rate']:.1e}，"
            f"最终参考 gap={item['final_reference_gap_after_fixed_steps']:.4e}，"
            f"最终 residual={item['final_reference_residual_norm_after_fixed_steps']:.4e}"
        )
