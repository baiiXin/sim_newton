...
def run_experiment(
    ...
    训练策略：
        ...
        - 从该随机起点连续执行当前 K 次 MLP 更新；
        - 步间 detach，每一步单独反向传播并更新参数。
...
# ============================================================
# 11. 主程序：依次运行两组 float32 在线随机扰动训练实验
# ============================================================

def main():
    base_output_dir = create_output_directory()
    print(f"📁 float32 在线随机扰动训练总输出目录: {base_output_dir}")
    ...
    ablation_summary = {
        ...
        "experiment_type": "float32_online_resampled_y0_perturbation_training",
        "torch_dtype": str(TORCH_DTYPE),
        "comparison_principle": (
            ...
            "initial state as y_epoch = y0 + sigma * epsilon. Evaluation and "
    ...
    summary_path = base_output_dir / "float32_online_resampled_training_summary.json"
    with summary_path.open("w", encoding="utf-8") as file:
        json.dump(ablation_summary, file, indent=2, ensure_ascii=False)

    summary_plot_path = base_output_dir / "float32_online_resampled_training_summary.png"
    ...
    print("\n" + "=" * 72)
    print("✅ 两组 float32 在线随机扰动训练实验全部完成。")
