"""
500-frame continuous rollout for the fixed-left-edge 15x15 triangular cloth.

The script loads the validation-selected multi-problem MLP checkpoint produced
by ``fixed_left_edge_15x15_cloth_train_compare.py``. Starting from the hardest extrapolation case whose physical current state
converges under the validation-selected MLP to a final residual below 1e-4, it
...
physical frame. There is no convergence-based early stopping. If an update becomes
non-finite, the solver keeps its last finite iterate for that frame, records the
frame and inner iteration, and continues the rollout. Each method uses its own
predicted position and velocity to construct the next frame, so errors are
allowed to accumulate naturally.
...
import fixed_left_edge_15x15_cloth_train_compare as core
...
def parse_args() -> argparse.Namespace:
    ...
    default_training_output = (
        Path(__file__).resolve().parent / "fixed_left_edge_15x15_cloth_train_compare"
    )
    parser.add_argument(
        "--training-output-dir",
        type=Path,
        default=default_training_output,
        help="Output directory created by fixed_left_edge_15x15_cloth_train_compare.py",
    )
    parser.add_argument(
        "--checkpoint",
        type=Path,
...
def solve_fixed_iterations(
    *,
    ...
    """Solve one frame using exactly ``inner_iterations`` update attempts."""
    ...
    nonfinite_iteration: int | None = None
    ...
    for iteration in range(inner_iterations):
        try:
            ...
            if raise_on_nonfinite:
                raise
            nonfinite_iteration = iteration + 1
            nonfinite_reason = str(exc)
            break
        ...
        if not bool(torch.isfinite(y_next).all()):
            ...
            nonfinite_iteration = iteration + 1
            nonfinite_reason = "updated state contains NaN or Inf"
            break
        if not bool(torch.isfinite(delta).all()):
            if raise_on_nonfinite:
                raise RuntimeError(
                    f"{solver} produced non-finite update at inner iteration {iteration + 1}"
                )
            nonfinite_iteration = iteration + 1
            nonfinite_reason = "update contains NaN or Inf"
            break
    ...
    return next_p, next_v, {
        ...
        "nonfinite_iteration": nonfinite_iteration,
        "nonfinite_reason": nonfinite_reason,
        "completed_inner_iterations": (
            inner_iterations if nonfinite_iteration is None else nonfinite_iteration - 1
        ),
    }
    ...
    *,
    ...
    for frame in range(1, frames + 1):
        ...
        for solver_name in ["learned", "gradient_descent", "full_newton"]:
            ...
            if info.get("nonfinite_iteration") is not None:
                event = {
                    "solver": solver_name,
                    "frame": frame,
                    "physical_time": frame * physical.dt,
                    "inner_iteration": int(info["nonfinite_iteration"]),
                    "completed_inner_iterations": int(info["completed_inner_iterations"]),
                    "reason": info.get("nonfinite_reason"),
                    "last_finite_residual": float(info["residual"]),
                    "last_finite_energy": float(info["energy"]),
                }
                nonfinite_events.append(event)
                print(
                    f"Warning: {solver_name} produced a non-finite update at frame {frame}, "
                    f"inner iteration {info['nonfinite_iteration']}; keeping last finite iterate. "
                    f"Reason: {info.get('nonfinite_reason')}"
                )
...
def render_video(
    *,
    ...
    def update(video_index: int) -> list[Any]:
        ...
        fig.suptitle(
            "Fixed left edge 15x15 triangular cloth: exactly 50 inner iterations per frame",
            fontsize=15,
        )
        return returned
    ...
    mp4_path = output_dir / "fixed_left_edge_15x15_cloth_500_frames.mp4"
    gif_path = output_dir / "fixed_left_edge_15x15_cloth_500_frames.gif"
...
def plot_worst_case_error_curves(
    rollout: dict[str, Any], physical: core.PhysicalConfig, frames: int, save_path: Path
) -> None:
    times = np.arange(1, frames + 1) * physical.dt
    labels = {
    ...
    for solver_name, label in labels.items():
        values = np.maximum(
            np.asarray(rollout["errors"][solver_name]["position_max"], dtype=float),
            ERROR_FLOOR,
        )
        ax.plot(times, values, label=label)
    ax.set_yscale("log")
    ax.set_xlabel("Physical time")
    ax.set_ylabel("Maximum free-vertex position error")
    ax.set_title("Continuous-rollout worst-case position error")
    ax.grid(True, alpha=0.3)
    ax.legend()
    plt.tight_layout()
    plt.savefig(save_path, dpi=230, bbox_inches="tight")
...
def plot_nonfinite_events(
    rollout: dict[str, Any], physical: core.PhysicalConfig, frames: int, save_path: Path
) -> None:
    events = rollout.get("nonfinite_events", [])
    solver_names = ["learned", "gradient_descent", "full_newton"]
    labels = {
        "learned": "MLP",
        "gradient_descent": "gradient descent",
        "full_newton": "full Newton",
    ...
    for solver_name in solver_names:
        solver_events = [e for e in events if e["solver"] == solver_name]
        if solver_events:
            any_event = True
            axes[0].scatter(
                [e["physical_time"] for e in solver_events],
                [e["inner_iteration"] for e in solver_events],
                label=labels[solver_name],
            )
        event_frames = {int(e["frame"]) for e in solver_events}
        cumulative = np.cumsum(
            [1 if frame in event_frames else 0 for frame in range(1, frames + 1)]
        )
        axes[1].plot(
            np.arange(1, frames + 1) * physical.dt,
            cumulative,
            label=labels[solver_name],
        )
    ...
    if not any_event:
        axes[0].text(
            0.5,
            0.5,
            "No non-finite update events",
            ha="center",
            va="center",
            transform=axes[0].transAxes,
        )
    axes[0].set_xlabel("Physical time")
    axes[0].set_ylabel("First non-finite inner iteration")
    axes[0].set_title("Locations of non-finite update events")
    axes[0].grid(True, alpha=0.3)
    if any_event:
        axes[0].legend()

    axes[1].set_xlabel("Physical time")
    axes[1].set_ylabel("Cumulative event count")
    axes[1].set_title("Accumulated non-finite events")
...
def diagnostics_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if records and "residual" in records[0]:
        ...
        result.update(
            ...
            median_residual=float(np.median(finite_residuals)) if finite_residuals.size else float("nan"),
            p95_residual=float(np.percentile(finite_residuals, 95)) if finite_residuals.size else float("nan"),
            max_residual=float(np.max(finite_residuals)) if finite_residuals.size else float("nan"),
            nonfinite_residual_frames=int(np.count_nonzero(~np.isfinite(residuals))),
    ...
    if records and "nonfinite_iteration" in records[0]:
        nonfinite = np.asarray(
            [r.get("nonfinite_iteration") is not None for r in records], dtype=bool
        )
        result.update(
            nonfinite_frames=int(np.count_nonzero(nonfinite)),
            nonfinite_frame_indices=[
                int(i + 1) for i, flag in enumerate(nonfinite.tolist()) if flag
            ],
            nonfinite_inner_iterations=[
                int(r["nonfinite_iteration"])
                for r in records
                if r.get("nonfinite_iteration") is not None
            ],
        )
    return result
...
def main() -> None:
    ...
    plot_worst_case_error_curves(
        rollout,
        physical,
        int(args.frames),
        output_dir / "rollout_worst_case_error_curves.png",
    )
    plot_nonfinite_events(
        rollout,
        physical,
        int(args.frames),
        output_dir / "rollout_nonfinite_events.png",
    )
    ...
    report = {
        ...
        "nonfinite_event_count": len(rollout["nonfinite_events"]),
        "nonfinite_events": rollout["nonfinite_events"],
        "nonfinite_handling": (
            "For MLP, gradient descent, and full Newton, keep the last finite "
            "iterate of the frame, record the event, and continue the rollout."
        ),
        "video_path": str(video_path) if video_path is not None else None,
        "trajectory_file": str(output_dir / "rollout_trajectories.npz"),
        "worst_case_plot": str(output_dir / "rollout_worst_case_error_curves.png"),
        "nonfinite_event_plot": str(output_dir / "rollout_nonfinite_events.png"),
    }
    ...
    save_json(
        {
            ...
            "final_residual_threshold": hard_case.get("final_residual_threshold"),
            "selected_candidate_metrics": hard_case.get("selected"),
        },
    ...
    print(f"Worst-case curves: {output_dir / 'rollout_worst_case_error_curves.png'}")
    print(f"Non-finite events: {output_dir / 'rollout_nonfinite_events.png'}")
