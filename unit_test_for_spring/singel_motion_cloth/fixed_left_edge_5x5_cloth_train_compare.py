...
-----------------------------------------------------------------------
1. The physical system is a 5x5 cloth-like triangular spring mesh with two
   fixed vertices: the left-top and left-bottom corners.
2. Only the remaining 23 vertices are optimization variables, so the learned
   optimizer input/output dimension is 69.
3. The spring network is exactly the edge set of a triangulated 5x5 mesh:
      - 20 horizontal edges,
      - 20 vertical edges,
      - 16 alternating cell diagonals.
...
6. A hard extrapolation-test physical time step is selected independently of
...
def default_physical_config() -> PhysicalConfig:
    # 5x5 cloth patch. Vertices are stored in row-major order.
...
def generate_sobol_points(
    *,
    ...
    return points, {
        "mode": "scrambled_sobol_69d_linf_cube",
        "seed": seed,
        "count": count,
...
def plot_gradient_descent_step_size_selection(
    ...
    ax.plot(alphas, np.maximum(residual_p95, PLOT_FLOOR), marker="o")
    ax.scatter([selected], [max(residual_p95[selected_index], PLOT_FLOOR)], s=90, zorder=3, label=f"selected {selected:.1e}")
    ax.axvline(selected, linestyle="--", alpha=0.75)
    ax.set_xscale("log")
    ax.set_yscale("log")
    ax.set_xlabel("Gradient-descent step size")
    ax.set_ylabel("Validation final residual p95 after fixed iterations")
...
def run_experiment(
    *,
    ...
    print(
        f"architecture=69->69->Identity->69, points={len(training_cpu):,}, "
    ...
    report = {
        ...
        "model": {
            "architecture": "69D dimensionless mass-preconditioned residual -> 69 -> Identity -> 69D update",
            "bias_free": True,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fixed-left-edge 5x5 triangular cloth learned optimizer with GD/Newton comparison"
    )
...
def main() -> None:
    ...
    summary = {
        "experiment_type": "fixed_left_edge_5x5_cloth_independent_time_step_generalization",
        "runtime_config": asdict(config),
        "physical_config": asdict(physical),
        "problem_split": asdict(split),
        "physics_checks": physics_checks,
