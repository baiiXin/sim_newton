"""
Fixed-left-edge 15x15 triangular cloth learned optimizer experiment.
...
def run_experiment(
    *,
    ...
    print(
        f"architecture={FREE_STATE_DIM}->{HIDDEN_DIM}->Identity->{FREE_STATE_DIM}, points={len(training_cpu):,}, "
    ...
    report = {
        ...
        "model": {
            "architecture": (
                ...
                f"{HIDDEN_DIM} -> Identity -> {FREE_STATE_DIM}D update"
            ),
            "bias_free": True,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Fixed-left-edge 15x15 triangular cloth learned optimizer with GD/Newton comparison"
    )
...
def main() -> None:
    ...
    summary = {
        "experiment_type": "fixed_left_edge_15x15_cloth_independent_time_step_generalization",
        "runtime_config": asdict(config),
        "physical_config": asdict(physical),
        "problem_split": asdict(split),
        "physics_checks": physics_checks,
