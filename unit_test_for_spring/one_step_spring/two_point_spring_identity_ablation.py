"""
Two-particle single-spring, single-step learned optimizer ablation.

Fixed activation: identity
Default device: cuda:1
Numerical precision: torch.float64

Experiment matrix inside this script:
- Training sizes: 10, 100, 1_000, 10_000, 100_000
- Optimizers:
  * SGD(lr=1e-2), applied network output multiplied by dt
  * Adam(lr=1e-3), raw network output
  * Adam(lr=1e-4), raw network output
...
# ============================================================
# 0. Fixed script identity and default experiment parameters
# ============================================================
...
def parse_optimizer_configs(values: Sequence[str] | None) -> list[dict[str, Any]]:
    ...
    for raw in values:
        ...
        matched_key = None
        for key in allowed:
            if name == key[0] and math.isclose(learning_rate, key[1], rel_tol=1e-12):
                matched_key = key
                break
        if matched_key is None:
            raise ValueError(
                "Only sgd:1e-2, adam:1e-3, and adam:1e-4 are supported "
                "because output scaling is tied to these experiment configurations."
            )
        if matched_key not in seen:
            parsed.append(
                {
                    "optimizer_name": matched_key[0],
                    "learning_rate": matched_key[1],
                    "output_scale_mode": allowed[matched_key],
                }
            )
...
class MLPOptimizer(nn.Module):
    """17 -> 64 -> fixed activation -> 6 learned iterative update."""
    ...
    def __init__(self, input_mean: torch.Tensor, input_std: torch.Tensor) -> None:
        ...
        if ACTIVATION_NAME == "identity":
            ...
            raise ValueError(f"Unsupported fixed activation {ACTIVATION_NAME!r}")
...
def ordered_optimizer_keys(records: Sequence[dict[str, Any]]) -> list[tuple[str, float, str]]:
    preferred = [
        ("sgd", 1e-2, "dt"),
        ("adam", 1e-3, "raw"),
        ("adam", 1e-4, "raw"),
    ]
    present = {optimizer_key(record) for record in records}
...
def plot_final_test_summary(records: Sequence[dict[str, Any]], save_path: Path) -> None:
    ...
    for key in ordered_optimizer_keys(records):
        selected = sorted(
    ...
    for ax in axes.reshape(-1):
        ...
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=7)
    fig.suptitle(f"{ACTIVATION_NAME} activation: validation-selected test summary", y=1.002)
    plt.tight_layout()
    plt.savefig(save_path, dpi=250, bbox_inches="tight")
    plt.close(fig)
...
def plot_training_loss_summary(records: Sequence[dict[str, Any]], save_path: Path) -> None:
    keys = ordered_optimizer_keys(records)
    sizes = sorted({int(record["dataset_size"]) for record in records})
    fig, axes = plt.subplots(len(keys), len(sizes), figsize=(6 * len(sizes), 4.8 * len(keys)), squeeze=False)
    for row, key in enumerate(keys):
        for col, size in enumerate(sizes):
            ax = axes[row, col]
            record = next(
                item for item in records
                if optimizer_key(item) == key and int(item["dataset_size"]) == size
            )
            ...
            ax.grid(True, alpha=0.3)
    fig.suptitle(f"{ACTIVATION_NAME} activation: training curves", y=1.01)
    plt.tight_layout()
    plt.savefig(save_path, dpi=250, bbox_inches="tight")
    plt.close(fig)
    ...
    for row, key in enumerate(keys):
        for col, size in enumerate(sizes):
            ...
            if record["best_validation_epoch"] is not None:
                ax.axvline(record["best_validation_epoch"], linestyle="--", alpha=0.7)
            ax.set_yscale("log")
            ax.set_title(f"{optimizer_label(key)}\nN={size:,}")
            ax.set_xlabel("Epoch")
            ax.set_ylabel("Validation residual p95")
            ax.grid(True, alpha=0.3)
    fig.suptitle(f"{ACTIVATION_NAME} activation: validation checkpoint curves", y=1.01)
    plt.tight_layout()
    plt.savefig(save_path, dpi=250, bbox_inches="tight")
    plt.close(fig)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            f"Two-particle spring learned-optimizer ablation with fixed "
            f"{ACTIVATION_NAME} activation."
        )
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Subset of: sgd:1e-2 adam:1e-3 adam:1e-4.",
    )
...
def main() -> None:
    ...
    print(f"Fixed activation: {ACTIVATION_NAME}")
    ...
    overall_report = {
        "experiment_type": "two_particle_single_spring_activation_script",
        "activation_name": ACTIVATION_NAME,
        "purpose": (
            "Train a repeatable MLP optimizer for one fixed nonlinear two-particle "
            "spring variational problem. Vary training-set size and network-parameter "
            "optimizer while keeping the physical problem and evaluation split fixed."
        ),
        "runtime_config": asdict(config),
        "network": {
            "architecture": f"17 -> 64 -> {ACTIVATION_NAME} -> 6",
        ...
        "optimizer_output_scaling": {
            ...
            "adam_1e-3": "raw network output",
            "adam_1e-4": "raw network output",
        },
        "physical_problem": {
            **asdict(problem),
            "y_star": tensor_to_list(problem_tensors.y_star),
