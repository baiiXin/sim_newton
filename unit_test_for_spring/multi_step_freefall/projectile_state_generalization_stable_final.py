...
- 网络：3维无量纲变分残差 -> 32 -> activation -> 3维无量纲更新
- 输入：无量纲变分残差 (dt^2/m)∇E / initial_radius
- 输出乘 initial_radius=1e-2，使网络原始输出保持 O(1)
- 训练 loss：无量纲变分能量差；与原能量只差正比例和常数，不使用监督标签
- 数值精度：torch.float64
...
def scaled_residual_vector(
    ...
    对当前抛体二次能量，它严格等于 y - y*，但计算时只使用
    当前迭代解和物理量，不使用精确解标签。
    """
    return (physics.dt * physics.dt / physics.mass) * residual_vector(
        y, positions, velocities, physics
    )
...
def normalized_variational_objective(
    ...
    无量纲变分目标：
        ...
        L_hat(y) = 1/2 || ((dt^2/m) ∇E(y)) / residual_scale ||^2.

    对当前二次问题：

        L_hat(y)
        = dt^2 / (m * residual_scale^2) * (E(y) - E(y*)).

    因而它与原始变分能量具有相同极小点和同向梯度，只改变梯度尺度，
    并且不需要把 y* 作为训练监督标签。
    """
    normalized_residual = (
        scaled_residual_vector(y, positions, velocities, physics)
        / residual_scale
    )
    return 0.5 * torch.sum(normalized_residual * normalized_residual, dim=-1)
...
class MLPOptimizer(nn.Module):
    ...
    输入：
        u = ((dt^2 / m) * ∇E(y)) / residual_scale

    输出：
        delta_y = residual_scale * MLP(u)

    两层均不使用 bias，因此当 ∇E(y)=0 时，网络严格输出 0，
    精确解天然是迭代固定点。第一层使用正交初始化；输出层仍为零初始化，
    因而初始网络输出严格为 0，但第一步输出层更新会直接利用残差方向。
    """
    ...
    def __init__(
        ...
        self.register_buffer(
            "residual_scale",
            torch.tensor(float(residual_scale), dtype=DTYPE),
        )

        self.linear1 = nn.Linear(3, hidden_dim, bias=False)
        self.activation = activation_module(activation)
        self.linear2 = nn.Linear(hidden_dim, 3, bias=False)

        # 让初始随机特征在三个残差方向上尺度一致。
        nn.init.orthogonal_(self.linear1.weight)

        # 保留“初始输出严格为 0”的设置。第一步只有输出层更新，
        # 但由于输入直接是残差，第一次输出方向不再来自绝对状态的随机混合。
        nn.init.zeros_(self.linear2.weight)

    def forward(self, y: Tensor, positions: Tensor, velocities: Tensor) -> Tensor:
        normalized_residual = (
            scaled_residual_vector(y, positions, velocities, self.physics)
            / self.residual_scale
        )
        hidden = self.activation(self.linear1(normalized_residual))
        raw_update = self.linear2(hidden)
        return self.residual_scale * raw_update
...
def plot_training_gap(
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Positive training metric")
    plt.title("Training metric on log scale")
    plt.yscale("log")
    plt.grid(True, alpha=0.3)
    plt.tight_layout()
    plt.savefig(output_path, dpi=180)
...
def one_step_quality_diagnostics(
    ...
    """检查当前模型的一步更新是否真的改善当前 batch。"""
    delta = model(initial_y, positions, velocities)
    updated_y = initial_y + delta

    before_error = torch.linalg.vector_norm(initial_y - exact_y, dim=-1)
    after_error = torch.linalg.vector_norm(updated_y - exact_y, dim=-1)

    before_energy = variational_energy(initial_y, positions, velocities, physics)
    after_energy = variational_energy(updated_y, positions, velocities, physics)

    ideal_update = exact_y - initial_y
    flat_delta = delta.reshape(-1)
    flat_ideal = ideal_update.reshape(-1)
    denominator = torch.linalg.vector_norm(flat_delta) * torch.linalg.vector_norm(flat_ideal)
    if float(denominator.item()) > 0.0:
        cosine = float(torch.dot(flat_delta, flat_ideal).item() / denominator.item())
    else:
        cosine = float("nan")

    return {
        "mean_error_before": float(before_error.mean().item()),
        "mean_error_after": float(after_error.mean().item()),
        ...
        "sample_error_improvement_fraction": float(
            (after_error < before_error).to(torch.float64).mean().item()
        ),
        "sample_energy_improvement_fraction": float(
            (after_energy < before_energy).to(torch.float64).mean().item()
        ),
    }
    ...
    torch.save(
        {
            ...
            "input_definition": "((dt^2/m) * variational residual) / residual_scale",
            "output_definition": "residual_scale * raw network output",
...
def train_one_experiment(
    ...
    training_objective_history: List[float] = []
    raw_loss_history: List[float] = []
    gap_history: List[float] = []
    k_history: List[int] = []
    one_step_diagnostic_history: List[Dict[str, Any]] = []
    ...
    for epoch in range(1, training_cfg.epochs + 1):
        ...
        training_objective = torch.zeros((), device=device, dtype=DTYPE)
        ...
        initial_y_for_diagnostics = y.detach().clone()
        ...
        for _ in range(k_steps):
            ...
            # 真正用于反向传播的目标。它与原始能量只差一个正常数和
            # 与网络参数无关的常数，但梯度尺度与 O(1) 网络输出匹配。
            ...
            # 以下两项只用于保持原实验的物理量监控输出。
            raw_loss = raw_loss + current_energy.mean()
            monitoring_gap = monitoring_gap + (
                current_energy - exact_energy
            ).mean()

        if not torch.isfinite(training_objective):
            stopped_reason = f"epoch {epoch}: training objective is non-finite"
            print(f"[{spec.name}] {stopped_reason}")
            break

        training_objective.backward()
        gradient_norm = float(
            torch.nn.utils.clip_grad_norm_(
                model.parameters(),
                max_norm=training_cfg.gradient_clip_norm,
            ).item()
        )
        optimizer.step()
        ...
        training_objective_history.append(
            float(training_objective.detach().cpu().item())
        )
        ...
        if should_record_diagnostics:
            diagnostics = one_step_quality_diagnostics(
                model=model,
                initial_y=initial_y_for_diagnostics,
                positions=p_flat,
                velocities=v_flat,
                exact_y=y_star_flat,
                physics=physics,
            ...
            diagnostics["gradient_norm_before_clip"] = gradient_norm
            one_step_diagnostic_history.append(diagnostics)
        ...
        if (
            ...
            message = (
                ...
                f"K={k_steps}, "
                f"objective={training_objective_history[-1]:.8e}, "
                f"raw_loss={raw_loss_history[-1]:.8e}, "
                f"gap={gap_history[-1]:.8e}"
            )
            if one_step_diagnostic_history and one_step_diagnostic_history[-1]["epoch"] == epoch:
                diagnostic = one_step_diagnostic_history[-1]
                message += (
    ...
    plot_training_gap(
        ...
        experiment_dir / "training_normalized_variational_objective.png",
    )
    plot_training_gap(
        epoch_axis,
        gap_history,
    ...
    report = {
        ...
        "training_objective_history": training_objective_history,
        "raw_loss_history": raw_loss_history,
        "training_gap_history": gap_history,
        "one_step_diagnostic_history": one_step_diagnostic_history,
        "residual_scale": float(residual_scale),
        "network_input": "dimensionless scaled variational residual",
        "network_output": "residual_scale times raw network output",
        "training_objective": (
            "0.5 * ||((dt^2/m) * residual) / residual_scale||^2; "
            "equivalent to scaled physical energy gap"
        ),
        "k_history": k_history,
...
def main() -> None:
    ...
    print("=" * 80)
    print("跨运动状态的单步抛体变分优化器实验（稳定残差输入版）")
    ...
    residual_scale = float(data_cfg.initial_radius)
    ...
    torch.save(
        {
            ...
            "residual_scale": residual_scale,
            "network_input": "((dt^2/m) * variational residual) / residual_scale",
            "network_output": "residual_scale * raw network output",
    ...
    save_json(
        output_dir / "fixed_dataset_split.json",
        {
            ...
            "notes": {
                ...
                "training_loss": (
                    "dimensionless variational objective computed from the "
                    "scaled residual; exactly proportional to energy gap"
                ),
                "fixed_point_property": (
                    "bias-free network maps zero residual to zero update"
                ),
                "problem_batching": (
