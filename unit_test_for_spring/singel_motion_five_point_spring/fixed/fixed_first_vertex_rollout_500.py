"""
500-frame continuous rollout for the fixed-first-vertex spring chain.

The script loads the validation-selected multi-problem MLP checkpoint produced
by ``fixed_first_vertex_train_compare.py``. Starting from the solver-independent
hard extrapolation case selected by the training script, it propagates four
...
import fixed_first_vertex_train_compare as core
...
def parse_args() -> argparse.Namespace:
    ...
    default_training_output = (
        Path(__file__).resolve().parent / "fixed_first_vertex_train_compare"
    )
    parser.add_argument(
        "--training-output-dir",
        type=Path,
        default=default_training_output,
        help="Output directory created by fixed_first_vertex_train_compare.py",
    )
    parser.add_argument(
        "--checkpoint",
        type=Path,
...
def solve_fixed_iterations(
    *,
    ...
    y = p_full[1:, :].reshape(1, core.FREE_STATE_DIM).clone()
...
def solve_reference_frame(
    *,
    ...
    initial_y = p_full[1:, :].reshape(core.FREE_STATE_DIM)
    start_time = time.perf_counter()
...
def free_position_errors(
    ...
    point_error = torch.linalg.vector_norm(
        prediction[1:, :] - reference[1:, :], dim=-1
    )
...
def run_rollout(
    *,
    ...
    free_masses_device = torch.tensor(
        physical.masses[1:], dtype=TORCH_DTYPE, device=device
    )
    free_masses_cpu = torch.tensor(physical.masses[1:], dtype=TORCH_DTYPE)
...
def create_chain_artists(ax: Any, label: str) -> dict[str, Any]:
    line, = ax.plot([], [], [], marker="o", linewidth=2, label=label)
    anchor, = ax.plot([], [], [], marker="s", markersize=8, linestyle="None", label="fixed anchor")
    return {"line": line, "anchor": anchor}


def update_chain_artists(artists: dict[str, Any], points: np.ndarray) -> None:
    artists["line"].set_data(points[:, 0], points[:, 1])
    artists["line"].set_3d_properties(points[:, 2])
    artists["anchor"].set_data([points[0, 0]], [points[0, 1]])
    artists["anchor"].set_3d_properties([points[0, 2]])


def create_comparison_artists(ax: Any) -> dict[str, Any]:
    prediction, = ax.plot([], [], [], marker="o", linewidth=2, label="solver")
    reference, = ax.plot([], [], [], marker="x", linewidth=1.5, linestyle="--", label="reference")
    connectors = [ax.plot([], [], [], linewidth=0.8, alpha=0.7)[0] for _ in range(core.NUM_FREE_PARTICLES)]
    ax.legend(fontsize=8)
    return {"prediction": prediction, "reference": reference, "connectors": connectors}


def update_comparison_artists(
    artists: dict[str, Any], prediction: np.ndarray, reference: np.ndarray
) -> None:
    artists["prediction"].set_data(prediction[:, 0], prediction[:, 1])
    artists["prediction"].set_3d_properties(prediction[:, 2])
    artists["reference"].set_data(reference[:, 0], reference[:, 1])
    artists["reference"].set_3d_properties(reference[:, 2])
    for index, connector in enumerate(artists["connectors"], start=1):
        pair = np.stack([prediction[index], reference[index]], axis=0)
        connector.set_data(pair[:, 0], pair[:, 1])
        connector.set_3d_properties(pair[:, 2])
...
def render_video(
    *,
    ...
    top_artists = [
        create_chain_artists(ax, title) for ax, title in zip(axes_top, top_titles)
    ]
    for ax in axes_top:
        ax.legend(fontsize=8)

    mlp_diff_artists = create_comparison_artists(ax_mlp_diff)
    gd_diff_artists = create_comparison_artists(ax_gd_diff)
    ...
    ax_error.set_ylabel("Free-particle position RMS error")
    ...
    def update(video_index: int) -> list[Any]:
        ...
        for solver_name, title, ax, artists in zip(
            ...
            update_chain_artists(artists, current)
        ...
        update_comparison_artists(mlp_diff_artists, mlp_current, reference)
        update_comparison_artists(gd_diff_artists, gd_current, reference)
        ...
        returned.extend(
            [
                mlp_diff_artists["prediction"],
                mlp_diff_artists["reference"],
                *mlp_diff_artists["connectors"],
                gd_diff_artists["prediction"],
                gd_diff_artists["reference"],
                *gd_diff_artists["connectors"],
            ]
        )
        ...
        for solver_name, line in error_lines.items():
            y = np.maximum(
                np.asarray(errors[solver_name]["position_rms"][:state_index]),
                ERROR_FLOOR,
            )
            line.set_data(x, y)
            returned.append(line)
        fig.suptitle(
            "Fixed first vertex: continuous rollout with exactly 50 inner iterations per frame",
            fontsize=15,
        )
        return returned
    ...
    mp4_path = output_dir / "fixed_anchor_500_frames.mp4"
    gif_path = output_dir / "fixed_anchor_500_frames.gif"
...
def main() -> None:
    ...
    initial_p = tensor_state(
        selected_state["p_n_full"], device=torch.device("cpu"), shape=(5, 3)
    )
    initial_v = tensor_state(
        selected_state["v_n_full"], device=torch.device("cpu"), shape=(5, 3)
    )
    initial_p[0] = torch.tensor(physical.anchor, dtype=TORCH_DTYPE)
    initial_v[0] = 0.0
