...
- Network family: plain 3x3 Conv2d field optimizer; no Metamizer-style per-sample normalization and no norm layers.

Default experiment grid: activation=ReLU, depth=3, width=64,
...
class MLPOptimizer(nn.Module):
    """History-based full-state plain convolutional learned optimizer.

    This is the pure-convolution variant: it treats the 5x5 cloth as a spatial
    grid, uses 9 input channels [current residual xyz, previous residual xyz,
    previous update xyz], and predicts a 3-channel update field. It intentionally
    does not apply Metamizer-style per-sample normalization, BatchNorm, GroupNorm,
    or any other normalization layer.
    """
    ...
    def __init__(self, residual_length_scale: float, model_spec: ModelSpec) -> None:
        ...
        self.hidden_convs = nn.ModuleList(
            ...
            for _ in range(max(model_spec.depth - 1, 0))
        )
        ...
        nn.init.orthogonal_(self.input_conv.weight, gain=gain)
    ...
    def architecture_description(self) -> str:
        ...
        return (
            f"plain Conv2d optimizer on 5x5 grid: "
            f"B x 9 x {GRID_ROWS} x {GRID_COLS} input "
            f"[current residual, previous residual, previous update] -> "
            f"{self.model_spec.depth} hidden conv layers with "
            f"{self.model_spec.width} channels and {self.model_spec.activation} -> "
    ...
    def forward(
        ...
        *,
        ...
        h = self.activation(self.input_conv(h))
        for layer in self.hidden_convs:
