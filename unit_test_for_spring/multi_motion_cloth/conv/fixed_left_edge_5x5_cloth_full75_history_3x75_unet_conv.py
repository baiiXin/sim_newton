...
- Network family: small 5x5 U-Net field optimizer; no Metamizer-style per-sample normalization and no norm layers.

Default experiment grid: activation=ReLU, depth=1, width=64,
all convolution layers are bias-free. Previous history script defaulted to cuda:0;
this variant defaults to cuda:1.
"""
...
class MLPOptimizer(nn.Module):
    """History-based full-state small U-Net learned optimizer.

    This is the 5x5 Metamizer-inspired U-Net variant: it treats the cloth as a
    spatial grid, uses 9 input channels [current residual xyz, previous residual
    xyz, previous update xyz], performs one downsample/upsample stage with a skip
    connection, and predicts a 3-channel update field. It intentionally does not
    apply Metamizer-style per-sample normalization, BatchNorm, GroupNorm, or any
    other normalization layer.
    """
    ...
    def __init__(self, residual_length_scale: float, model_spec: ModelSpec) -> None:
        ...
        self.input_channels = 3 * SPATIAL_DIM  # current residual, previous residual, previous update
        self.output_channels = SPATIAL_DIM
        hidden_channels = int(model_spec.width)
        bottleneck_channels = 2 * hidden_channels
    ...
    def architecture_description(self) -> str:
        ...
        return (
            f"small U-Net Conv2d optimizer on 5x5 grid: "
            f"B x 9 x {GRID_ROWS} x {GRID_COLS} input "
            f"[current residual, previous residual, previous update] -> "
            f"encoder {self.model_spec.width} channels -> 2x2 bottleneck "
            f"{2 * self.model_spec.width} channels -> skip decoder -> "
            f"B x 3 x {GRID_ROWS} x {GRID_COLS} update; "
            f"no input normalization and no norm layers; "
            f"fixed vertices are hard-projected after output ({bias_text})"
        )

    @staticmethod
    def _full_state_to_grid_channels(x: torch.Tensor) -> torch.Tensor:
        if x.ndim != 2 or x.shape[-1] != FULL_STATE_DIM:
            raise ValueError(f"Expected shape (B, {FULL_STATE_DIM}), got {tuple(x.shape)}")
        return x.reshape(x.shape[0], GRID_ROWS, GRID_COLS, SPATIAL_DIM).permute(0, 3, 1, 2).contiguous()

    @staticmethod
    def _grid_channels_to_full_state(x: torch.Tensor) -> torch.Tensor:
        if x.ndim != 4 or x.shape[1:] != (SPATIAL_DIM, GRID_ROWS, GRID_COLS):
            raise ValueError(f"Expected shape (B, {SPATIAL_DIM}, {GRID_ROWS}, {GRID_COLS}), got {tuple(x.shape)}")
        return x.permute(0, 2, 3, 1).contiguous().reshape(x.shape[0], FULL_STATE_DIM)
    ...
    def forward(
        ...
        *,
        ...
        x = torch.cat(
            [
                self._full_state_to_grid_channels(current_residual),
                self._full_state_to_grid_channels(previous_residual),
                self._full_state_to_grid_channels(previous_update),
            ],
            dim=1,
        )
        skip = self.activation(self.enc1_conv1(x))
        skip = self.activation(self.enc1_conv2(skip))
        h = self.downsample(skip)
        h = self.activation(self.bottleneck_conv1(h))
        h = self.activation(self.bottleneck_conv2(h))
        ...
        h = self.activation(self.dec_conv1(h))
        h = self.activation(self.dec_conv2(h))
        raw_delta_grid = self.output_conv(h)
        raw_delta = self.residual_length_scale * self._grid_channels_to_full_state(raw_delta_grid)
    ...
    *,
    ...
    report = {
        ...
        "model": {
            ...
            "hidden_initialization": "orthogonal with activation-specific gain",
            ...
            "fixed_point_policy": "Metamizer-style hard projection after learned output; residual/history fixed entries are zero.",
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "5x5 triangular-cloth full-75D fixed-projection ablation; "
            f"history_input={USE_HISTORY_INPUT}, all models use_bias={USE_BIAS}"
        )
    )
...
def main() -> None:
    ...
    save_json({
        ...
        "model_input_dimension": MODEL_INPUT_DIM,
        "use_history_input": USE_HISTORY_INPUT,
        "fixed_point_policy": "Metamizer-style hard projection after learned output; no fixed one-hot input.",
        "grid_rows": GRID_ROWS,
        "grid_cols": GRID_COLS,
    ...
    summary = {
        "experiment_type": "fixed_left_edge_5x5_cloth_full75_metamizer_projection_input_ablation",
        "use_bias": USE_BIAS,
        "runtime_config": asdict(config),
        "physical_config": asdict(physical),
        "motion_split": asdict(motion_split),
