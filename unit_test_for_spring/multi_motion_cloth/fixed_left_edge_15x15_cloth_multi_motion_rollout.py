...
by ``fixed_left_edge_15x15_cloth_multi_motion_train_compare.py``. Starting from the
...
import fixed_left_edge_15x15_cloth_multi_motion_train_compare as core
...
def parse_args() -> argparse.Namespace:
    ...
    default_training_output = (
        Path(__file__).resolve().parent / "fixed_left_edge_15x15_cloth_multi_motion_train_compare"
    )
    parser.add_argument(
        "--training-output-dir",
        type=Path,
        default=default_training_output,
        help="Output directory created by fixed_left_edge_15x15_cloth_multi_motion_train_compare.py",
    )
    parser.add_argument(
        "--checkpoint",
        type=Path,
...
def solve_reference_frame(
    *,
    ...
    """Advance the reference trajectory without stopping on tolerance misses.

    The core reference solver returns its best finite iterate when strict
    convergence is missed. An additional emergency fallback is kept here so an
    unexpected numerical exception still does not terminate the requested
    rollout: a backtracked mass-preconditioned residual step is attempted, and
    the current finite state is used as the final fallback.
    """
    ...
    try:
    ...
    except Exception as exc:  # rollout must continue if a finite fallback exists
        emergency_message = f"{type(exc).__name__}: {exc}"
        y0 = initial_y.reshape(1, core.FREE_STATE_DIM)
        q_batch = q.reshape(1, core.FREE_STATE_DIM)
        masses_batch = free_masses.reshape(1, core.NUM_FREE_PARTICLES)
        gradient = core.stationarity_residual(y0, q_batch, masses_batch, physical)
        mass_per_coordinate = masses_batch.repeat_interleave(core.SPATIAL_DIM, dim=-1)
        direction = -physical.dt**2 * gradient / mass_per_coordinate
        energy0 = float(core.variational_energy(y0, q_batch, masses_batch, physical).item())
        ...
        while alpha >= 2.0**-30:
            ...
            if bool(torch.isfinite(candidate).all()) and bool(
                torch.all(core.spring_lengths_from_free(candidate, physical) > core.DISTANCE_EPS)
            ):
                candidate_energy = float(
                    core.variational_energy(candidate, q_batch, masses_batch, physical).item()
                )
        ...
        fallback_residual = float(
            core.stationarity_residual_norm(
        ...
        info = {
            ...
            "converged": False,
            "acceptable": fallback_residual <= core.REFERENCE_ACCEPTABLE_RESIDUAL,
...
def render_video(
    *,
    ...
    def update(video_index: int) -> list[Any]:
        ...
        fig.suptitle(
            "15x15 triangular cloth multi-motion model: exactly 50 inner iterations per frame",
            fontsize=15,
        )
        return returned
    ...
    mp4_path = output_dir / "fixed_left_edge_15x15_cloth_multi_motion_500_frames.mp4"
    gif_path = output_dir / "fixed_left_edge_15x15_cloth_multi_motion_500_frames.gif"
...
def diagnostics_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    if records and "iterations" in records[0]:
        ...
        used_best = np.asarray(
            [bool(r.get("used_best_finite_iterate", False)) for r in records], dtype=bool
        )
        result.update(
