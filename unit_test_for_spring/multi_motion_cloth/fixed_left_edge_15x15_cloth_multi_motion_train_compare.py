...
--------------------
1. Keep the 15x15 triangular spring mesh with the left-top and left-bottom
   corner vertices fixed. The 223 free vertices form a 669-dimensional state.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="15x15 triangular-cloth multi-motion learned optimizer")
...
def validate_args(args: argparse.Namespace) -> RuntimeConfig:
    positive_ints = {
        "total_time_steps": args.total_time_steps, "train_points_per_problem": args.train_points_per_problem,
        "eval_points_per_problem": args.eval_points_per_problem, "epochs": args.epochs,
        "validation_interval": args.validation_interval, "diagnostic_interval": args.diagnostic_interval,
        "evaluation_steps": args.evaluation_steps, "evaluation_batch_size": args.evaluation_batch_size,
        "initial_k": args.initial_k, "k_increase_interval": args.k_increase_interval,
        "k_increase_amount": args.k_increase_amount, "max_k": args.max_k,
    }
    ...
    return RuntimeConfig(
        ...
        epochs=int(args.epochs), validation_interval=int(args.validation_interval),
        diagnostic_interval=int(args.diagnostic_interval), evaluation_steps=int(args.evaluation_steps),
        evaluation_batch_size=int(args.evaluation_batch_size), initial_k=int(args.initial_k),
        k_increase_interval=int(args.k_increase_interval), k_increase_amount=int(args.k_increase_amount),
        max_k=int(args.max_k), report_steps=report_steps,
        ...
        device=str(args.device), run_single_motion_baseline=not bool(args.skip_single_motion_baseline),
        skip_plots=bool(args.skip_plots), save_datasets=bool(args.save_datasets),
    )
...
def main() -> None:
    ...
    multi_training = build_dataset_for_motion_times(
        lookup=lookup, motion_indices=motion_split.train_motion_indices, time_indices=TRAIN_TIME_INDICES,
        points_per_problem=config.train_points_per_problem, base_seed=TRAIN_SOBOL_SEED,
        role="multi_motion_training", physical=physical, include_explicit_train_points=True,
    )
    ...
    single_training = build_dataset_for_motion_times(
        lookup=lookup, motion_indices=(0,), time_indices=TRAIN_TIME_INDICES,
        points_per_problem=single_points_per_problem, base_seed=TRAIN_SOBOL_SEED + 1_000_000,
        role="single_motion_equal_budget_training", physical=physical, include_explicit_train_points=True,
    )
    ...
    validation = build_dataset_for_motion_times(
        lookup=lookup, motion_indices=motion_split.validation_motion_indices,
        time_indices=VALIDATION_TIME_INDICES, points_per_problem=config.eval_points_per_problem,
        base_seed=VALIDATION_SOBOL_SEED, role="unseen_motion_validation", physical=physical,
        include_explicit_train_points=False,
    )
    seen_interp = build_dataset_for_motion_times(
        lookup=lookup, motion_indices=motion_split.train_motion_indices,
        time_indices=SEEN_INTERPOLATION_TIME_INDICES, points_per_problem=config.eval_points_per_problem,
        base_seed=SEEN_INTERPOLATION_TEST_SOBOL_SEED, role="seen_motion_temporal_interpolation",
        physical=physical, include_explicit_train_points=False,
    )
    seen_extrap = build_dataset_for_motion_times(
        lookup=lookup, motion_indices=motion_split.train_motion_indices,
        time_indices=SEEN_EXTRAPOLATION_TIME_INDICES, points_per_problem=config.eval_points_per_problem,
        base_seed=SEEN_EXTRAPOLATION_TEST_SOBOL_SEED, role="seen_motion_temporal_extrapolation",
        physical=physical, include_explicit_train_points=False,
    )
    unseen_id = build_dataset_for_motion_times(
        lookup=lookup, motion_indices=motion_split.id_test_motion_indices,
        time_indices=UNSEEN_TEST_TIME_INDICES, points_per_problem=config.eval_points_per_problem,
        base_seed=UNSEEN_ID_TEST_SOBOL_SEED, role="unseen_id_test", physical=physical,
        include_explicit_train_points=False,
    )
    ood_test = build_dataset_for_motion_times(
        lookup=lookup, motion_indices=motion_split.ood_test_motion_indices,
        time_indices=UNSEEN_TEST_TIME_INDICES, points_per_problem=config.eval_points_per_problem,
        base_seed=OOD_TEST_SOBOL_SEED, role="ood_test", physical=physical,
        include_explicit_train_points=False,
    )
    current_seen = build_special_state_dataset(
        lookup=lookup, motion_indices=motion_split.train_motion_indices,
        time_indices=SEEN_INTERPOLATION_TIME_INDICES, state="current", role="current_state_seen_motion",
    )
    current_id = build_special_state_dataset(
        lookup=lookup, motion_indices=motion_split.id_test_motion_indices,
        time_indices=UNSEEN_TEST_TIME_INDICES, state="current", role="current_state_unseen_id",
    )
    current_ood = build_special_state_dataset(
        lookup=lookup, motion_indices=motion_split.ood_test_motion_indices,
        time_indices=UNSEEN_TEST_TIME_INDICES, state="current", role="current_state_ood",
    )
    evaluation_datasets = {
    ...
    if not config.skip_plots:
        plot_gradient_descent_step_size_selection(gd_selection, output_dir / "gradient_descent_step_size_selection.png")
    ...
    for name, dataset in evaluation_datasets.items():
        ...
        shared_baselines[name] = {
            "gradient_descent": evaluate_solver_on_dataset(
                solver="gradient_descent", dataset_cpu=dataset, physical=physical,
                steps=config.evaluation_steps, batch_size=config.evaluation_batch_size,
                report_steps=config.report_steps, device=device, gd_step_size=gd_step_size,
            ),
            "full_newton": evaluate_solver_on_dataset(
                solver="full_newton", dataset_cpu=dataset, physical=physical,
                steps=config.evaluation_steps, batch_size=config.evaluation_batch_size,
                report_steps=config.report_steps, device=device,
            ),
        }
    ...
    reports = [run_experiment(
        experiment_name="multi_motion", training_cpu=multi_training, validation_cpu=validation,
        evaluation_datasets=evaluation_datasets, output_dir=output_dir, config=config,
        physical=physical, gd_step_size=gd_step_size, shared_baselines=shared_baselines,
    )]
    if config.run_single_motion_baseline:
        reports.append(run_experiment(
            experiment_name="single_motion_equal_budget_baseline", training_cpu=single_training,
            validation_cpu=validation, evaluation_datasets=evaluation_datasets,
            output_dir=output_dir, config=config, physical=physical,
            gd_step_size=gd_step_size, shared_baselines=shared_baselines,
        ))

    summary = {
        "experiment_type": "fixed_left_edge_15x15_cloth_multi_motion_generalization",
        "runtime_config": asdict(config), "physical_config": asdict(physical),
        "motion_split": asdict(motion_split), "motions": [asdict(m) for m in motions],
        "physics_checks": physics_checks, "gradient_descent_selection": gd_selection,
        "hard_case_selection": hard_case, "shared_baselines": shared_baselines,
        "experiments": reports,
        "metric_policy": {
    ...
    print("500-frame rollout input: " + str(output_dir / "multi_motion" / "best_validation_model_state_dict.pt"))
