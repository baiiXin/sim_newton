...
One-script fixed-point handling消融：同一批 full 25 x 3 = 75D dataset、同一套
GD/Newton baseline、同一训练/验证/测试流程，比较四种 learned-optimizer 处理策略：

1. A_default_init:
   - reduced 69D physics state update; network still receives 3 x 75D history input.
   - fixed-vertex output is discarded before applying the update.
   - residual input includes boundary spring force on fixed vertices.
   - nn.Linear keeps PyTorch default initialization.
2. A_zero_output_init:
   - same as A_default_init, but hidden layers use orthogonal initialization and output layer is zero-initialized.
3. B_zero_fixed_residual:
   - full 75D learned rollout state with Metamizer-style hard projection after every learned update.
   - residual input is free-only residual embedded into 75D, so fixed residual entries are zero.
   - hidden orthogonal initialization and zero output layer.
4. B_boundary_force_residual:
   - same full 75D hard-projection rollout as B_zero_fixed_residual.
   - residual input includes boundary spring force on fixed vertices.
   - hidden orthogonal initialization and zero output layer.

The network never receives a fixed-point one-hot channel and does not use velocities or accelerations.
Network input is always 3 x 75D = 225D [current residual, previous residual, previous update].
Network output is always 75D. Fixed vertices are either discarded before applying the update
(reduced_discard) or gated to zero and hard-projected after applying the update (full_project).

Default experiment grid: the four ablation presets x activation in {identity, ReLU, Tanh},
depth=1, width=256, all linear layers are bias-free, default device cuda:0.
"""
...
UPDATE_POLICIES = ("reduced_discard", "full_project")
RESIDUAL_POLICIES = ("boundary_force", "free_only")
INITIALIZATION_POLICIES = ("pytorch_default", "orthogonal_zero_output")
ABLATION_PRESETS: tuple[tuple[str, str, str, str], ...] = (
    (
        "A_default_init",
        "reduced_discard",
        "boundary_force",
    ...
    (
        ...
        "reduced_discard",
        "boundary_force",
        "orthogonal_zero_output",
    ),
    (
        "B_zero_fixed_residual",
        "full_project",
        "free_only",
        "orthogonal_zero_output",
    ),
    (
        "B_boundary_force_residual",
        "full_project",
        "boundary_force",
        "orthogonal_zero_output",
    ),
...
class ModelSpec:
    ...
    def experiment_name(self) -> str:
        ...
        return (
            f"{self.ablation_name}_activation_{self.activation}_depth_{self.depth:02d}_"
...
def build_model_specs(config: RuntimeConfig) -> list[ModelSpec]:
    ...
    for ablation_name, update_policy, residual_policy, initialization_policy in ABLATION_PRESETS:
        ...
        if initialization_policy not in INITIALIZATION_POLICIES:
            raise ValueError(f"Unsupported initialization_policy={initialization_policy}")
...
def full_stationarity_residual(
    ...
    """Return the 75D stationarity residual before eliminating fixed vertices.

    The inertial part is defined only for the 23 free vertices. Spring forces are
    accumulated on all 25 vertices, so fixed vertices carry boundary-reaction
    force information. The free slice matches stationarity_residual(...).
    """
    free = reshape_free(y)
    q_free = reshape_free(q)
    full = full_positions_from_free(y, physical)
    ...
    full_grad.index_add_(-2, edges[:, 0], -edge_grad)
    full_grad.index_add_(-2, edges[:, 1], edge_grad)
    return full_grad.reshape(*y.shape[:-1], FULL_STATE_DIM)
...
def mass_preconditioned_free_only_residual_full(
    y_full: torch.Tensor,
    q_full: torch.Tensor,
    masses: torch.Tensor,
    physical: PhysicalConfig,
) -> torch.Tensor:
    """Free 69D residual embedded into 75D; fixed entries are exactly zero."""
...
def mass_preconditioned_boundary_force_residual_full(
    y_full: torch.Tensor,
    q_full: torch.Tensor,
    masses: torch.Tensor,
    physical: PhysicalConfig,
) -> torch.Tensor:
    """Full 75D residual; fixed entries contain spring boundary-reaction force."""
    y_free = free_state_from_full_state(project_fixed_vertices(y_full, physical))
    q_free = free_state_from_full_state(q_full)
    residual_full = full_stationarity_residual(y_free, q_free, masses, physical)
    full_masses = torch.as_tensor(
        physical.masses, dtype=y_full.dtype, device=y_full.device
    ).reshape(*([1] * (residual_full.ndim - 1)), NUM_PARTICLES)
    full_masses = full_masses.expand(*residual_full.shape[:-1], NUM_PARTICLES)
    mass_per_coordinate = full_masses.repeat_interleave(SPATIAL_DIM, dim=-1)
    return physical.dt**2 * residual_full / mass_per_coordinate
...
def mass_preconditioned_residual_full(
    ...
    *,
    ...
    if residual_policy == "free_only":
        return mass_preconditioned_free_only_residual_full(y_full, q_full, masses, physical)
    if residual_policy == "boundary_force":
        return mass_preconditioned_boundary_force_residual_full(y_full, q_full, masses, physical)
    raise ValueError(f"Unsupported residual_policy={residual_policy}")
...
class MLPOptimizer(nn.Module):
    def __init__(self, residual_length_scale: float, model_spec: ModelSpec) -> None:
        ...
        if model_spec.update_policy not in UPDATE_POLICIES:
            raise ValueError(f"Unsupported update_policy: {model_spec.update_policy}")
        if model_spec.residual_policy not in RESIDUAL_POLICIES:
            raise ValueError(f"Unsupported residual_policy: {model_spec.residual_policy}")
        if model_spec.initialization_policy not in INITIALIZATION_POLICIES:
            raise ValueError(
                f"Unsupported initialization_policy: {model_spec.initialization_policy}"
            )
        ...
        if model_spec.initialization_policy == "orthogonal_zero_output":
        ...
        elif model_spec.initialization_policy == "pytorch_default":
            # Keep the initialization already performed by nn.Linear.reset_parameters.
            pass
        else:
            raise ValueError(model_spec.initialization_policy)
    ...
    def architecture_description(self) -> str:
        ...
        history_text = (
            "current/full residual + previous/full residual + previous/full update"
            if USE_HISTORY_INPUT else "current/full residual only"
        )
        if self.model_spec.update_policy == "reduced_discard":
            fixed_policy_text = "75D raw update; fixed entries discarded, free 69D update applied"
        else:
            fixed_policy_text = "75D raw update; fixed entries gated to zero, then hard-projected"
        return (
            f"{MODEL_INPUT_DIM}D input ({history_text}; residual_policy="
            f"{self.model_spec.residual_policy}) -> "
            f"[{self.model_spec.width}, {self.model_spec.activation}] x "
            f"{self.model_spec.depth} -> {fixed_policy_text} "
            f"({bias_text}; init={self.model_spec.initialization_policy})"
        )
    ...
    def forward(
        ...
        *,
        ...
        current_residual = mass_preconditioned_residual_full(
            y,
            q,
            masses,
            physical,
            residual_policy=self.model_spec.residual_policy,
        )
        ...
        raw_delta_full = self.residual_length_scale * self.output_layer(h)
        return raw_delta_full, current_residual
...
def apply_model_update(
    ...
    if model.model_spec.update_policy == "reduced_discard":
        y_free = free_state_from_full_state(project_fixed_vertices(y, physical))
        delta_free = free_state_from_full_state(raw_delta_full)
        y_next_free = y_free + delta_free
        delta_applied_full = full_vector_from_free_vector(delta_free)
        y_next = project_fixed_vertices(full_state_from_free_state(y_next_free, physical), physical)
    elif model.model_spec.update_policy == "full_project":
        delta_applied_full = raw_delta_full * free_update_gate_like(y)
        y_next = project_fixed_vertices(y + delta_applied_full, physical)
    else:
        raise ValueError(f"Unsupported update_policy={model.model_spec.update_policy}")
    return y_next, delta_applied_full, current_residual
...
def _state_metrics(
    ...
    y_projected = project_fixed_vertices(y, physical)
    point_errors = torch.linalg.vector_norm(
        reshape_full(y_projected) - reshape_full(dataset.exact_y), dim=-1
    )
    energy = variational_energy_full(y_projected, dataset.q, dataset.masses, physical)
    fixed_positions = torch.as_tensor(
        physical.fixed_positions,
        dtype=y.dtype,
        device=y.device,
    ...
    fixed_errors = torch.linalg.vector_norm(
        reshape_full(y_projected)[..., list(FIXED_VERTEX_INDICES), :] - fixed_positions,
        dim=-1,
    )
    return {
        "residual": stationarity_residual_norm_full(y_projected, dataset.q, dataset.masses, physical),
        "energy_gap": energy - exact_energy,
        "exact_error": torch.linalg.vector_norm(y_projected - dataset.exact_y, dim=-1),
        ...
        "spring_length_error": torch.mean(
            torch.abs(
                spring_lengths_from_full(y_projected, physical)
                - spring_lengths_from_full(dataset.exact_y, physical)
            ),
            dim=-1,
        ),
...
def evaluate_solver_on_dataset(
    *,
    ...
    for start in range(0, len(dataset_cpu), batch_size):
        ...
        previous_residual = torch.zeros_like(y) if USE_HISTORY_INPUT else None
        previous_update = torch.zeros_like(y) if USE_HISTORY_INPUT else None
        ...
        for step in range(steps + 1):
            ...
            if solver == "learned":
                ...
                y, _ = apply_gradient_descent_update_full(y, batch.q, batch.masses, physical, gd_step_size)
            else:
...
def one_step_diagnostics(model: MLPOptimizer, dataset: DatasetBundle, physical: PhysicalConfig) -> dict[str, float]:
    with torch.no_grad():
        y0 = project_fixed_vertices(dataset.initial_y, physical)
        previous_residual = torch.zeros_like(y0) if USE_HISTORY_INPUT else None
        previous_update = torch.zeros_like(y0) if USE_HISTORY_INPUT else None
        ...
        residual0 = stationarity_residual_norm_full(y0, dataset.q, dataset.masses, physical)
        residual1 = stationarity_residual_norm_full(y1, dataset.q, dataset.masses, physical)
        ideal = dataset.exact_y - y0
        cosine = torch.nn.functional.cosine_similarity(delta, ideal, dim=-1, eps=1e-30)
        fixed_error = _state_metrics(y1, dataset, variational_energy_full(dataset.exact_y, dataset.q, dataset.masses, physical), physical)["fixed_vertex_max_error"]
        return {
            ...
            "fixed_vertex_max_error_after": float(fixed_error.max().item()),
...
def run_experiment(
    *,
    ...
    initial_energy = variational_energy_full(
    ...
    for epoch_index in range(start_epoch, config.epochs):
        ...
        model.train()
        y = project_fixed_vertices(training.initial_y, physical)
        previous_residual = torch.zeros_like(y) if USE_HISTORY_INPUT else None
        previous_update = torch.zeros_like(y) if USE_HISTORY_INPUT else None
        ...
        for _ in range(k):
            ...
            if USE_HISTORY_INPUT:
                previous_residual = current_residual.detach()
                previous_update = delta.detach()
            energy = variational_energy_full(y, training.q, training.masses, physical)
    ...
    report = {
        ...
        "model": {
            ...
            "ablation_name": model_spec.ablation_name,
            "update_policy": model_spec.update_policy,
            "residual_policy": model_spec.residual_policy,
            "initialization_policy": model_spec.initialization_policy,
            "hidden_initialization": (
                "orthogonal with activation-specific gain"
                if model_spec.initialization_policy == "orthogonal_zero_output"
                else "PyTorch nn.Linear default reset_parameters"
            ),
            "output_layer_initialization": (
                "zero weights and zero bias when present"
                if model_spec.initialization_policy == "orthogonal_zero_output"
                else "PyTorch nn.Linear default reset_parameters"
            ),
            ...
            "use_history_input": USE_HISTORY_INPUT,
            "fixed_point_policy": (
                "fixed entries discarded before applying free 69D update"
                if model_spec.update_policy == "reduced_discard"
                else "fixed entries gated to zero and hard-projected after learned output"
            ),
...
def select_hard_ood_case(
    ...
    residual = stationarity_residual_norm_full(
    ...
    for problem_index in sorted(torch.unique(ood_dataset.problem_index).tolist()):
        ...
        record = {
            ...
            "sample_initial_y_full75": ood_dataset.initial_y[sample_index].tolist(),
            "sample_initial_y_free69": free_state_from_full_state(ood_dataset.initial_y[sample_index]).tolist(),
        }
...
def run_physics_checks(physical: PhysicalConfig, motion: MotionSpec) -> dict[str, Any]:
    ...
    y_free = free_state_from_full(p).reshape(1, -1)
    y_full = project_fixed_vertices(full_state_from_positions(p).reshape(1, -1), physical)
    reconstructed = full_positions_from_free(y_free, physical).reshape(NUM_PARTICLES, SPATIAL_DIM)
    fixed_error = float(torch.max(torch.abs(reshape_full(y_full)[0, list(FIXED_VERTEX_INDICES)] - torch.tensor(physical.fixed_positions, dtype=TORCH_DTYPE))).item())
    lengths = spring_lengths_from_free(y_free, physical).squeeze(0)
    projected_delta = torch.ones(1, FULL_STATE_DIM, dtype=TORCH_DTYPE) * free_update_gate_like(y_full)
    return {
        ...
        "fixed_update_gate_sum": float(reshape_full(projected_delta)[0, list(FIXED_VERTEX_INDICES), :].sum().item()),
    }
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "5x5 triangular-cloth fixed-point handling ablation; "
            f"history_input={USE_HISTORY_INPUT}, all models use_bias={USE_BIAS}, device=cuda:0 by default"
        )
    )
    ...
    with save_path.open("w", newline="", encoding="utf-8") as file:
        ...
        for summary in summaries:
            ...
            writer.writerow({
                ...
                "ablation_name": spec.get("ablation_name"),
                "update_policy": spec.get("update_policy"),
                "residual_policy": spec.get("residual_policy"),
                "initialization_policy": spec.get("initialization_policy"),
...
def main() -> None:
    ...
    print(f"Device: {device}; use_bias={USE_BIAS}; ablation_presets={len(ABLATION_PRESETS)}")
    ...
    save_json({
        ...
        "model_input_dimension": MODEL_INPUT_DIM,
        "use_history_input": USE_HISTORY_INPUT,
        "fixed_point_policy": "ablation over reduced_discard vs full_project; no fixed one-hot input.",
        "ablation_presets": [
            {
                "ablation_name": name,
                "update_policy": update_policy,
            ...
            for name, update_policy, residual_policy, initialization_policy in ABLATION_PRESETS
        ],
        "grid_rows": GRID_ROWS,
        "grid_cols": GRID_COLS,
    ...
    summary = {
        "experiment_type": "fixed_left_edge_5x5_cloth_fixed_point_projection_ablation",
        "use_bias": USE_BIAS,
        "runtime_config": asdict(config),
        "physical_config": asdict(physical),
        "motion_split": asdict(motion_split),
