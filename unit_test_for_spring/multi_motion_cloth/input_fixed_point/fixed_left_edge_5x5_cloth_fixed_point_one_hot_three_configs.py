"""
Fixed-left-edge 5x5 triangular-cloth learned-optimizer experiment.

This script evaluates exactly three bias-free model configurations:
    1. identity activation, hidden width 64, hidden depth 2,
    2. ReLU activation, hidden width 256, hidden depth 2,
    3. Tanh activation, hidden width 128, hidden depth 2.
The default device is cuda:0.

The learned optimizer uses a Metamizer-inspired one-step history input.  At
iteration k, the MLP receives the original three 69D history channels plus a
25D fixed-vertex binary one-hot/mask, for a total of 232 input dimensions:
    ...
    4. one binary entry per mesh vertex (1=fixed, 0=free).
...
MODEL_INPUT_DIM = HISTORY_INPUT_CHANNELS * FREE_STATE_DIM + FIXED_VERTEX_INDICATOR_DIM
...
REQUESTED_MODEL_CONFIGURATIONS = (
    ("identity", 2, 64),
    ("relu", 2, 256),
    ("tanh", 2, 128),
)
HIDDEN_DEPTHS = tuple(sorted({depth for _, depth, _ in REQUESTED_MODEL_CONFIGURATIONS}))
HIDDEN_WIDTHS = tuple(sorted({width for _, _, width in REQUESTED_MODEL_CONFIGURATIONS}))
USE_BIAS = False
OPTIMIZER_NAME = "adam"
LEARNING_RATE = 1e-3
...
def build_model_specs(config: RuntimeConfig) -> list[ModelSpec]:
    # Keep only the three explicitly requested activation/width/depth pairings.
    # The command-line activation/depth/width arguments may filter this list,
    # but they never create additional Cartesian-product configurations.
    specs = [
        ModelSpec(activation=activation, depth=depth, width=width, use_bias=USE_BIAS)
        for activation, depth, width in REQUESTED_MODEL_CONFIGURATIONS
        if activation in config.activations
        and depth in config.depths
        and width in config.widths
    ]
    if not specs:
        raise ValueError(
            "The supplied activation/depth/width filters removed all three requested configurations"
        )
...
class MLPOptimizer(nn.Module):
    ...
    def architecture_description(self) -> str:
        ...
        return (
            ...
            f"{FIXED_VERTEX_INDICATOR_DIM}D fixed-vertex binary indicator, "
    ...
    def forward(
        ...
        *,
        ...
        indicator_view_shape = (1,) * (y.ndim - 1) + (FIXED_VERTEX_INDICATOR_DIM,)
        fixed_vertex_indicator = self.fixed_vertex_indicator.view(
            indicator_view_shape
        ).expand(*y.shape[:-1], FIXED_VERTEX_INDICATOR_DIM)
        h = torch.cat(
            [
    ...
    *,
    ...
    report = {
        ...
        "model": {
            ...
            "history_input_channel_dimension": FREE_STATE_DIM,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "5x5 triangular-cloth fixed-point-one-hot experiment with exactly "
            f"three requested model configurations; all models use_bias={USE_BIAS}"
        )
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Run one zero-based index from the filtered three-configuration list.",
    )
...
def save_all_experiments_markdown_report(
    ...
    *,
    ...
    lines = [
        ...
        "- Learned-optimizer input: current residual, previous residual, previous displacement (`3 x 69`) and a 25D fixed-vertex binary one-hot/mask, flattened to `232D`.",
...
def main() -> None:
    ...
    summary = {
        "experiment_type": "fixed_left_edge_5x5_cloth_history_input_fixed_point_one_hot_three_config_comparison",
        "use_bias": USE_BIAS,
        "runtime_config": asdict(config),
        "physical_config": asdict(physical),
        "motion_split": asdict(motion_split),
