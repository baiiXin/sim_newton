...
This script evaluates the full Cartesian product of:
    activations = {identity, ReLU, Tanh},
    hidden widths = {64, 128, 256},
    hidden depths = {1, 2, 5, 10}.
All linear layers are bias-free.  The default device is cuda:1.

The learned optimizer uses a per-vertex Metamizer-inspired one-step history
input.  For each of the 25 mesh vertices, the MLP receives 10 features:
    1. current mass-preconditioned residual (3D),
    2. previous mass-preconditioned residual (3D),
    3. previous gated optimizer displacement (3D),
    4. fixed-vertex binary indicator (1D; 1=fixed, 0=free).
Thus the flattened model input is 25 x 10 = 250D.  The network predicts a
full 25 x 3 = 75D displacement.  A hard gate derived from the fixed-vertex
indicator zeros the two fixed-vertex updates before the 23 free-vertex updates
are extracted and applied to the reduced 69D physical state.  History stores
the full 75D residual and gated displacement and is detached between optimizer
iterations, while the reduced state remains differentiable through the full
K-step rollout.
...
FIXED_VERTEX_INDICATOR_DIM = NUM_PARTICLES
PER_VERTEX_INPUT_DIM = HISTORY_INPUT_CHANNELS * SPATIAL_DIM + 1
MODEL_INPUT_DIM = NUM_PARTICLES * PER_VERTEX_INPUT_DIM
...
class ModelSpec:
    ...
    def experiment_name(self) -> str:
        ...
        return (
            f"history_input_fixed_point_one_hot_default_init_activation_{self.activation}_"
            f"depth_{self.depth:02d}_width_{self.width:03d}_{bias_name}"
        )
...
def build_model_specs(config: RuntimeConfig) -> list[ModelSpec]:
    """Build the requested activation x depth x width Cartesian product."""
    specs = [
...
def full_vector_from_free(free_vector: torch.Tensor) -> torch.Tensor:
    """Embed a reduced 69D free-vertex vector into a full 75D vertex vector.

    Fixed-vertex entries are exactly zero.  The operation remains differentiable
    with respect to all free-vertex entries.
    """
    if free_vector.shape[-1] != FREE_STATE_DIM:
        raise ValueError(
            f"Expected final free-vector dimension {FREE_STATE_DIM}, "
            f"got {tuple(free_vector.shape)}"
        )
    free_points = free_vector.reshape(
        *free_vector.shape[:-1], NUM_FREE_PARTICLES, SPATIAL_DIM
    )
    full_points = free_vector.new_zeros(
        *free_vector.shape[:-1], NUM_PARTICLES, SPATIAL_DIM
    )
    full_points[..., list(FREE_VERTEX_INDICES), :] = free_points
    return full_points.reshape(*free_vector.shape[:-1], FULL_STATE_DIM)
...
class LearnedOptimizerState:
    """Full-mesh one-step history carried within one independent solve."""
    ...
    def zeros_like(cls, y: torch.Tensor) -> "LearnedOptimizerState":
        if y.shape[-1] != FREE_STATE_DIM:
            raise ValueError(
                f"Expected reduced state dimension {FREE_STATE_DIM}, got {tuple(y.shape)}"
            )
        zeros = y.new_zeros(*y.shape[:-1], FULL_STATE_DIM)
...
class MLPOptimizer(nn.Module):
    def __init__(self, residual_length_scale: float, model_spec: ModelSpec) -> None:
        ...
        fixed_vertex_indicator = torch.zeros(NUM_PARTICLES, dtype=TORCH_DTYPE)
        fixed_vertex_indicator[list(FIXED_VERTEX_INDICES)] = 1.0
        self.register_buffer("fixed_vertex_indicator", fixed_vertex_indicator)
        self.register_buffer(
            "free_vertex_update_gate",
            (1.0 - fixed_vertex_indicator).reshape(NUM_PARTICLES, 1),
        )
    ...
    def architecture_description(self) -> str:
        ...
        return (
            f"{NUM_PARTICLES} vertices x {PER_VERTEX_INPUT_DIM} features "
            f"[current residual(3), previous residual(3), previous gated update(3), "
            f"fixed flag(1)] = {MODEL_INPUT_DIM}D -> "
            f"[{self.model_spec.width}, {self.model_spec.activation}] x "
            f"{self.model_spec.depth} -> {MODEL_OUTPUT_DIM}D full-mesh update -> "
            f"hard fixed-vertex gate -> {FREE_STATE_DIM}D reduced update ({bias_text})"
        )
    ...
    def forward(
        ...
        *,
        ...
        expected_history_shape = (*y.shape[:-1], FULL_STATE_DIM)
        if optimizer_state.previous_residual.shape != expected_history_shape:
            raise ValueError(
                "previous_residual shape mismatch: "
                f"expected {expected_history_shape}, got "
                f"{tuple(optimizer_state.previous_residual.shape)}"
            )
        if optimizer_state.previous_update.shape != expected_history_shape:
            raise ValueError(
        ...
        current_residual_free = mass_preconditioned_residual(
            y, q, masses, physical
        )
        current_residual_full = full_vector_from_free(current_residual_free)

        leading_shape = y.shape[:-1]
        current_points = current_residual_full.reshape(
            *leading_shape, NUM_PARTICLES, SPATIAL_DIM
        ) / self.residual_length_scale
        previous_residual_points = optimizer_state.previous_residual.reshape(
            *leading_shape, NUM_PARTICLES, SPATIAL_DIM
        ) / self.residual_length_scale
        previous_update_points = optimizer_state.previous_update.reshape(
            *leading_shape, NUM_PARTICLES, SPATIAL_DIM
        ) / self.residual_length_scale

        indicator_shape = (1,) * len(leading_shape) + (NUM_PARTICLES, 1)
        fixed_indicator = self.fixed_vertex_indicator.reshape(
            indicator_shape
        ).expand(*leading_shape, NUM_PARTICLES, 1)

        # Per-vertex ordering:
        # {res_xyz, last_res_xyz, last_delta_y_xyz, fixed_flag}.
        h = torch.cat(
            [
                current_points,
                previous_residual_points,
                previous_update_points,
                fixed_indicator,
            ],
            dim=-1,
        ).reshape(*leading_shape, MODEL_INPUT_DIM)

        for layer in self.hidden_layers:
            h = self.activation(layer(h))

        raw_delta_full = self.residual_length_scale * self.output_layer(h)
        raw_delta_points = raw_delta_full.reshape(
            *leading_shape, NUM_PARTICLES, SPATIAL_DIM
        )
        gate_shape = (1,) * len(leading_shape) + (NUM_PARTICLES, 1)
        gated_delta_points = raw_delta_points * self.free_vertex_update_gate.reshape(
            gate_shape
        )
        gated_delta_full = gated_delta_points.reshape(*leading_shape, FULL_STATE_DIM)
        delta_free = free_state_from_full(gated_delta_points)
        return delta_free, current_residual_full, gated_delta_full
...
def apply_model_update(
    ...
    delta_free, current_residual_full, gated_delta_full = model(
        y, q, masses, optimizer_state, physical=physical
    )
    next_state = LearnedOptimizerState(
        previous_residual=current_residual_full.detach(),
        previous_update=gated_delta_full.detach(),
    )
    ...
    report = {
        ...
        "model": {
            ...
            "input_channels": [
                ...
                "fixed_vertex_binary_one_hot",
            ],
            "history_input_channel_dimension": FULL_STATE_DIM,
            "fixed_vertex_indicator_dimension": FIXED_VERTEX_INDICATOR_DIM,
            "per_vertex_input_dimension": PER_VERTEX_INPUT_DIM,
            "full_model_output_dimension": MODEL_OUTPUT_DIM,
            "reduced_applied_update_dimension": FREE_STATE_DIM,
            "output_gate": "delta_i = (1 - fixed_flag_i) * raw_delta_i",
            "fixed_vertex_indicator_indices": list(FIXED_VERTEX_INDICES),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "5x5 triangular-cloth fixed-point-mask full Cartesian-product ablation; "
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Run one zero-based index from the filtered Cartesian-product configuration list.",
    )
...
def save_all_experiments_markdown_report(
    ...
    *,
    ...
    lines = [
        ...
        "- Learned-optimizer input: 25 per-vertex feature blocks `{res_xyz, last_res_xyz, last_delta_y_xyz, fixed_flag}`, flattened to `25 x 10 = 250D`.",
...
def main() -> None:
    ...
    save_json({
        ...
        "fixed_vertex_indicator": [
            1 if index in FIXED_VERTEX_INDICES else 0 for index in range(NUM_PARTICLES)
        ],
        "fixed_vertex_indicator_dimension": FIXED_VERTEX_INDICATOR_DIM,
        "per_vertex_input_dimension": PER_VERTEX_INPUT_DIM,
        "model_input_dimension": MODEL_INPUT_DIM,
        "model_output_dimension": MODEL_OUTPUT_DIM,
        "reduced_applied_update_dimension": FREE_STATE_DIM,
        "output_gate": "delta_i = (1 - fixed_flag_i) * raw_delta_i",
    ...
    summary = {
        "experiment_type": "fixed_left_edge_5x5_cloth_per_vertex_history_fixed_mask_full_cartesian_ablation",
        "use_bias": USE_BIAS,
        "runtime_config": asdict(config),
        "physical_config": asdict(physical),
        "motion_split": asdict(motion_split),
