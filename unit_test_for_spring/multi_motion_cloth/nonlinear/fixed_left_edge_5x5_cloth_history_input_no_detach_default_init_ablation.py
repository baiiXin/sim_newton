...
carried only within that problem's iterative solve.  Stored history tensors remain connected between iterations, so gradients flow
through the current state, previous residual, and previous optimizer displacement
across the complete K-step rollout.  No residual-threshold update guard is used.
...
class ModelSpec:
    ...
    def experiment_name(self) -> str:
        ...
        return (
            f"history_input_no_detach_default_init_activation_{self.activation}_depth_{self.depth:02d}_"
    ...
    *,
    ...
    report = {
        ...
        "model": {
            ...
            "history_gradient_policy": "full gradient flow through previous residual and previous update between iterations",
...
def main() -> None:
    ...
    summary = {
        "experiment_type": "fixed_left_edge_5x5_cloth_history_input_no_detach_default_init_activation_depth_width_ablation",
        "use_bias": USE_BIAS,
        "runtime_config": asdict(config),
        "physical_config": asdict(physical),
        "motion_split": asdict(motion_split),
