#!/usr/bin/env python3
"""Maximum-GPU-memory preflight for the 5x5 cloth history-input ablation.
...
Example
-------
python test_max_gpu_memory_cuda1.py

To select a different visible GPU:
python test_max_gpu_memory_cuda1.py --device cuda:0
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Test maximum CUDA memory for the 5x5 cloth ablation."
    )
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "max_gpu_memory_test_cuda1",
    )
...
def print_summary(records: Sequence[dict[str, Any]], device: str) -> None:
    print("\n" + "=" * 118)
    print(f"Maximum-memory preflight summary on {device}")
    print("=" * 118)
    header = (
