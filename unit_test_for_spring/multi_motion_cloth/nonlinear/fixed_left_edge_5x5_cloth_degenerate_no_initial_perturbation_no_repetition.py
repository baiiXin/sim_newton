...
This script evaluates activation in {identity, ReLU, Tanh}, hidden depth in
{1, 2, 5, 10}, and hidden width in {69, 128, 256}.  In this variant,
all linear layers are bias-free.  The default device is cuda:0.

The learned optimizer uses a Metamizer-inspired one-step history input.  At
iteration k, the MLP receives three 69D channels concatenated into 207D:
    1. current mass-preconditioned residual u_k / s,
    2. previous mass-preconditioned residual u_{k-1} / s,
    3. previous optimizer displacement delta_y_{k-1} / s.
History is zero-initialized for every independent optimization problem and is
carried only within that problem's iterative solve.  Stored history tensors are
detached between iterations, while the state y remains differentiable through
the complete K-step rollout.  No residual-threshold update guard is used.

Training uses one row per motion/time-step problem (256 training rows total),
float64, Adam(lr=1e-3), 5000 epochs, K={1, 3, 5, 10, 30} for 1000 epochs each,
validation every 200 epochs, and gradient clipping with maximum norm 10.  The
loss is averaged over K.

This is the degenerate-dataset variant: no initial-state perturbation is used.
For every motion/time-step problem, the optimizer initial state is exactly the
current physical state p_n (with fixed vertices eliminated).  Each problem is
stored exactly once; no duplicated rows are used in training, validation, or
testing.  Reference solutions are used only as targets and for evaluation,
never as dataset initial states.
...
MODEL_INPUT_DIM = HISTORY_INPUT_CHANNELS * FREE_STATE_DIM
DATASET_INITIAL_STATE_MODE = "current_state_single_no_perturbation"
INITIAL_STATE_PERTURBATION_ENABLED = False
...
class ModelSpec:
    ...
    def experiment_name(self) -> str:
        ...
        return (
            f"degenerate_no_initial_perturbation_history_input_default_init_"
...
def generate_unperturbed_point(
    *,
    problem: TimeStepProblem,
    physical: PhysicalConfig,
) -> tuple[torch.Tensor, dict[str, Any]]:
    """Return the single unperturbed current state for one physical problem."""
    current_state = (
        free_state_from_full(problem.p_n_full)
        .detach()
        .cpu()
        .to(TORCH_DTYPE)
        .reshape(1, FREE_STATE_DIM)
        .contiguous()
    )
    if not bool(nondegenerate_mask(current_state, physical)[0]):
        raise ValueError("The unperturbed current state is degenerate")
    max_abs_distance_to_exact = float(
        torch.max(torch.abs(current_state.squeeze(0) - problem.exact_y_free)).item()
    )
    return current_state, {
        "mode": DATASET_INITIAL_STATE_MODE,
        "initial_state_source": "current_physical_state_p_n",
        "perturbation_enabled": INITIAL_STATE_PERTURBATION_ENABLED,
        "count": 1,
        "unique_initial_state_count": 1,
        "duplicated_row_count": 0,
        "radius_linf": 0.0,
        "random_seed": None,
        "max_abs_distance_to_exact": max_abs_distance_to_exact,
        "reference_problem_raw_radius": problem.raw_sampling_radius,
        "reference_problem_clamped_radius_unused": problem.sampling_radius,
    }
...
def build_problem_dataset(
    *,
    ...
    initial_y, sampling = generate_unperturbed_point(
        problem=problem,
        physical=physical,
    )
    return DatasetBundle(
        initial_y=initial_y,
        q=problem.q_free.reshape(1, -1).clone(),
        masses=problem.free_masses.reshape(1, -1).clone(),
        exact_y=problem.exact_y_free.reshape(1, -1).clone(),
    ...
    *,
    ...
    return DatasetBundle(
        ...
        metadata={
            ...
            "rows_per_problem": 1,
            "unique_initial_states_per_problem": 1,
            "effective_unique_initial_state_count": len(problem_indices),
            "dataset_initial_state_mode": DATASET_INITIAL_STATE_MODE,
            "initial_state_perturbation_enabled": INITIAL_STATE_PERTURBATION_ENABLED,
...
def build_dataset_for_motion_times(
    *,
    ...
    return concatenate_datasets(datasets, role=role)
...
class MLPOptimizer(nn.Module):
    def __init__(self, residual_length_scale: float, model_spec: ModelSpec) -> None:
        ...
        self.output_layer = nn.Linear(
            model_spec.width, FREE_STATE_DIM, bias=model_spec.use_bias
        )
    ...
    def architecture_description(self) -> str:
        ...
        return (
            f"3 x {FREE_STATE_DIM}D history channels "
            ...
            f"{self.model_spec.depth} -> {FREE_STATE_DIM}D update ({bias_text})"
        )
...
def run_experiment(
    *,
    ...
    print(
        ...
        f"unique_initial_states={training_cpu.metadata['effective_unique_initial_state_count']}, "
        f"dataset_mode={training_cpu.metadata['dataset_initial_state_mode']}, "
    ...
    report = {
        ...
        "model": {
            ...
            "input_channel_dimension": FREE_STATE_DIM,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "5x5 triangular-cloth activation/depth/width ablation with a "
            "degenerate no-initial-perturbation dataset with exactly one row per problem; "
...
def save_all_experiments_markdown_report(
    ...
    *,
    ...
    lines = [
        ...
        "- Learned-optimizer input: current residual, previous residual, and previous displacement (`3 x 69`, flattened to `207D`).",
        "- Residual-threshold update guard: disabled.",
        "- Dataset initial state: unperturbed current physical state `p_n`.",
        "- Each motion/time-step problem is stored exactly once; no duplicated dataset rows are used.",
        "",
    ]
...
def main() -> None:
    ...
    save_json({
        ...
        "initial_state_perturbation_enabled": INITIAL_STATE_PERTURBATION_ENABLED,
        "dataset_note": (
            "Each motion/time-step problem uses current state p_n as its only initial "
            "state and is stored exactly once; no duplicated rows are present."
        ),
    ...
    if len(multi_training) != 256:
        raise AssertionError(f"Expected 256 unique training problems, got {len(multi_training)}")
    ...
    expected_dataset_sizes = {
        "multi_motion_training": 256,
        "unseen_motion_validation": 40,
        "seen_motion_temporal_interpolation": 256,
        "seen_motion_temporal_extrapolation": 320,
        "unseen_id_test": 80,
        "ood_test": 160,
    }
    actual_dataset_sizes = {
        "multi_motion_training": len(multi_training),
        "unseen_motion_validation": len(validation),
        "seen_motion_temporal_interpolation": len(seen_interp),
        "seen_motion_temporal_extrapolation": len(seen_extrap),
        "unseen_id_test": len(unseen_id),
        "ood_test": len(ood_test),
    }
    if actual_dataset_sizes != expected_dataset_sizes:
        raise AssertionError(
            f"Unexpected one-row-per-problem dataset sizes: {actual_dataset_sizes}; "
            f"expected {expected_dataset_sizes}"
        )
    evaluation_datasets = {
