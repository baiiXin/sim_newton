"""
Fixed-left-edge 5x5 triangular-cloth large-MLP learned-optimizer ablation.

This script evaluates the complete Cartesian product
activation {ReLU, Tanh} x hidden width {512, 1024} x hidden depth {1, 2}
x bias {disabled, enabled}, for 16 model configurations in total.
The default device is cuda:1.
...
def build_model_specs(config: RuntimeConfig) -> list[ModelSpec]:
    # Requested order: activation x hidden width x hidden depth x bias.
    specs = [
        ModelSpec(
    ...
    report = {
        ...
        "model": {
            ...
            "linear_bias_initialization": (
                ...
                "no bias parameter is created when bias is disabled"
            ),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "5x5 triangular-cloth ReLU/Tanh x width x depth x bias ablation "
            "on cuda:1 by default"
        )
    )
    ...
    parser.add_argument(
        ...
        choices=("no", "yes"),
        default=["no", "yes"],
        help="Bias choices. Use 'no', 'yes', or both (default: both).",
    )
    parser.add_argument(
        "--config-index",
        type=int,
        default=None,
        help=(
            "Run one zero-based index from the filtered "
            "activation/width/depth/bias grid."
        ),
    )
...
def validate_args(args: argparse.Namespace) -> RuntimeConfig:
    ...
    biases = tuple(dict.fromkeys(value == "yes" for value in args.biases))
    if not biases:
        raise ValueError("At least one bias option is required")
...
def plot_ablation_heatmaps(
    ...
    *,
    ...
    depths: Sequence[int],
    widths: Sequence[int],
    biases: Sequence[bool],
    save_path: Path,
) -> None:
    successful = _successful_experiment_summaries(summaries)
    if not successful:
        return
    by_spec: dict[tuple[str, int, int, bool], dict[str, Any]] = {}
    for summary in successful:
        spec = summary.get("model_spec") or {}
        key = (
            str(spec.get("activation")),
            int(spec.get("depth")),
            int(spec.get("width")),
            bool(spec.get("use_bias")),
        )
        by_spec[key] = summary
    ...
    fig, axes = plt.subplots(
        len(metrics),
        len(column_specs),
        figsize=(5.2 * len(column_specs), 4.8 * len(metrics)),
        squeeze=False,
    )
    image = None
    for row, (_, metric_title) in enumerate(metrics):
        for col, (activation, use_bias) in enumerate(column_specs):
            ...
            axes[row, col].set_xticks(
                range(len(widths)), [str(width) for width in widths]
            )
            axes[row, col].set_yticks(
                range(len(depths)), [str(depth) for depth in depths]
            )
            axes[row, col].set_xlabel("Hidden width")
            axes[row, col].set_ylabel("Hidden depth")
            bias_label = "bias" if use_bias else "no bias"
            axes[row, col].set_title(
                f"{activation}, {bias_label}: {metric_title}"
            )
            for i in range(len(depths)):
                for j in range(len(widths)):
                    ...
                    axes[row, col].text(
                        j, i, label, ha="center", va="center", fontsize=8
                    )
    if image is not None:
        colorbar = fig.colorbar(image, ax=axes.ravel().tolist(), shrink=0.85)
        colorbar.set_label("log10 residual")
    fig.suptitle("Activation-width-depth-bias ablation summary")
    fig.subplots_adjust(top=0.92, wspace=0.30, hspace=0.32)
    plt.savefig(save_path, dpi=220, bbox_inches="tight")
    plt.close(fig)
    ...
    for axis, (metric_key, title) in zip(axes, metric_specs):
        groups = sorted({
            (
                ...
                bool((summary.get("model_spec") or {}).get("use_bias")),
            )
            for summary in successful
        })
        for activation, use_bias in groups:
            selected = [
                summary
                for summary in successful
                if (
                    str((summary.get("model_spec") or {}).get("activation"))
                    == activation
                    and bool((summary.get("model_spec") or {}).get("use_bias"))
                    == use_bias
                )
            ]
            ...
            bias_label = "bias" if use_bias else "no bias"
            axis.scatter(
                x, np.maximum(y, PLOT_FLOOR), label=f"{activation}, {bias_label}"
            )
        axis.set_xscale("log")
        axis.set_yscale("log")
...
def save_all_experiments_markdown_report(
    ...
    *,
    ...
    lines.extend([
        "## Top Configurations",
        "",
        "| Rank | Activation | Width | Depth | Bias | Parameters | Validation p95 | Validation max | OOD p95 | OOD max |",
        "|---:|---|---:|---:|---|---:|---:|---:|---:|---:|",
    ])
    for rank, summary in enumerate(ranked[:10], start=1):
        ...
        lines.append(
            "| "
            f"{rank} | {spec.get('activation')} | {spec.get('width')} | {spec.get('depth')} | "
            f"{spec.get('use_bias')} | {summary.get('parameter_count')} | "
            f"{summary.get('final_residual_p95')} | "
...
def save_output_artifact_manifest(
    *,
    ...
    save_json({
        ...
        "note": (
            "The large-width bias ablation preserves every original plot filename pattern "
...
def main() -> None:
    ...
    print(f"Device: {device}; bias_options={config.biases}")
    ...
    save_json({
        ...
        "bias_options": list(config.biases),
    ...
    summary = {
        "experiment_type": "fixed_left_edge_5x5_cloth_relu_tanh_width_depth_bias_ablation",
        "bias_options": list(config.biases),
        "runtime_config": asdict(config),
        "physical_config": asdict(physical),
        "motion_split": asdict(motion_split),
