...
ReLU-versus-Tanh activation ablation on independent multi-time-step problems.
...
12. Train exactly two otherwise identical multi-problem models, using ReLU and
    Tanh respectively as the hidden activation function.
...
class MLPOptimizer(nn.Module):
    ...
    Bias-free residual optimizer:
        ...
        u       = dt^2 M^{-1} grad E(y) / s
        delta_y = s * W2 * phi(W1 * u)

    The activation phi is the only controlled variable in this experiment:
    either ReLU or Tanh. The input and raw output are dimensionless. Because
    both linear layers are bias-free and both tested activations satisfy
    phi(0)=0, grad E(y)=0 implies delta_y=0, so every stationary solution is
    ...
    def __init__(
        ...
        if activation_name == "relu":
            ...
            raise ValueError(
                ...
                f"expected one of {ACTIVATION_NAMES}."
            )
...
def run_experiment(
    *,
    ...
    print(
        ...
        f"architecture=6_dimless_residual->64->{activation_name}->6_dimless_update, "
    ...
    report = {
        "config": {
            ...
            "architecture": (
                ...
                f"64 -> {activation_name} -> 6-dimensional update"
            ),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare ReLU and Tanh activations for the independent multi-time-step "
            "two-particle spring learned optimizer."
        )
    )
...
def main() -> None:
    ...
    save_json(
        {
            ...
            "fixed_model_configuration": {
                "architecture": (
                    ...
                    "-> 64 -> activation -> 6D dimensionless update"
                ),
                "tested_activations": list(ACTIVATION_NAMES),
                "optimizer": "Adam",
                "learning_rate": LEARNING_RATE,
                "residual_length_scale": config.residual_length_scale,
                "gradient_clip_norm": config.gradient_clip_norm,
    ...
    for activation_name in ACTIVATION_NAMES:
        summaries.append(
            run_experiment(
                experiment_name=f"multi_problem_{activation_name}",
                activation_name=activation_name,
    ...
    overall_report = {
        "experiment_type": "relu_vs_tanh_activation_ablation",
        "purpose": (
            "Compare ReLU and Tanh while holding the multi-time-step training "
            "problems, datasets, initialization seed, optimizer, loss, rollout "
            "schedule, and checkpoint selection fixed."
        ),
        "runtime_config": asdict(config),
        "physical_config": asdict(physical),
        "problem_split": asdict(split),
