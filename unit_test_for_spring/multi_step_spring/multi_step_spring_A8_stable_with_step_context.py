"""
Two-particle single-spring learned-optimizer supplementary ablation.

This script runs only one new controlled experiment:

    A8_stable_with_step_context

A8 starts from A3_stable_full and changes only the network input.  It keeps
A3's dimensionless mass-preconditioned residual and appends standardized
single-time-step problem information:

    [u, q, m1, m2, dt, k, l0],

where u = dt^2 M^{-1} grad E(y) / s.  The resulting input dimension is 17.
All other settings are inherited from A3: bias-free linear network,
orthogonal first-layer initialization, zero output-layer initialization,
length-scaled output, shifted and nondimensionalized energy objective,
global gradient clipping, full-batch K curriculum, validation checkpoint
selection, and the complete interpolation/extrapolation/special-state/Newton
evaluation suite.
...
DEFAULT_ORIGINAL_EXPERIMENT_DIR = Path(
    "/data/zhoucy/sim_newton/unit_test_for_spring/multi_step_spring/"
    "multi_step_spring_ablation"
)
DEFAULT_OUTPUT_DIR = DEFAULT_ORIGINAL_EXPERIMENT_DIR / "A8_stable_with_step_context_rerun"
...
class ExperimentSpec:
    ...
    def input_dimension(self) -> int:
        if self.input_mode == "dimensionless_residual_with_step_context":
            return 17
...
EXPERIMENT_SPECS: tuple[ExperimentSpec, ...] = (
    ExperimentSpec(
        name="A8_stable_with_step_context",
        short_label="A8",
        input_mode="dimensionless_residual_with_step_context",
        use_bias=False,
        ...
        comparison_role="supplementary input ablation from A3",
        changed_factor=(
            "Append standardized single-time-step problem information to the "
            "A3 dimensionless residual input"
        ),
        interpretation=(
            "Tests whether the dimensionless residual is already a sufficient "
            "optimizer state, or whether explicit q/m/dt/k/l0 context improves "
            "interpolation or extrapolation. Because context remains nonzero at "
            "the exact solution, exact-state drift is evaluated explicitly."
        ),
    ),
)
...
def create_output_directory(configured_path: str) -> Path:
...
def build_step_context_features(
    q: torch.Tensor,
    masses: torch.Tensor,
    physical: PhysicalConfig,
) -> torch.Tensor:
    """Build 11D single-time-step context [q,m1,m2,dt,k,l0]."""
    ...
    return torch.cat([q, masses, physical_constants], dim=-1)


def compute_step_context_normalizer(
    dataset: DatasetBundle,
    physical: PhysicalConfig,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Compute training-set statistics only for the appended 11D context."""
    context = build_step_context_features(dataset.q, dataset.masses, physical)
    mean = context.mean(dim=0)
    std = context.std(dim=0, unbiased=False)
    # m1, m2, dt, k, and l0 are fixed in this experiment. After centering,
    # those entries are exactly zero; divisor one avoids division by zero.
    std = torch.where(std > 1e-12, std, torch.ones_like(std))
    return mean, std
...
class MLPOptimizer(nn.Module):
    ...
    def __init__(
        ...
        *,
        spec: ExperimentSpec,
        residual_length_scale: float,
        step_context_mean: torch.Tensor | None = None,
        step_context_std: torch.Tensor | None = None,
    ) -> None:
        super().__init__()
        ...
        if step_context_mean is None or step_context_std is None:
            raise ValueError(
                "A8 requires training-set normalizer statistics for step context."
            )
        if tuple(step_context_mean.shape) != (11,) or tuple(step_context_std.shape) != (11,):
            raise ValueError("Step-context normalizer tensors must have shape (11,).")
        self.register_buffer(
            "step_context_mean", step_context_mean.detach().clone()
        )
        self.register_buffer(
            "step_context_std", step_context_std.detach().clone()
        )
    ...
    def build_input(
        ...
        if self.spec.input_mode == "dimensionless_residual_with_step_context":
            residual_input = dimensionless_residual_input(
                y,
                q,
                masses,
                physical,
            ...
            step_context = build_step_context_features(q, masses, physical)
            standardized_context = (
                step_context - self.step_context_mean
            ) / self.step_context_std
            return torch.cat([residual_input, standardized_context], dim=-1)
...
def run_experiment(
    *,
    ...
    step_context_mean, step_context_std = compute_step_context_normalizer(
        training_cpu, physical
    )

    model = MLPOptimizer(
        spec=spec,
        residual_length_scale=config.residual_length_scale,
        step_context_mean=step_context_mean,
        step_context_std=step_context_std,
    ).to(device)
    ...
    report = {
        ...
        "config": {
            ...
            "step_context_standardization": True,
            "step_context_feature_order": [
                "q1_x", "q1_y", "q1_z", "q2_x", "q2_y", "q2_z",
                "m1", "m2", "dt", "spring_k", "rest_length",
            ],
            "step_context_mean": tensor_to_list(step_context_mean),
            "step_context_std": tensor_to_list(step_context_std),
            "residual_length_scale": config.residual_length_scale,
            ...
            "fixed_point_note": (
                "A8 does not structurally guarantee zero update at zero residual, "
                "because standardized step context can remain nonzero. The exact-state "
                "evaluation measures the resulting drift explicitly."
            ),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run only A8: A3 stable residual input plus standardized "
            "single-time-step context, with the complete evaluation suite."
        )
    )
    ...
    parser.add_argument(
        ...
        help=(
            "Result directory. Defaults to an A8 rerun subdirectory under the "
            "original ablation directory, so previous results are not overwritten."
        ),
    )
    ...
    parser.add_argument(
        ...
        help="Only A8_stable_with_step_context is available in this script.",
    )
    parser.add_argument(
...
def main() -> None:
    ...
    save_json(
        {
            ...
            "experiment_specs": [asdict(spec) for spec in selected_specs],
    ...
    dataset_metadata = {
        ...
        "datasets_match_original_A0_to_A7_ablation": True,
    ...
    save_ablation_summary_csv(ablation_rows, output_dir / "A8_summary.csv")
    save_json({"rows": ablation_rows}, output_dir / "A8_summary.json")

    overall_report = {
        "experiment_type": "single_supplementary_A8_input_ablation",
        "purpose": (
            "Test whether explicit single-time-step problem context improves the "
            "A3 stable residual solver, while re-running the complete evaluation suite."
        ),
        "runtime_config": asdict(config),
        "physical_config": asdict(physical),
        "problem_split": asdict(split),
    ...
    save_json(overall_report, output_dir / "A8_full_evaluation_summary.json")

    if not config.skip_plots and summaries:
        plot_model_comparison_final_metrics(
            summaries=summaries,
            save_path=output_dir / "A8_final_metrics.png",
        )
        plot_current_state_residual_model_comparison(
            ...
            report_steps=config.report_steps,
            save_path=output_dir / "A8_current_state_residual.png",
        )
        if newton_results is not None:
            plot_all_solver_final_metrics(
                summaries=summaries,
                newton_results=newton_results,
                save_path=output_dir / "A8_and_newton_final_metrics.png",
            )
            plot_current_state_residual_all_solvers(
                ...
                problem_indices=split.all_test_indices,
                report_steps=config.report_steps,
                save_path=output_dir / "A8_current_state_with_newton.png",
            )
            for record in summaries:
    ...
    print("\n" + "=" * 100)
    print("A8 training and the complete evaluation suite completed.")
    print(f"Summary CSV: {output_dir / 'A8_summary.csv'}")
    print(f"Summary JSON: {output_dir / 'A8_full_evaluation_summary.json'}")
