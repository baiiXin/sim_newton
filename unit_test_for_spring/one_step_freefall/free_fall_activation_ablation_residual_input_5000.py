"""
自由落体单帧变分问题：稳定残差输入结构的激活函数消融实验
===========================================================

本脚本保留原激活函数消融的全部数据集、优化器、验证集 checkpoint 选择、
独立测试集评估和绘图流程，只把已经验证较差的 12 维绝对状态输入结构替换为
稳定的无量纲质量预条件残差结构。

正式实验设置
------------
1. 网络固定为 3 -> 64 -> activation -> 3；
2. 输入为无量纲质量预条件变分残差：
       u = dt^2 M^{-1} grad E(y) / s；
   对本自由落体二次问题，该量恰好等于 (y-y*)/s，但实现中不使用 y* 构造输入；
3. 输出为无量纲更新，并映射回位置单位：delta_y = s * MLP(u)；
4. 两个线性层均不使用 bias，使零残差严格映射到零更新；
5. 第一层正交初始化，输出层零初始化；
6. 激活函数测试 identity、relu、leaky_relu、elu、tanh、gelu、silu；
7. 训练集目标规模固定为 8、1,000、100,000；规则网格实际规模为
   8、1,000、97,336；
8. 优化器固定为 SGD(lr=1e-2) 与 Adam(lr=1e-4)，共 42 组实验；
9. 数值精度为 torch.float64，默认设备为 cuda:1，训练使用 Full-Batch；
10. 训练目标仍是原始物理变分能量，只减去固定初始能量并除以正能量尺度；
    该处理不改变最优解和参数梯度方向；
11. 使用全局梯度范数裁剪，并记录一步更新质量诊断；
12. K 从 1 开始，每 1,000 epoch 增加 1，最高 K=5；
13. 每组完整训练 5,000 epoch，不执行 early stopping；
14. 共享固定的 1,024 点验证集和 3,072 点测试集；每 500 epoch 验证一次，
    用最终 residual p95 选择最佳 checkpoint；
15. 精确解只用于生成采样中心、报告误差、选择 checkpoint 和诊断，
    不作为网络输入，也不进入反向传播目标。

默认运行
--------
    python free_fall_activation_ablation_residual_input_5000.py
...
-----------------
    python free_fall_activation_ablation_residual_input_5000.py \
        ...
        --epochs 2 --validation-interval 1 --diagnostic-interval 1 \
...
DEFAULT_RESIDUAL_LENGTH_SCALE = 1e-2
...
class MLPOptimizer(nn.Module):
    """1×64 无量纲残差学习型迭代器：3 -> 64 -> activation -> 3。

    输入：
        u = dt^2 M^{-1} grad E(y) / s

    对当前自由落体问题，质量预条件后质量 m 被消去，且

        dt^2 M^{-1} grad E(y)
        = y - p_n - dt v_n + dt^2 g e_z.

    输出：
        delta_y = s * W2 * activation(W1 * u)

    两层均无 bias；所有被测激活函数都满足 activation(0)=0，故零残差严格
    映射到零更新。第一层正交初始化，第二层零初始化。
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.linear1 = nn.Linear(3, 64, bias=False)
        self.activation = create_activation(self.activation_name)
        self.linear2 = nn.Linear(64, 3, bias=False)
    ...
    def forward(
        ...
        single_state = y.ndim == 1
        if single_state:
            if y.shape[0] != 3:
                raise ValueError(f"Expected y shape [3], got {tuple(y.shape)}")
        ...
        p_n = history_batch[:, 0:3]
        v_n = history_batch[:, 3:6]
        g = params_batch[:, 1:2]
        dt = params_batch[:, 2:3]

        mass_preconditioned_residual = y_batch - p_n - dt * v_n
        gravity_term = torch.zeros_like(mass_preconditioned_residual)
        gravity_term[:, 2:3] = dt.square() * g
        mass_preconditioned_residual = (
            mass_preconditioned_residual + gravity_term
        )

        normalized_residual = (
            mass_preconditioned_residual / self.residual_length_scale
        )
        hidden = self.activation(self.linear1(normalized_residual))
        dimensionless_update = self.linear2(hidden)
        delta = self.residual_length_scale * dimensionless_update
        return delta.squeeze(0) if single_state else delta
...
def physical_energy_scale(
    *,
    ...
    """返回正的特征能量尺度 m*s^2/dt^2。"""
    if m <= 0.0 or dt <= 0.0 or residual_length_scale <= 0.0:
        raise ValueError("m, dt, and residual_length_scale must be positive.")
    return m * residual_length_scale**2 / dt**2
...
def one_step_quality_diagnostics(
    *,
    ...
    """报告一次学习更新的方向、收缩性和固定点性质，不参与反向传播。"""
    model.eval()
    y0 = training_grid
    delta = model(y0, history, params)
    y1 = y0 + delta

    exact_batch = y_star.unsqueeze(0).expand_as(y0)
    error_before = torch.linalg.vector_norm(y0 - exact_batch, dim=-1)
    error_after = torch.linalg.vector_norm(y1 - exact_batch, dim=-1)
    residual_before = stationarity_residual_norm(y0, p_n, v_n, m, g, dt)
    residual_after = stationarity_residual_norm(y1, p_n, v_n, m, g, dt)
    energy_before = variational_energy(y0, p_n, v_n, m, g, dt)
    energy_after = variational_energy(y1, p_n, v_n, m, g, dt)

    ideal_update = exact_batch - y0
    ...
    if float(denominator.item()) > 0.0:
        update_ideal_cosine = float(
            (torch.dot(flat_delta, flat_ideal) / denominator).item()
        )
    else:
        update_ideal_cosine = float("nan")

    nonzero = error_before > 1e-15
    contraction = error_after[nonzero] / error_before[nonzero]
    if contraction.numel() == 0:
        ...
        contraction_stats = {
            ...
            "median": float(torch.quantile(contraction, 0.50).item()),
            "p95": float(torch.quantile(contraction, 0.95).item()),
            "max": float(contraction.max().item()),
        }

    fixed_point_delta = model(y_star, history, params)
    return {
        ...
        "update_ideal_cosine": update_ideal_cosine,
        ...
        "fixed_point_update_norm": float(
            torch.linalg.vector_norm(fixed_point_delta).item()
        ),
    }
...
def run_experiment(
    *,
    ...
    params = torch.tensor([m, g, dt], dtype=TORCH_DTYPE, device=device)
    exact_energy = variational_energy(y_star, p_n, v_n, m, g, dt).detach()
    e_star = float(exact_energy.item())
    initial_energy = variational_energy(
        training_grid, p_n, v_n, m, g, dt
    ).detach()
    ...
    print(
        ...
        f"architecture=3_dimless_residual->64->{activation_name}->3_dimless_update"
    )
    print(
    ...
    print(
        ...
        f"energy_scale={energy_scale:.3e}, grad_clip={config.gradient_clip_norm:g}"
    )
    print(
        "bias_free=True; orthogonal_first_layer=True; "
        "zero_output_layer=True; zero_residual_is_exact_fixed_point=True"
    )
    ...
    for epoch_index in range(config.epochs):
        ...
        for _ in range(k):
            y = y + model(y, history, params)
            current_energy = variational_energy(y, p_n, v_n, m, g, dt)

            # 只对原始物理能量做常数平移和正尺度缩放：
            #   L_hat = (E(y_k) - stop_grad(E(y_0))) / E_scale.
            # 不改变参数梯度方向，不使用精确解构造训练目标。
            ...
            validation_log.append(
                {
                    ...
                    "training_K": k,
                    "selection_key": (
                        list(current_key) if current_key is not None else None
                    ),
    ...
    best_test_eval, best_pn_eval, best_pn_trajectory = evaluate_checkpoint(
        best_state_dict
    )
    last_test_eval, last_pn_eval, last_pn_trajectory = evaluate_checkpoint(
        last_state_dict
    )
    ...
    report = {
        "config": {
            ...
            "device": str(device),
            "architecture": f"3 -> 64 -> {activation_name} -> 3",
            "hidden_dims": [64],
            "activation_name": activation_name,
            ...
            "checkpoint_selection": (
                "minimum final-step validation residual p95; tie-break by "
                "residual median and energy-gap p95"
            ),
            ...
            "loss": (
                "sum over rollout steps of mean((physical_energy(y_k)-"
                "stop_grad(physical_energy(y_0)))/energy_scale)"
            ),
            "loss_equivalence": (
            ...
            "backpropagation": (
                "full unroll without detach; one backward and one optimizer "
                "step per epoch"
            ),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Free-fall residual-input MLP activation-function ablation with validation-selected checkpoints."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        help="Characteristic length s in residual input and position output mapping.",
    )
    parser.add_argument(
        "--gradient-clip-norm",
        type=float,
    ...
    parser.add_argument(
        ...
        help="Interval for one-step update-quality diagnostics.",
    )
...
def main() -> None:
    ...
    overall_report = {
        "experiment_type": "activation_function_ablation_with_dimensionless_residual_input",
        "purpose": (
            "Use the validated dimensionless mass-preconditioned residual architecture while "
            "holding the three training-grid scales, two optimizer settings, datasets, loss, "
            "initialization, rollout schedule, and checkpoint selection fixed; change only "
            "the hidden activation function."
        ),
        "runtime_config": asdict(config),
        "network": {
            "architecture_template": "3 -> 64 -> activation -> 3",
            "hidden_dims": [64],
            "activation_names": config.activation_names,
            "input_definition": (
                "dt^2 * M^{-1} * stationarity_residual / residual_length_scale"
            ),
            "output_definition": (
                "residual_length_scale * dimensionless network output"
            ),
            ...
            "zero_residual_is_exact_fixed_point": True,
        ...
        "training_processing": {
            ...
            "diagnostic_interval": config.diagnostic_interval,
            "loss": (
                "shifted and positively scaled original physical variational energy"
            ),
            "exact_solution_in_backward_objective": False,
        },
