...
data_path = os.path.join(DATA, "cloth_data_cloth2.npy")
topy_path = os.path.join(DATA, "cloth_topy_cloth2.npy")
output_video_path = os.path.join(OUTPUT, 'video', 'cloth2.mp4')
FPS = 30
Smooth_shade = False
...
try:
    cloth_f = np.load(topy_path, allow_pickle=True)
...
print(f"Topology shape: {cloth_f.shape}")
print(f"Data shape: {sim_data.shape}")
print(cloth_f.dtype, sim_data.dtype)

# 提取初始帧顶点，用于注册网格
cloth_v = sim_data[0]

# 注册初始网格
ps.register_surface_mesh("Cloth", cloth_v, cloth_f, color=(0.5, 0.7, 1.0), smooth_shade=Smooth_shade)

# ==========================================
# 4. 读取动画数据
# ==========================================
# 这里 cloth_data_cloth2.npy 已经只包含 cloth2 的每帧顶点位置
...
def callback():
    ...
    # --- 2. 更新 Polyscope ---
    ps.get_surface_mesh("Cloth").update_vertex_positions(current_frame_data)
...
# target: 看向原点 (0,0,0)
ps.look_at((8.0, 0.0, -8.0), (0.0, 0.0, 0.0))
