...
def main():
    ...
    for epoch in range(epochs):
        if epoch > 0 and epoch % 200 == 0 and K < 2:
            K += 1
    ...
    # MLP 迭代点
    mlp_points = np.array([it["y"] for it in mlp_hist["iterations"]])

    # 画 MLP 轨迹连线
    ax.plot(
        mlp_points[:, 0], mlp_points[:, 1], mlp_points[:, 2],
        '-o', linewidth=1.5, markersize=4, label='MLP trajectory'
    )

    # 给每个 MLP 点标序号
    for i, p in enumerate(mlp_points):
        if i == 0:
            # 初值点
            ax.scatter(p[0], p[1], p[2], s=80, c='black')
            ax.text(p[0], p[1], p[2], '0', fontsize=10, color='black')
        else:
            ax.scatter(p[0], p[1], p[2], s=35, c='tab:blue')
            ax.text(p[0], p[1], p[2], str(i), fontsize=9, color='tab:blue')

    # Newton 收敛点（这个问题中 1 步就收敛，所以取第 1 步即可）
    newton_conv = np.array(newton_hist["iterations"][1]["y"])
    ax.scatter(
        newton_conv[0], newton_conv[1], newton_conv[2],
        marker='*', s=250, c='crimson', label='Newton converged point'
    )
    ax.text(
        newton_conv[0], newton_conv[1], newton_conv[2],
    ...
    ax.set_title('Output Distribution / Iteration Trajectory')
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.set_zlabel('z')
    ax.legend()
    ...
    plt.tight_layout()
    plt.savefig('optimization_distribution.png', dpi=300, bbox_inches='tight')
    print("🖼️ 输出分布图已保存至: optimization_distribution.png")
    print("="*50)
