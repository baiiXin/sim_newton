...
def main():
    ...
    for epoch in range(epochs):
        ...
        # === 严格遵循您的伪代码 ===
        for k in range(K):
            delta = mlp(delta, history, params)  # 前向：网络预测位置更新
            y = y + delta                     # 应用更新
        ...
        if epoch % 100 == 0 or epoch == epochs - 1:
            y_eval = y0.clone() + dt * v_n
            d = dt * v_n
            eval_steps = []
            for i in range(10):
                with torch.no_grad():
                    d = mlp(d, history, params)
                y_eval = y_eval + d
    ...
    y_mlp, y_new = y0.clone() + dt * v_n, y0.clone() + dt * v_n
    d_m = dt * v_n
    ...
    for _ in range(max_steps):
        with torch.no_grad(): d_m = mlp(d_m, history, params)
