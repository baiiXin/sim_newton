...
Evaluation:
    ...
    - Plot the per-epoch training loss and objective-gap curves without removing
      any existing logs or evaluation artifacts.
...
def plot_training_loss_curve(
    ...
    """
    Plot the training objective recorded after processing each epoch.

    Each epoch randomly selects one physical-state group and trains on its ten
    perturbed optimization initials. The curves preserve two complementary
    views:
        1. mean / worst final variational energy over the ten trained samples;
        2. mean / worst objective gap E(y) - E(y*) over the same samples.

    For the current single-step experiment, each final value is also the loss
    produced by that sample's only learned-optimizer update.
    """

    epochs = [item["epoch"] for item in train_log]
    mean_losses = [item["mean_final_loss"] for item in train_log]
    max_losses = [item["max_final_loss"] for item in train_log]
    mean_gaps = [max(item["mean_final_gap"], PLOT_FLOOR) for item in train_log]
    max_gaps = [max(item["max_final_gap"], PLOT_FLOOR) for item in train_log]

    fig, axes = plt.subplots(1, 2, figsize=(15, 5.5))

    axes[0].plot(epochs, mean_losses, linewidth=1.3, label="Mean training loss")
    axes[0].plot(epochs, max_losses, linewidth=1.1, alpha=0.85, label="Worst training loss")
    ...
    axes[1].plot(epochs, mean_gaps, linewidth=1.3, label="Mean training gap")
    axes[1].plot(epochs, max_gaps, linewidth=1.1, alpha=0.85, label="Worst training gap")
    axes[1].set_yscale("log")
    axes[1].set_xlabel("Epoch")
    axes[1].set_ylabel(r"Objective gap $E(y)-E(y^*)$")
    axes[1].set_title("Training Objective Gap over Epochs")
    axes[1].legend()
    axes[1].grid(True, alpha=0.3)

    fig.suptitle("Per-Epoch Training Loss Curves", fontsize=14)
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close(fig)
...
def plot_optimizer_training_loss_comparison(
    ...
    """Compare per-epoch training loss curves for the two optimizers."""

    fig, axes = plt.subplots(1, 2, figsize=(15, 5.5))

    for summary in summaries:
        epochs = np.arange(len(summary["training_mean_loss_by_epoch"]))
        label = f"{summary['optimizer_name'].upper()} lr={summary['learning_rate']:.0e}"

        axes[0].plot(
            epochs,
            summary["training_mean_loss_by_epoch"],
            linewidth=1.2,
            label=f"{label} mean",
        )
        axes[0].plot(
            epochs,
            summary["training_max_loss_by_epoch"],
            linewidth=1.0,
            alpha=0.70,
            label=f"{label} worst",
        )
        axes[1].plot(
            epochs,
            np.maximum(summary["training_mean_gap_by_epoch"], PLOT_FLOOR),
            linewidth=1.2,
            label=f"{label} mean",
        )
        axes[1].plot(
            epochs,
            np.maximum(summary["training_max_gap_by_epoch"], PLOT_FLOOR),
            linewidth=1.0,
            alpha=0.70,
            label=f"{label} worst",
        )
    ...
    axes[1].set_xlabel("Epoch")
    axes[1].set_ylabel(r"Objective gap $E(y)-E(y^*)$")
    axes[1].set_title("Training Objective Gap: Optimizer Comparison")
...
def run_experiment(
    ...
    training_loss_plot_path = output_dir / "training_loss_curve.png"
    plot_training_loss_curve(
        train_log=train_log,
        save_path=training_loss_plot_path,
    )
    ...
    report = {
        ...
        "artifacts": {
            ...
            "training_loss_curve_png": str(training_loss_plot_path),
    ...
    return {
        ...
        "training_mean_loss_by_epoch": [item["mean_final_loss"] for item in train_log],
        "training_max_loss_by_epoch": [item["max_final_loss"] for item in train_log],
        "training_mean_gap_by_epoch": [item["mean_final_gap"] for item in train_log],
        "training_max_gap_by_epoch": [item["max_final_gap"] for item in train_log],
        "training_loss_curve_path": str(training_loss_plot_path),
...
def main() -> None:
    ...
    plot_optimizer_training_loss_comparison(
        summaries=summaries,
        save_path=base_output_dir / "optimizer_comparison_training_losses.png",
    )
    ...
    print(f"Cross-optimizer training-loss plot: {base_output_dir / 'optimizer_comparison_training_losses.png'}")
    for summary in summaries:
        print(
