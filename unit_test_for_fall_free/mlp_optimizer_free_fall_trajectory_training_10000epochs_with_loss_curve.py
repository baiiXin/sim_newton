...
Training schedule:
    ...
    - Start from K = 1 learned-optimizer iteration per sample.
    - Increase K by one every 1000 epochs.

Evaluation:
    - Preserve all visited training points and final points for visualization.
    - On all 100 analytic test states, compare MLP and Newton residuals for 15 steps.
...
def plot_training_loss_curve(
    ...
    """Plot per-epoch training loss and objective-gap curves.

    Each epoch randomly selects one physical-state group and trains on its ten
    perturbed optimization initials. The plotted mean and max values aggregate
    the final state of those ten samples after the current learned-optimizer
    depth K.
    """

    epochs = np.asarray([item["epoch"] for item in train_log], dtype=np.int32)
    mean_losses = np.asarray(
        [item["mean_final_loss"] for item in train_log], dtype=float
    )
    max_losses = np.asarray(
        [item["max_final_loss"] for item in train_log], dtype=float
    )
    mean_gaps = np.maximum(
        np.asarray([item["mean_final_gap"] for item in train_log], dtype=float),
        PLOT_FLOOR,
    )
    max_gaps = np.maximum(
        np.asarray([item["max_final_gap"] for item in train_log], dtype=float),
        PLOT_FLOOR,
    )

    fig, axes = plt.subplots(1, 2, figsize=(15, 5.5))

    axes[0].plot(epochs, mean_losses, linewidth=1.0, label="Mean final training loss")
    axes[0].plot(epochs, max_losses, linewidth=1.0, label="Worst final training loss")
    axes[0].set_title("Training Loss over Epochs")
    axes[0].set_xlabel("Epoch")
    axes[0].set_ylabel(r"Variational energy $E(y)$")

    axes[1].plot(epochs, mean_gaps, linewidth=1.0, label="Mean final objective gap")
    axes[1].plot(epochs, max_gaps, linewidth=1.0, label="Worst final objective gap")
    axes[1].set_yscale("log")
    axes[1].set_title(r"Training Objective Gap $E(y)-E(y^*)$")
    axes[1].set_xlabel("Epoch")
    axes[1].set_ylabel("Objective gap")

    for ax in axes:
        for boundary in range(K_INCREASE_INTERVAL, EPOCHS, K_INCREASE_INTERVAL):
            ax.axvline(boundary, linestyle="--", linewidth=0.7, alpha=0.28)
        ax.legend()
        ax.grid(True, alpha=0.3)

    fig.suptitle(
        "Per-Epoch Training Curves; Dashed Lines Mark Curriculum K Increases",
        fontsize=13,
    )
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
...
def plot_final_residual_comparison(
    ...
    """Plot the requested 100-frame x 15-step residual comparison."""
    ...
    ax_curve.set_title("Residual Convergence: 100 Test Frames x 15 Iterations")
    ax_curve.legend()
...
def plot_optimizer_training_loss_comparison(
    summaries: List[Dict],
    save_path: Path,
) -> None:
    """Compare SGD and Adam using their per-epoch training curves."""

    fig, axes = plt.subplots(2, 2, figsize=(15, 10))

    for summary in summaries:
        epochs = np.asarray(summary["training_epochs"], dtype=np.int32)
        label = f"{summary['optimizer_name'].upper()} lr={summary['learning_rate']:.0e}"

        axes[0, 0].plot(
            epochs, summary["training_mean_final_losses"], linewidth=1.0, label=label
        )
        axes[0, 1].plot(
            epochs, summary["training_max_final_losses"], linewidth=1.0, label=label
        )
        axes[1, 0].plot(
            epochs,
            np.maximum(summary["training_mean_final_gaps"], PLOT_FLOOR),
            linewidth=1.0,
            label=label,
        )
        axes[1, 1].plot(
            epochs,
            np.maximum(summary["training_max_final_gaps"], PLOT_FLOOR),
            linewidth=1.0,
            label=label,
        )

    axes[0, 0].set_title("Mean Final Training Loss")
    axes[0, 1].set_title("Worst Final Training Loss")
    axes[1, 0].set_title("Mean Final Training Objective Gap")
    axes[1, 1].set_title("Worst Final Training Objective Gap")
    ...
    for ax in axes[0]:
        ax.set_ylabel(r"Variational energy $E(y)$")
    ...
    fig.suptitle(
        "Cross-Optimizer Training Curves; Dashed Lines Mark Curriculum K Increases",
        fontsize=13,
    )
    plt.tight_layout()
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
...
def run_experiment(
    ...
    training_loss_curve_path = output_dir / "training_loss_curve.png"
    plot_training_loss_curve(
        train_log=train_log,
        save_path=training_loss_curve_path,
    )
    ...
    final_eval_path = output_dir / "final_test_100_frames_15_steps.npz"
    np.savez_compressed(final_eval_path, **final_evaluation)

    plot_final_residual_comparison(
        evaluation=final_evaluation,
        save_path=output_dir / "final_test_residual_comparison_100_frames_15_steps.png",
    )
    ...
    report = {
        ...
        "artifacts": {
            ...
            "training_loss_curve_png": str(training_loss_curve_path),
    ...
    return {
        ...
        "training_epochs": [item["epoch"] for item in train_log],
        "training_mean_final_losses": [item["mean_final_loss"] for item in train_log],
        "training_max_final_losses": [item["max_final_loss"] for item in train_log],
        "training_mean_final_gaps": [item["mean_final_gap"] for item in train_log],
        "training_max_final_gaps": [item["max_final_gap"] for item in train_log],
        "training_loss_curve_path": str(training_loss_curve_path),
...
def main() -> None:
    ...
    optimizer_training_loss_plot_path = (
        base_output_dir / "optimizer_comparison_training_losses.png"
    )
    plot_optimizer_training_loss_comparison(
        summaries=summaries,
        save_path=optimizer_training_loss_plot_path,
    )
    ...
    summary_report = {
        ...
        "optimizer_comparison_training_loss_plot": str(
            optimizer_training_loss_plot_path
        ),
    ...
    print(f"Cross-optimizer training loss plot: {optimizer_training_loss_plot_path}")
    for summary in summaries:
        print(
