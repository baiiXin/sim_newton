...
# 本脚本显式使用 float32，并比较两个候选优化器在“精确 y0 + 在线扰动”训练下的性能。
...
# float32 混合初值训练实验：固定两个前序实验中表现较好的优化器方案。
#
# 每个 epoch 依次训练两条轨迹：
# 1. 精确初值轨迹：从 y0 出发；
# 2. 在线扰动轨迹：重新采样 y_online = y0 + sigma * epsilon，epsilon ~ N(0, I)。
#
# 每条轨迹均采用相同的完整展开训练方式：
# 1. 从当前轨迹初值完整展开 K 步 MLP 迭代；
# 2. 轨迹内部不执行 detach，因此梯度穿过完整轨迹；
# 3. 将当前轨迹前 K 步的 loss 相加；
# 4. 对当前轨迹单独执行一次 backward 和一次 optimizer.step；
# 5. 再进入下一条轨迹。
#
# 测试集只包含一个固定的小扰动初值：
#     y_test = y0 + 1e-4 * epsilon_test。
# 测试扰动使用独立固定随机种子，因此不同优化器实验共享完全相同的测试点。
#
# 其他固定条件：
# 1. 输入归一化统计量由在线扰动采样分布解析给出：位置均值为 y0，位置标准差为 sigma；
# 2. 网络输出乘以 dt；
# 3. 所有张量、模型参数和优化器状态均使用 torch.float32；
# 4. SGD 使用不带 momentum 的标准形式；
# 5. SGD 学习率固定为 1e-2；Adam 学习率固定为 1e-4；
# 6. K 从 5 开始，每 200 个 epoch 增加 5，1000 个 epoch 内依次为
#    5、10、15、20、25。
LOCAL_RANDOM_SEED = 123
TEST_RANDOM_SEED = 456
...
# 最终测试点相对于 y0 的绝对坐标扰动标准差。
TEST_PERTURBATION_STD = 1e-4
        ...
        ABLATION_EXPERIMENTS.append(
            {
                ...
                "description": (
                    "Float32 exact-y0 plus online resampled-perturbation training "
                    "with full normalization, "
                    f"{optimizer_name.upper()} optimizer, "
...
def sample_training_initial_y(
    y0,
    perturbation_std,
    generator,
):
    """
    为当前 epoch 在线采样一个训练初值。

    不再预先构造固定训练集。每次调用都重新生成：

        y_epoch = y0 + sigma * epsilon,
        epsilon ~ N(0, I).
    ...
    return y0 + perturbation_std * noise


def compute_online_input_normalizer(
    y0,
    perturbation_std,
    history,
    params,
    ...
    """
    根据在线采样分布解析构造输入归一化统计量。

    当前实验中，只有位置 y 会变化：

        y ~ N(y0, sigma^2 I).

    因此位置分量使用均值 y0 和标准差 sigma。history 与 params 在当前
    固定物理问题中不变化，使用其固定值作为均值，并将标准差设为 1。
    标准化后，这些固定输入分量恒为 0。
    """
    ...
    mean = torch.cat([y0, history, params], dim=-1)
    std = torch.ones_like(mean)
    std[: y0.numel()] = perturbation_std
    return mean, std
...
def plot_training_points_and_results(
    ...
    每个 epoch 依次训练两条轨迹：
        1. 从精确 y0 出发的轨迹；
        2. 从在线随机扰动 y0 + sigma * epsilon 出发的轨迹。

    每条轨迹均完整展开 K 步，并在该轨迹结束后单独更新一次网络参数。

    左图：所有轨迹中的训练输入点 x^(0), ..., x^(K-1)。
    右图：所有轨迹的最终结果点 x^(K)。

    特殊标记：
        半透明黑色 x：每个 epoch 在线采样的扰动起点；
        黑色大 x：精确初值 y0，也是每个 epoch 第一条训练轨迹的起点；
        红色 *：Newton 法收敛解。
    """
    ...
    for epoch_item in visualization_log:
        ...
        for trajectory in epoch_item["trajectories"]:
            ...
            if trajectory["trajectory_type"] == "online_perturbation":
                online_initial_points.append(
                    to_plot_coordinates(
                        [trajectory["initial_y"]],
                        reference_y0=reference_y0,
                        relative=relative,
                    )
                )
    ...
    online_initial_points = np.vstack(online_initial_points)
    y0_point = to_plot_coordinates(
        [reference_y0],
        reference_y0=reference_y0,
        relative=relative,
    )[0]
    ...
    for ax in (ax_train, ax_result):
        ax.scatter(
            online_initial_points[:, 0],
            online_initial_points[:, 1],
            online_initial_points[:, 2],
            marker="x",
            s=18,
            c="black",
            alpha=0.20,
    ...
    ax_result.set_title(r"All Trajectory Result Points: $x^{(K)}$")

    train_points_for_limits = np.vstack(
        all_train_points
        + [online_initial_points, y0_point.reshape(1, 3), newton_point.reshape(1, 3)]
    )
    ...
    marker_handles = [
        ...
        Line2D([0], [0], marker="^", color="w",
               label=r"Trajectory result points $x^{(K)}$",
               markerfacecolor="gray", markersize=8),
        Line2D([0], [0], marker="x", color="black",
               label="Online sampled perturbation starts",
               linestyle="None", markersize=7),
        Line2D([0], [0], marker="x", color="black",
               label="Exact y0 training start",
               linestyle="None", markersize=10, markeredgewidth=2.2),
        Line2D([0], [0], marker="*", color="w",
               label="Newton converged solution",
    ...
    for bucket in range(num_buckets):
        ...
        end_epoch = min((bucket + 1) * bucket_size - 1, epochs - 1)
        color_handles.append(
            Line2D([0], [0], marker="o", color="w",
                   label=f"Epoch {start_epoch}-{end_epoch}",
                   markerfacecolor=cmap(bucket), markersize=8)
        )
    ...
    fig.suptitle("Exact-y0 and Online-Perturbation Training by Epoch Range", fontsize=14)

    plt.tight_layout(rect=[0.00, 0.00, 0.80, 0.95])
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close(fig)

    print(f"🖼️ 混合初值训练点和结果点分布图已保存至: {save_path}")
...
def plot_summary_report(
    ...
    图 1：每个 epoch 在线采样训练起点对应的最终 gap；
    图 2：固定从 y0 出发进行周期性冻结评估后的 gap；
    图 3 和图 4：使用测试初值 y0 对比 MLP 与 Newton 法。
    """
    ...
    axes[0, 0].set_title("Worst Training Gap across y0 and Online-Perturbed Trajectories")
    ...
    axes[0, 1].set_title("Periodic Frozen Eval Gap from Original y0")
...
def plot_final_residual_comparison(
    ...
    当前实验的测试集只包含 initial_index=0，即固定的小扰动测试点。
    """
    ...
    if initial_description is None:
        title_suffix = f"Initial {initial_index}"
    else:
        title_suffix = f"Initial {initial_index}: {initial_description}"
...
def run_experiment(
    ...
    """
    运行一组 float32 混合初值训练实验，并将结果写入独立子目录。

    每个 epoch 依次处理两条轨迹：
        1. 精确初值轨迹：从 y0 出发；
        2. 在线扰动轨迹：重新采样 y_online = y0 + sigma * epsilon。

    对于每条轨迹：
        - 完整展开当前 K 次 MLP 更新；
        - 轨迹内部不执行 detach；
        - 累加前 K 个步骤的 loss；
        - 对该轨迹单独执行一次 backward 和一次 optimizer.step。

    测试集只包含一个固定的小扰动点：
        y_test = y0 + TEST_PERTURBATION_STD * epsilon_test。
    """
    ...
    print("\n" + "=" * 72)
    print(f"🧪 float32 精确 y0 + 在线扰动训练实验组: {experiment_name}")
    ...
    y_star = (
        p_n
        + dt * v_n
        - dt**2 * torch.tensor([0.0, 0.0, g], dtype=TORCH_DTYPE)
    )

    history = torch.cat([p_n, v_n])
    params = torch.tensor([m, g, dt], dtype=TORCH_DTYPE)

    # 在线扰动训练点使用独立可复现随机序列。
    training_generator = torch.Generator(device=y0.device)
    training_generator.manual_seed(LOCAL_RANDOM_SEED)

    # 测试集只包含一个固定的 1e-4 扰动点，不使用正式训练随机序列。
    test_generator = torch.Generator(device=y0.device)
    test_generator.manual_seed(TEST_RANDOM_SEED)
    test_initial_y = sample_training_initial_y(
        y0=y0,
        perturbation_std=TEST_PERTURBATION_STD,
        generator=test_generator,
    )
    evaluation_initial_points = [test_initial_y.clone()]

    if use_normalization:
        input_mean, input_std = compute_online_input_normalizer(
            y0=y0,
            perturbation_std=perturbation_std,
            history=history,
            params=params,
    ...
    with torch.no_grad():
        newton_solution = y0 + newton_direction(y0, p_n, v_n, m, g, dt)

        preview_generator = torch.Generator(device=y0.device)
        preview_generator.manual_seed(LOCAL_RANDOM_SEED)
        preview_online_y = sample_training_initial_y(
            y0=y0,
            perturbation_std=perturbation_std,
            generator=preview_generator,
        )
        preview_y0_delta = mlp(y0, history, params)
        preview_online_delta = mlp(preview_online_y, history, params)
    ...
    print("training_mode=exact_y0_plus_online_resampled_y0_perturbation")
    print("evaluation_mode=fixed_small_y0_perturbation_only")
    print("fixed_training_dataset=False")
    print("training_initial_states_per_epoch=2")
    print("optimizer_steps_per_epoch=2")
    print(f"online_training_perturbation_std={perturbation_std:.1e}")
    print(f"test_perturbation_std={TEST_PERTURBATION_STD:.1e}")
    print(f"fixed_test_initial_y={test_initial_y.tolist()}")
    ...
    print(f"首个在线采样训练初值预览: {preview_online_y.tolist()}")
    print(f"初始化网络在精确 y0 上的输出: {preview_y0_delta.tolist()}")
    print(f"初始化网络在在线扰动点上的输出: {preview_online_delta.tolist()}")
    print(
        f"策略: K 从 {INITIAL_K} 开始，每 {K_INCREASE_INTERVAL} 个 epoch "
        f"增加 {K_INCREASE_AMOUNT}；每个 epoch 先训练精确 y0 轨迹，再训练一条"
        "在线扰动轨迹；每条轨迹内部完整展开，不做步间 detach；"
        "累计该轨迹全部步骤的 loss 后，分别执行一次 backward 和一次 optimizer.step"
    )
    print(f"可视化: 每 {COLOR_BUCKET_SIZE} 个 epoch 使用一种颜色\n")

    # --------------------------------------------------------
    # 9.4 训练：每个 epoch 依次训练精确 y0 轨迹和在线扰动轨迹
    # --------------------------------------------------------

    for epoch in range(epochs):
        if epoch > 0 and epoch % K_INCREASE_INTERVAL == 0 and K < MAX_K:
            K = min(K + K_INCREASE_AMOUNT, MAX_K)

        online_initial_y = sample_training_initial_y(
            y0=y0,
            perturbation_std=perturbation_std,
            generator=training_generator,
        )
        epoch_trajectory_specs = [
            ("exact_y0", y0.clone()),
            ("online_perturbation", online_initial_y),
        ]
        ...
        for trajectory_index, (trajectory_type, initial_y) in enumerate(epoch_trajectory_specs):
            # 每条轨迹单独累计 K 步 loss，单独反向传播并更新一次参数。
            opt.zero_grad(set_to_none=True)
            y = initial_y.clone()
            trajectory_train_points = []
            trajectory_total_loss = None

            epoch_initial_rows.append([
                epoch,
                epoch // COLOR_BUCKET_SIZE,
                trajectory_index,
                1 if trajectory_type == "online_perturbation" else 0,
                initial_y[0].item(),
                initial_y[1].item(),
                initial_y[2].item(),
            ])
            ...
            for iteration_index in range(K):
                ...
                objective_gap = max(float(loss.detach().item() - E_star), 0.0)
            ...
            trajectory_result_point = y.detach().clone()
            summed_trajectory_loss = float(trajectory_total_loss.detach().item())
            mean_step_loss = summed_trajectory_loss / K
            ...
            result_point_rows.append([
                ...
                epoch // COLOR_BUCKET_SIZE,
                trajectory_index,
                trajectory_result_point[0].item(),
                trajectory_result_point[1].item(),
                trajectory_result_point[2].item(),
            ])
            ...
            trajectory_record = {
                ...
                "train_points": [point.tolist() for point in trajectory_train_points],
                "result_point": trajectory_result_point.tolist(),
                "summed_trajectory_loss": summed_trajectory_loss,
                "mean_step_loss": mean_step_loss,
                "final_loss": final_loss,
                "final_gap": final_gap,
            }
            epoch_trajectories.append(trajectory_record)
        ...
        train_log.append(
            {
                "epoch": epoch,
                "K": K,
                "online_sampled_initial_y": online_initial_y.tolist(),
                "trajectories": epoch_trajectories,
                ...
                "summed_trajectory_losses": epoch_summed_trajectory_losses,
                "total_epoch_loss": float(np.sum(epoch_summed_trajectory_losses)),
            }
        )

        # ----------------------------------------------------
        # 每隔固定 epoch，从唯一固定小扰动测试点出发做冻结网络评估。
        # ----------------------------------------------------

        if epoch % EVAL_INTERVAL == 0 or epoch == epochs - 1:
            eval_steps = evaluate_mlp_steps(
                mlp=mlp,
                initial_y=test_initial_y,
                history=history,
                params=params,
                p_n=p_n,
                v_n=v_n,
            ...
            final_eval_gap = float(eval_steps[-1]["loss"] - E_star)
            ...
            eval_log.append(
                {
                    ...
                    "evaluations": [
                        {
                            "initial_index": 0,
                            "initial_y": test_initial_y.tolist(),
                            "steps": eval_steps,
                            "final_gap": final_eval_gap,
                        }
                    ],
                    "mean_final_gap": final_eval_gap,
                    "max_final_gap": final_eval_gap,
                }
            )
            ...
            print(
                f"Epoch {epoch:4d} | "
                f"K={K:2d} | "
                f"Eval Gap from fixed 1e-4 perturbation ({EVAL_STEPS} steps): "
                f"{final_eval_gap:.4e}"
            )

    print("\n✅ 本组训练完成。开始最终对比评估...\n")

    epoch_initial_rows = np.asarray(epoch_initial_rows, dtype=np.float32)
    ...
    # --------------------------------------------------------
    # 9.5 最终测试：从固定 1e-4 扰动点出发，对比 MLP 与 Newton
    # --------------------------------------------------------

    reference_comparison = compare_mlp_and_newton(
        mlp=mlp,
        initial_y=test_initial_y,
        history=history,
        params=params,
    ...
    final_cases = [
        {
            ...
            "initial_type": "fixed_small_y0_perturbation",
            "initial_y": test_initial_y.tolist(),
    ...
    report = {
        "config": {
            ...
            "training_mode": "exact_y0_plus_online_resampled_y0_perturbation",
            "fixed_training_dataset": False,
            "training_initial_states_per_epoch": 2,
            "optimizer_steps_per_epoch": 2,
            "training_trajectory_order": ["exact_y0", "online_perturbation"],
            "training_strategy": "full_unrolled_sum_loss_one_backward_per_trajectory",
            "evaluation_mode": "fixed_small_y0_perturbation_only",
            ...
            "contour_projection_axis_names": [
                ["x", "y", "z"][axis] for axis in CONTOUR_PROJECTION_AXES
            ],
            ...
            "use_normalization": use_normalization,
            "normalization_mode": "analytic_statistics_from_online_sampling_distribution",
            ...
            "online_training_perturbation_std": perturbation_std,
            "online_training_perturbation_std_definition": (
                "Absolute coordinate standard deviation sigma in per-epoch "
                "online sampling: y_online = y0 + sigma * epsilon, "
                "epsilon ~ N(0, I)."
            ),
            "test_perturbation_std": TEST_PERTURBATION_STD,
            "test_perturbation_definition": (
                "Fixed evaluation point: y_test = y0 + test_sigma * epsilon_test."
            ),
            ...
            "evaluation_initial_points": [point.tolist() for point in evaluation_initial_points],
            "p_n": p_n.tolist(),
            "v_n": v_n.tolist(),
            "m": m,
            "g": g,
        ...
        "final_comparison": {"cases": final_cases},
        "summary": {
            "final_test_gap_after_fixed_steps": final_reference_gap,
            "final_test_residual_norm_after_fixed_steps": final_reference_residual_norm,
            "newton_test_residual_norm_after_fixed_steps": (
                reference_comparison["newton"]["iterations"][-1]["residual_norm"]
            ),
            "final_periodic_eval_gap_from_fixed_test_perturbation": eval_log[-1]["max_final_gap"],
        },
    }
    ...
    # --------------------------------------------------------
    # 9.7 统计图、固定测试点 residual 图、轨迹图与二维等高线图
    # --------------------------------------------------------
    ...
    plot_final_residual_comparison(
        comparison=reference_comparison,
        save_path=output_dir / "final_residual_comparison_initial_0.png",
        initial_index=0,
        initial_description="Fixed y0 perturbation (std=1e-4)",
    )
    ...
    plot_final_test_distribution(
        ...
        save_path=output_dir / "final_test_distribution_initial_0.png",
    ...
    plot_final_test_energy_contour_2d(
        ...
        g=g,
        dt=dt,
        save_path=output_dir / "final_test_energy_contour_2d_initial_0.png",
        projection_axes=CONTOUR_PROJECTION_AXES,
        relative=PLOT_RELATIVE_COORDINATES,
    )
    ...
    return {
        ...
        "final_reference_residual_norm_after_fixed_steps": final_reference_residual_norm,
...
# ============================================================
# 10. float32 精确 y0 + 在线扰动训练跨优化器汇总图
# ============================================================
...
def plot_float32_optimizer_perturbation_range_summary(
    ...
    axes[0].set_title("Fixed-Test Objective Gap vs. Training Perturbation Range")
    axes[0].set_ylabel("Gap")

    axes[1].set_title("Periodic Eval Gap from Fixed 1e-4 Test Perturbation")
    axes[1].set_ylabel("Gap")

    axes[2].set_title("Fixed-Test Residual vs. Training Perturbation Range")
    ...
    print(f"🖼️ float32 精确 y0 + 在线扰动训练跨优化器汇总图已保存至: {save_path}")


# ============================================================
# 11. 主程序：依次运行两组 float32 精确 y0 + 在线扰动完整轨迹反向传播实验
# ============================================================

def main():
    base_output_dir = create_output_directory()
    print(f"📁 float32 精确 y0 + 在线扰动完整轨迹反向传播实验总输出目录: {base_output_dir}")
    print(f"torch default dtype: {torch.get_default_dtype()}")

    # 固定物理问题。两组实验只改变训练参数优化器类型。
    m = 1.0
    g = 9.8
    dt = 0.01
    ...
    ablation_summary = {
        ...
        "experiment_type": "float32_exact_y0_plus_online_perturbation_full_unrolled_training",
        "torch_dtype": str(TORCH_DTYPE),
        "comparison_principle": (
            ...
            "physical problem, model initialization seed, per-epoch online "
            "perturbation seed, network architecture, training schedule, "
            "analytic input normalization, output dt scaling, and sigma=1e-2. "
            "There is no fixed training dataset: every epoch first trains one "
            "trajectory from exact y0 and then resamples one online initial state "
            "as y_online = y0 + sigma * epsilon. Each trajectory is fully "
            "unrolled for K steps without detach. The K step losses of that "
            "trajectory are summed before one backward call and one optimizer "
            "step. K starts at 5 and increases by 5 every 200 epochs. Evaluation "
            "and final testing only use one fixed y0 perturbation with std=1e-4. SGD uses "
            "lr=1e-2 without momentum; Adam uses lr=1e-4."
        ),
        "training_mode": "exact_y0_plus_online_resampled_y0_perturbation",
        "fixed_training_dataset": False,
        "training_initial_states_per_epoch": 2,
    ...
    summary_path = base_output_dir / "float32_y0_plus_online_perturbation_full_unrolled_training_summary.json"
    with summary_path.open("w", encoding="utf-8") as file:
        json.dump(ablation_summary, file, indent=2, ensure_ascii=False)

    summary_plot_path = base_output_dir / "float32_y0_plus_online_perturbation_full_unrolled_training_summary.png"
    ...
    print("\n" + "=" * 72)
    print("✅ 两组 float32 精确 y0 + 在线扰动完整轨迹反向传播实验全部完成。")
    ...
    for item in experiment_summaries:
        print(
            ...
            f"最终固定测试点 gap={item['final_reference_gap_after_fixed_steps']:.4e}，"
