...
# 本脚本显式使用 float32，并比较两个候选优化器在在线随机扰动训练下的性能。
...
# float32 在线扰动训练实验：固定两个前序实验中表现较好的优化器方案，
# 每个 epoch 都重新从 y0 附近采样一个随机扰动初值。
#
# 固定条件：
# 1. 不再预先构造固定训练集；
# 2. 每个 epoch 重新采样 y_epoch = y0 + sigma * epsilon，epsilon ~ N(0, I)；
# 3. 测试集只保留未扰动的原始初值 y0；
# 4. 输入归一化统计量由采样分布解析给出：位置均值为 y0，位置标准差为 sigma；
# 5. 网络输出乘以 dt；
# 6. 所有张量、模型参数和优化器状态均使用 torch.float32；
# 7. SGD 使用不带 momentum 的标准形式；
# 8. SGD 学习率固定为 1e-2；Adam 学习率固定为 1e-4。
#
# 本实验只使用一个扰动尺度：
#     sigma = 1e-2
#
# 训练策略：
# 1. 每个 epoch 在线采样一个新的 y_epoch = y0 + sigma * epsilon；
# 2. 从 y_epoch 出发完整展开 K 步 MLP 迭代；
# 3. 不在轨迹内部的迭代步之间 detach；
# 4. 将该轨迹前 K 步的 loss 相加；
# 5. 每个 epoch 只执行一次 backward 和一次 optimizer.step；
        ...
        ABLATION_EXPERIMENTS.append(
            {
                ...
                "description": (
                    "Float32 online resampled-perturbation training with full "
                    f"normalization, {optimizer_name.upper()} optimizer, "
...
def plot_training_points_and_results(
    ...
    每个 epoch 都重新从 y0 附近采样一个训练初值，并从该点出发连续执行
    K 次 MLP 更新。脚本不再使用固定训练集。
    ...
    for epoch_item in visualization_log:
        ...
        for trajectory in epoch_item["trajectories"]:
            ...
            sampled_initial = to_plot_coordinates(
            ...
            sampled_initial_points.append(sampled_initial)
    ...
    sampled_initial_points = np.vstack(sampled_initial_points)
    reference_point = to_plot_coordinates(
        [reference_y0],
        reference_y0=reference_y0,
        relative=relative,
    )[0]
    ...
    for ax in (ax_train, ax_result):
        ax.scatter(
            sampled_initial_points[:, 0],
            sampled_initial_points[:, 1],
            sampled_initial_points[:, 2],
            marker="x",
            s=18,
            c="black",
            alpha=0.20,
    ...
    ax_train.set_title(
        r"All Online Training Input Points: "
        r"$x^{(0)}, x^{(1)}, \ldots, x^{(K-1)}$"
    )
    ax_result.set_title(r"All Epoch Result Points: $x^{(K)}$")

    train_points_for_limits = np.vstack(
        all_train_points
        + [sampled_initial_points, reference_point.reshape(1, 3), newton_point.reshape(1, 3)]
    )
    ...
    marker_handles = [
        ...
        Line2D([0], [0], marker="x", color="black",
               label="Online sampled epoch initial states",
               linestyle="None", markersize=7),
        Line2D([0], [0], marker="x", color="black",
               label="Original y0 (evaluation only)",
    ...
    fig.suptitle("Online-Resampled Training Data and Results by Epoch Range", fontsize=14)

    plt.tight_layout(rect=[0.00, 0.00, 0.80, 0.95])
    plt.savefig(save_path, dpi=300, bbox_inches="tight")
    plt.close(fig)

    print(f"🖼️ 在线训练点和结果点分布图已保存至: {save_path}")
...
def plot_summary_report(
    ...
    axes[0, 0].set_title("Training Gap from Online-Resampled Initial State")
...
def plot_final_residual_comparison(
    ...
    当前实验的测试集只包含 initial_index=0，即未扰动的原始初值 y0。
    """
    ...
    ax.set_yscale("log")
    if is_reference_y0:
        title_suffix = "Initial 0: Original y0 (Evaluation Only)"
    else:
...
def run_experiment(
    ...
    """
    运行一组 float32 在线随机扰动训练实验，并将结果写入独立子目录。

    训练策略：
        - 不预先构造固定训练集；
        - 每个 epoch 在线采样一个 y_epoch = y0 + sigma * epsilon；
        - 从该随机起点完整展开当前 K 次 MLP 更新；
        - 轨迹内部不执行 detach，因此梯度能够穿过完整轨迹；
        - 累加该轨迹前 K 个步骤的 loss；
        - 每个 epoch 对这一条轨迹执行一次 backward 和一次 optimizer.step；
        - K 从 5 开始，每 200 个 epoch 增加 5。

    测试策略：
        - 测试集只包含未扰动的原始初值 y0；
        - 周期评估和训练后最终评估都固定从 y0 出发。
    """
    ...
    print("\n" + "=" * 72)
    print(f"🧪 float32 在线随机扰动训练实验组: {experiment_name}")
    ...
    # 测试集只保留原始初值 y0。
    evaluation_initial_points = [y0.clone()]

    # 每个 epoch 从同一条可复现实验随机序列中重新采样一个训练起点。
    ...
    print("training_mode=online_resampled_y0_perturbation")
    print("evaluation_mode=original_y0_only")
    print("fixed_training_dataset=False")
    print("training_initial_states_per_epoch=1")
    print(f"evaluation_initial_points={len(evaluation_initial_points)}")
    ...
    print(f"首个在线采样训练初值预览: {preview_initial_y.tolist()}")
    print(f"初始化网络在该点上的输出: {preview_delta.tolist()}")
    print(
        f"策略: K 从 {INITIAL_K} 开始，每 {K_INCREASE_INTERVAL} 个 epoch "
        f"增加 {K_INCREASE_AMOUNT}；每个 epoch 在线采样一条轨迹；"
        "轨迹内部完整展开，不做步间 detach；累计该轨迹全部步骤的 loss 后，"
        "执行一次 backward 和一次 optimizer.step"
    )
    print(f"可视化: 每 {COLOR_BUCKET_SIZE} 个 epoch 使用一种颜色\n")

    # --------------------------------------------------------
    # 9.4 训练：每个 epoch 重新扰动 y0，生成一个新的训练轨迹
    # --------------------------------------------------------
    ...
    for epoch in range(epochs):
        ...
        initial_y = sample_training_initial_y(
            y0=y0,
            perturbation_std=perturbation_std,
            generator=training_generator,
        )

        # 每个 epoch 只有一条在线采样轨迹。轨迹内部完整展开 K 步，
        # 累加全部步骤的能量后，再统一执行一次反向传播和一次参数更新。
        ...
        # 保留与原四宫格绘图函数兼容的字段。由于每个 epoch 只有一条在线轨迹，
        # mean_final_loss 与 max_final_loss 都等于该轨迹的最终 loss。
        train_log.append(
            {
                "epoch": epoch,
                "K": K,
                "sampled_initial_y": initial_y.tolist(),
                "final_loss": final_loss,
                "final_gap": final_gap,
                "summed_trajectory_loss": summed_trajectory_loss,
                "mean_step_loss": mean_step_loss,
                "mean_final_loss": final_loss,
                "max_final_loss": final_loss,
            }
        )

        # ----------------------------------------------------
        # 每隔固定 epoch，从唯一测试初值 y0 出发做冻结网络评估。
        # ----------------------------------------------------
        ...
        if epoch % EVAL_INTERVAL == 0 or epoch == epochs - 1:
            ...
            print(
                f"Epoch {epoch:4d} | "
                f"K={K:2d} | "
                f"Eval Gap from y0 ({EVAL_STEPS} steps): {final_eval_gap:.4e}"
            )
    ...
    # --------------------------------------------------------
    # 9.5 最终测试：只从 y0 出发，对比 MLP 与 Newton
    # --------------------------------------------------------
    ...
    report = {
        "config": {
            ...
            "training_mode": "online_resampled_y0_perturbation",
            "fixed_training_dataset": False,
            "training_initial_states_per_epoch": 1,
            "training_strategy": "full_unrolled_sum_loss_one_backward_per_epoch_trajectory",
            "evaluation_mode": "original_y0_only",
            ...
            "use_normalization": use_normalization,
            "normalization_mode": "analytic_statistics_from_sampling_distribution",
            ...
            "perturbation_std_definition": (
                ...
                "online sampling: y_epoch = y0 + sigma * epsilon, "
                "epsilon ~ N(0, I)."
            ),
        ...
        "summary": {
            ...
            "final_periodic_eval_gap_from_y0": eval_log[-1]["max_final_gap"],
        },
    }
    ...
    # --------------------------------------------------------
    # 9.7 统计图、y0 residual 图、y0 三维轨迹图与二维等高线图
    # --------------------------------------------------------
...
def plot_float32_optimizer_perturbation_range_summary(
    ...
    axes[1].set_title("Periodic Eval Gap from y0 vs. Perturbation Range")
    ...
    print(f"🖼️ float32 在线扰动训练跨优化器汇总图已保存至: {save_path}")


# ============================================================
# 11. 主程序：依次运行两组 float32 在线随机扰动完整轨迹反向传播实验
# ============================================================

def main():
    base_output_dir = create_output_directory()
    print(f"📁 float32 在线随机扰动完整轨迹反向传播实验总输出目录: {base_output_dir}")
    ...
    ablation_summary = {
        ...
        "experiment_type": "float32_online_resampled_y0_perturbation_full_unrolled_training",
        "torch_dtype": str(TORCH_DTYPE),
        "comparison_principle": (
            ...
            "There is no fixed training dataset: every epoch resamples one "
            "initial state as y_epoch = y0 + sigma * epsilon. From that point, "
            "the model is fully unrolled for K steps without detach. The K step "
            "losses are summed before one backward call and one optimizer step. "
            "K starts at 5 and increases by 5 every 200 epochs. Evaluation and "
            "final testing only use the original unperturbed y0. SGD uses "
    ...
    summary_path = base_output_dir / "float32_online_resampled_full_unrolled_training_summary.json"
    with summary_path.open("w", encoding="utf-8") as file:
        json.dump(ablation_summary, file, indent=2, ensure_ascii=False)

    summary_plot_path = base_output_dir / "float32_online_resampled_full_unrolled_training_summary.png"
    ...
    print("\n" + "=" * 72)
    print("✅ 两组 float32 在线随机扰动完整轨迹反向传播实验全部完成。")
    ...
    for item in experiment_summaries:
        print(
            ...
            f"最终 y0 gap={item['final_reference_gap_after_fixed_steps']:.4e}，"
