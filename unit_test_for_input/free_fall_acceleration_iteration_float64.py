"""
自由落体单帧变分问题：加速度变量迭代版规则网格 Full-Batch 训练数据规模消融实验（float64 全优化器对照）
====================================================

本版把原脚本中的“直接迭代下一帧位置 y”改成“迭代待求解加速度 a”。
核心内层流程为：
    当前物理状态 x^n, v^n
    初始化/给定待求解加速度 a^(0)
    MLP(previous_delta_a, x^n, v^n, m, g, dt) -> delta_a^(k)
    a^(k+1) = a^(k) + delta_a^(k)
    v^(n+1) = v^n + dt * a^(K)
    x^(n+1) = x^n + dt * v^(n+1)

为了保留原来的“规则网格数据规模消融”实验结构，训练阶段仍然先在
x^(n+1) 空间中生成以解析解 y_star 为中心的规则网格，然后通过
    a = (x^(n+1) - x^n - dt * v^n) / dt^2
把这些位置样本转换为初始加速度猜测。这样原来的采样半径、数据规模、
能量 loss、绘图和最终 held-out 测试仍然可比较。

最终测试严格按用户给出的物理流程：从真实状态 x^n, v^n 出发，令
    a^(0) = 0, previous_delta_a = 0,
展开 K 次 MLP 加速度增量，再由 a^(K) 更新 v^(n+1), x^(n+1)。

默认保持：
1. 物理问题固定；
2. 网络结构固定为 12 -> 32 -> 32 -> 3；
3. 所有张量、模型参数、梯度和优化器状态使用 torch.float64；
4. 输入使用当前加速度规则网格的解析统计量做逐特征归一化；
5. 网络直接输出加速度增量 delta_a，不再乘以 dt；
6. loss 只使用由当前加速度 a 映射出的 x^(n+1) 的隐式欧拉变分能量；
7. 每个 epoch 使用当前完整规则网格作为一个 Full-Batch；
8. 在同一组参数下完整展开 K 步轨迹，不在步间 detach；
9. 每个 epoch 仅执行一次 backward 和一次 optimizer.step；
10. 训练阶段从 K=1 开始，每 2000 个 epoch 增加 1 次展开迭代，最高 K=5；
11. 总训练轮数为 10000；
12. 默认在 cuda:1 上运行。
"""
...
# float64 对照实验：保留输入逐特征归一化。
# 加速度迭代版中，网络输出就是 delta_a，不再乘以 dt。
...
# 固定局部采样范围：
# 先在 x^(n+1) 空间中取 [y_star - R, y_star + R]，
# 再通过运动学关系转换为初始加速度猜测。
DEFAULT_SAMPLING_RADIUS = 0.01
DEFAULT_EPOCHS = 10_000
DEFAULT_EVAL_INTERVAL = 100
DEFAULT_FINAL_TEST_STEPS = 50
# 训练阶段使用 curriculum：K 从 1 开始，每 2000 个 epoch 增加 1，最高为 5。
DEFAULT_INITIAL_K = 1
DEFAULT_K_INCREASE_INTERVAL = 2_000
...
# 汇总图中每条训练 loss 曲线最多保存/绘制这些点，避免 10000 epoch × 42 组实验让总汇总 JSON 过大。
DEFAULT_SUMMARY_TRAIN_LOSS_POINTS = 1_000
...
def finite_plot_value(value: float | None) -> float:
    """将不可绘制的 None/非有限值转换为 NaN，有限小值截断到绘图下限。"""
...
def format_metric(value: float | None) -> str:
    """用于最终终端汇总打印；None 表示该实验没有形成有效训练日志。"""
...
def make_training_curve_for_summary(
    train_log: Sequence[dict],
    max_points: int = DEFAULT_SUMMARY_TRAIN_LOSS_POINTS,
) -> list[dict]:
    """
    从完整 train_log 中抽样出用于总汇总图和总汇总 JSON 的训练 loss 曲线。

    每个实验目录下的 optimization_report.json 仍然保存完整逐 epoch train_log；
    这里的抽样曲线只用于跨 6 个优化器设置做总对比。
    """
    ...
    if len(train_log) <= max_points:
        selected_indices = list(range(len(train_log)))
    else:
        ...
        selected_indices = sorted(set(selected_indices + [len(train_log) - 1]))

    curve = []
    for index in selected_indices:
        item = train_log[index]
        curve.append(
            {
                "epoch": item["epoch"],
                "K": item["K"],
                "mean_training_trajectory_loss": item[
                    "mean_training_trajectory_loss"
                ],
                "mean_training_gap_for_readability": item[
                    "mean_training_gap_for_readability"
                ],
            }
        )
    return curve
...
class MLPOptimizer(nn.Module):
    ...
    输入：
        [previous_delta_a, p_n, v_n, m, g, dt]，共 12 维。

    输出：
        delta_a，表示当前内层迭代的加速度增量。

    注意：本版默认不做 output * dt，因为输出变量已经从位置更新量
    改成了加速度增量。
    """
    ...
    def __init__(
        ...
        *,
        ...
        use_output_dt_scaling: bool = False,
    ...
    def forward(
        ...
        """输入 y 在本版中表示 previous_delta_a，支持 [3] 和 [B, 3]。"""
...
def acceleration_to_next_position(
    ...
    由当前加速度猜测 a 计算下一帧位置：
        v^(n+1) = v^n + dt * a
        x^(n+1) = x^n + dt * v^(n+1)
    支持 acceleration.shape == [3] 或 [B, 3]。
    """

    return p_n + dt * (v_n + dt * acceleration)


def acceleration_to_next_velocity(
    acceleration: torch.Tensor,
    v_n: torch.Tensor,
    dt: float,
) -> torch.Tensor:
    ...
    return v_n + dt * acceleration


def next_position_to_acceleration(
    y: torch.Tensor,
    p_n: torch.Tensor,
    v_n: torch.Tensor,
    dt: float,
    ...
    """
    把下一帧位置样本 y 转换为等价的加速度猜测：
        a = (y - x^n - dt * v^n) / dt^2
    """

    return (y - p_n - dt * v_n) / (dt**2)
...
def compute_regular_grid_input_normalizer(
    *,
    ...
    输入顺序为：
        [previous_delta_a, p_n, v_n, m, g, dt]

    训练网格先在 x^(n+1) 空间中采样，再除以 dt^2 转成加速度空间，
    因此加速度网格的标准差等于原位置网格标准差 / dt^2。
    history 和 params 在当前固定物理问题中保持不变，其 std 置为 1。
    """
    ...
    position_std_value = radius * math.sqrt((n + 1.0) / (3.0 * (n - 1.0)))
    acceleration_std_value = position_std_value / (dt**2)
    ...
    input_std = torch.cat(
        [
            torch.full((3,), acceleration_std_value, dtype=TORCH_DTYPE),
...
def evaluate_reference_trajectory(
    ...
    initial_previous_delta_a: torch.Tensor,
    history: torch.Tensor,
    params: torch.Tensor,
    p_n: torch.Tensor,
    v_n: torch.Tensor,
    ...
    """冻结网络后，从 a^(0)=0 的加速度流程出发展开测试轨迹。"""

    acceleration = initial_acceleration.clone()
    previous_delta_a = initial_previous_delta_a.clone()
    iterations = []

    for step in range(num_steps + 1):
        y = acceleration_to_next_position(acceleration, p_n, v_n, dt)
        v_next = acceleration_to_next_velocity(acceleration, v_n, dt)
        ...
        iterations.append(
            {
                "step": step,
                "a": tensor_to_list(acceleration),
                "previous_delta_a": tensor_to_list(previous_delta_a),
                "v_next": tensor_to_list(v_next),
                "y": tensor_to_list(y),
                "energy": energy,
                "gap": energy - e_star,
        ...
        with torch.no_grad():
            delta_a = model(previous_delta_a, history, params)
            acceleration = acceleration + delta_a
            previous_delta_a = delta_a
            iterations[-1]["next_delta_a"] = tensor_to_list(delta_a)
            iterations[-1]["next_delta_a_norm"] = float(torch.norm(delta_a).item())

    return {
        "initial_acceleration": tensor_to_list(initial_acceleration),
        "initial_previous_delta_a": tensor_to_list(initial_previous_delta_a),
        "initial_y": tensor_to_list(
            acceleration_to_next_position(initial_acceleration, p_n, v_n, dt)
        ),
...
def run_experiment(
    *,
    ...
    # 解析解对应的真实加速度。自由落体问题中 a_star = [0, 0, -g]。
    a_star = torch.tensor([0.0, 0.0, -g], dtype=TORCH_DTYPE, device=device)
    y_star = acceleration_to_next_position(a_star, p_n_device, v_n_device, dt)
    history = torch.cat([p_n_device, v_n_device])
    params = torch.tensor([m, g, dt], dtype=TORCH_DTYPE, device=device)

    zero_acceleration = torch.zeros(3, dtype=TORCH_DTYPE, device=device)
    zero_previous_delta_a = torch.zeros(3, dtype=TORCH_DTYPE, device=device)
    x_zero_acceleration = acceleration_to_next_position(
        zero_acceleration,
        p_n_device,
        v_n_device,
        dt,
    ...
    # 原脚本缓存的是 x^(n+1) 空间中的规则网格。
    # 本版把它转换成等价的初始加速度猜测网格，作为训练展开的 a^(0)。
    training_acceleration_grid = next_position_to_acceleration(
        training_grid,
        p_n_device,
        v_n_device,
        dt,
    ...
    print(f"training_position_grid_device={training_grid.device}")
    print(f"training_position_grid_memory_mb={training_grid.numel() * training_grid.element_size() / 1024**2:.2f}")
    print(f"training_acceleration_grid_memory_mb={training_acceleration_grid.numel() * training_acceleration_grid.element_size() / 1024**2:.2f}")
    print(f"sampling_center=y_star={tensor_to_list(y_star)}")
    print(f"acceleration_center=a_star={tensor_to_list(a_star)}")
    print(f"zero_acceleration_position={tensor_to_list(x_zero_acceleration)}")
    print(f"sampling_radius_in_position_space={config.sampling_radius}")
    print(f"sampling_radius_in_acceleration_space={config.sampling_radius / (dt**2):.8e}")
    print("sampling_distribution=regular_cartesian_grid_in_position_space_then_converted_to_acceleration")
    ...
    print("training_unroll_mode=curriculum")
    print(
    ...
    for epoch in range(config.epochs):
        ...
        # 训练网格现在表示初始加速度猜测 a^(0)。
        # 内层展开：MLP(previous_delta_a, p_n, v_n, params) -> delta_a，
        #           a <- a + delta_a，
        #           y = x^n + dt * (v^n + dt * a)。
        acceleration = training_acceleration_grid
        previous_delta_a = training_acceleration_grid
        ...
        for _ in range(k):
            delta_a = model(previous_delta_a, history, params)
            acceleration = acceleration + delta_a
            y = acceleration_to_next_position(acceleration, p_n_device, v_n_device, dt)
        ...
        if epoch % config.eval_interval == 0 or epoch == config.epochs - 1:
            ...
            reference_trajectory = evaluate_reference_trajectory(
                model=model,
                initial_acceleration=zero_acceleration,
                initial_previous_delta_a=zero_previous_delta_a,
                history=history,
                params=params,
                p_n=p_n_device,
                v_n=v_n_device,
    ...
    report = {
        "config": {
            ...
            "dataset_mode": "position_grid_near_y_star_converted_to_initial_acceleration_grid_full_batch",
            "sampling_center": "y_star_in_position_space_and_a_star_in_acceleration_space",
            "sampling_radius_per_axis_in_position_space": config.sampling_radius,
            "sampling_radius_per_axis_in_acceleration_space": config.sampling_radius / (dt**2),
            "sampling_distribution": (
                "Cartesian product of three uniform linspace axes over [-R, R] "
                "in x^(n+1) space; then converted to acceleration by "
                "a=(x_next-x_n-dt*v_n)/dt^2."
            ),
            ...
            "grid_storage": (
                "The complete position grid is precomputed once in chunks and cached "
                "on the training device, then converted to a full-batch initial "
                "acceleration grid for the acceleration-iteration solver."
            ),
            "grid_precompute_chunk_size": config.grid_precompute_chunk_size,
            "training_position_grid_memory_mb": (
                training_grid.numel() * training_grid.element_size() / 1024**2
            ),
            "training_acceleration_grid_memory_mb": (
                training_acceleration_grid.numel()
                * training_acceleration_grid.element_size()
                / 1024**2
            ),
            "training_mode": "full_batch",
            ...
            "training_unroll_mode": "curriculum",
            "training_unroll_schedule": (
                "K starts at initial_K, increases by K_increase_amount every "
                "K_increase_interval epochs, and is capped at max_K."
            ),
            ...
            "normalization_definition": (
                ...
                "from the acceleration grid. Only previous_delta_a varies across "
                "samples; fixed history and parameter features use std=1."
            ),
            "loss": (
                "For the full acceleration grid, sum the mean variational energy "
                "of x^(n+1)(a) over all unrolled acceleration steps. No additional "
                "loss term and no division by K."
            ),
            "backpropagation": (
                "Full unrolled trajectory backpropagation without detach; "
                "one backward call and one optimizer.step per epoch."
            ),
            "evaluation_mode": "held_out_physical_state_with_a0_equals_zero_and_previous_delta_a0_equals_zero",
            ...
            "a_star": tensor_to_list(a_star),
            "zero_acceleration_position": tensor_to_list(x_zero_acceleration),
    ...
    plot_reference_trajectory_3d(
        mlp_trajectory=final_mlp_trajectory,
        newton_solution=tensor_to_list(newton_solution),
        initial_y=tensor_to_list(x_zero_acceleration),
    ...
    final_train_item = train_log[-1] if train_log else None
    training_loss_curve_for_summary = make_training_curve_for_summary(train_log)
    ...
    return {
        ...
        "final_training_epoch": (
            None if final_train_item is None else final_train_item["epoch"]
        ),
        "final_training_K": (
            None if final_train_item is None else final_train_item["K"]
        ),
        "final_mean_training_trajectory_loss": (
            None
            if final_train_item is None
            else final_train_item["mean_training_trajectory_loss"]
        ),
        "final_mean_training_gap_for_readability": (
            None
            if final_train_item is None
            else final_train_item["mean_training_gap_for_readability"]
        ),
        "training_loss_curve_for_summary": training_loss_curve_for_summary,
...
def plot_dataset_scale_summary(
    ...
    除 held-out p_n 的最终 gap/residual，也额外输出最终训练 loss gap，
    让总对比图同时包含测试集数据和训练 loss 数据。
    """
    ...
    for optimizer_config in OPTIMIZER_CONFIGS:
        ...
        train_loss_gaps = [
            finite_plot_value(item.get("final_mean_training_gap_for_readability"))
            for item in selected
        ]
        ...
        axes[2].plot(sizes, train_loss_gaps, marker="^", label=label)
    ...
    axes[1].set_ylabel(r"$\|\nabla E(y)\|_2$")
    axes[2].set_title("Final Training Loss Gap vs. Training Dataset Size")
    axes[2].set_ylabel(r"Mean $\sum_k [E(y^{(k)})-E(y^*)]$")
...
def plot_training_loss_summary_6_subplots(
    summaries: Sequence[dict],
    save_path: Path,
) -> None:
    """
    将 6 种优化器/学习率设置的训练 loss 曲线画成 2×3 子图。

    每个子图对应一种 optimizer/lr 设置；同一子图内用不同曲线对比不同
    训练数据规模。曲线使用 run_experiment 返回的抽样 loss 数据，因此不会
    让总汇总 JSON 变得过大。
    """

    num_configs = len(OPTIMIZER_CONFIGS)
    num_cols = 3
    num_rows = math.ceil(num_configs / num_cols)
    fig, axes = plt.subplots(
        num_rows,
        num_cols,
        figsize=(6 * num_cols, 4.8 * num_rows),
        sharex=True,
    )
    axes_array = np.asarray(axes).reshape(-1)

    for ax_index, optimizer_config in enumerate(OPTIMIZER_CONFIGS):
        ...
        for item in selected:
            curve = item.get("training_loss_curve_for_summary", [])
            if not curve:
                continue

            epochs = [point["epoch"] for point in curve]
            losses = [
                finite_plot_value(point["mean_training_gap_for_readability"])
                for point in curve
            ]
            ax.plot(
                epochs,
                losses,
                linewidth=1.4,
                label=f"N={item['dataset_size']:,}",
            )

        ax.set_title(f"{optimizer_name.upper()} lr={learning_rate:.0e}")
        ax.set_yscale("log")
        ax.set_xlabel("Epoch")
        ax.set_ylabel(r"Training loss gap")
        ax.grid(True, alpha=0.3)
        ax.legend(fontsize=7)

    for ax in axes_array[num_configs:]:
        ax.axis("off")

    fig.suptitle(
        "Training Loss Curves by Optimizer Setting "
        "(one subplot per optimizer/lr)",
        y=1.02,
        fontsize=14,
    )
    plt.tight_layout()
...
def main() -> None:
    ...
    a_star = torch.tensor([0.0, 0.0, -g], dtype=TORCH_DTYPE)
    y_star = acceleration_to_next_position(a_star, p_n, v_n, dt)
    ...
    overall_report = {
        "experiment_type": "acceleration_iteration_fixed_local_regular_grid_dataset_size_ablation",
        "purpose": (
            ...
            "position grid near y_star, convert that grid to initial acceleration "
            "guesses, and evaluate the held-out physical state using a^(0)=0."
        ),
        ...
        "sampling_distribution": (
            "Cartesian product of three uniform linspace axes over [-R, R] "
            "in x^(n+1) space; each axis uses an even number of points. "
            "The position samples are converted to acceleration samples by "
            "a=(x_next-x_n-dt*v_n)/dt^2."
        ),
        "training_set_contains_y_star": False,
        ...
        "grid_precompute_chunk_size": config.grid_precompute_chunk_size,
        "grid_storage": (
            "For each dataset scale, precompute the complete position grid once "
            "in chunks on the training device, reuse it for all optimizer and "
            "learning-rate experiments, and convert it to an acceleration grid "
        ...
        "normalization_definition": (
            "Per-feature mean and population std are computed analytically "
            "for each acceleration grid. Only previous_delta_a varies across "
            "samples; fixed history and parameter features use std=1."
        ),
        "loss": (
            "For the full acceleration grid, sum the mean variational energy of "
            "x^(n+1)(a) over all unrolled acceleration steps. No additional "
            "loss term and no division by K."
        ),
    ...
    training_loss_summary_plot_path = (
        base_output_dir / "training_loss_6_optimizer_settings_summary.png"
    )
    plot_training_loss_summary_6_subplots(
        experiment_summaries,
        training_loss_summary_plot_path,
    )
    ...
    print(f"🖼️ 训练 loss 六子图: {training_loss_summary_plot_path}")
    for item in experiment_summaries:
        print(
            ...
            f"final_train_loss_gap={format_metric(item.get('final_mean_training_gap_for_readability'))}, "
            f"最终 p_n gap={format_metric(item['final_reference_gap_after_fixed_steps'])}, "
            f"最终 p_n residual={format_metric(item['final_reference_residual_norm_after_fixed_steps'])}"
        )
