...
# 5. 随机扰动点数量固定为 10 个。
#
# 本实验只改变训练网络参数时使用的优化器与学习率：
#     SGD :  1e-2, 1e-3, 1e-4
#     Adam:  1e-2, 1e-3, 1e-4
#
# 六组实验复用相同训练数据、网络初始化随机种子与训练流程，
# 使差异集中在优化器和学习率本身。SGD 使用不带 momentum 的标准形式。
...
for optimizer_name in OPTIMIZER_NAMES:
    for learning_rate in LEARNING_RATE_VALUES:
        experiment_index = len(ABLATION_EXPERIMENTS) + 1
        ABLATION_EXPERIMENTS.append(
            {
                "name": (
                    f"{experiment_index:02d}_{optimizer_name}_"
                    f"lr_{learning_rate:.0e}"
                ),
                "description": (
                    "Full normalization with 10 initial-point perturbations. "
...
def run_experiment(
    ...
    """
    运行一组“优化器 × 学习率”消融实验，并将结果写入独立子目录。

    六组实验保持物理问题、训练数据集、扰动范围、噪声生成种子、
    网络结构、初始化种子、训练策略和完整归一化方案一致，
    只改变训练网络参数时使用的优化器与学习率。
    """
    ...
    print("\n" + "=" * 72)
    print(f"🧪 优化器与学习率消融组: {experiment_name}")
...
def plot_optimizer_learning_rate_ablation_summary(
    experiment_summaries,
    save_path,
):
    """绘制不同优化器、不同学习率对应的最终误差指标。"""
    ...
    for optimizer_name in OPTIMIZER_NAMES:
        ...
        optimizer_label = optimizer_name.upper()

        axes[0].plot(
            learning_rates,
            final_reference_gaps,
            marker="o",
            label=f"{optimizer_label}: reference gap",
        )
        axes[0].plot(
            learning_rates,
            final_worst_eval_gaps,
            marker="s",
            linestyle="--",
            label=f"{optimizer_label}: worst eval gap",
        )
        axes[1].plot(
            learning_rates,
            final_reference_residuals,
            marker="o",
            label=f"{optimizer_label}: reference residual",
        )

    axes[0].set_xscale("log")
    axes[0].set_yscale("log")
    axes[0].set_title("Objective Gap vs. Learning Rate")
    ...
    axes[1].set_title("Residual vs. Learning Rate")
    ...
    print(f"🖼️ 优化器与学习率消融汇总图已保存至: {save_path}")


# ============================================================
# 10. 主程序：依次运行六组优化器与学习率消融
# ============================================================

def main():
    base_output_dir = create_output_directory()
    print(f"📁 优化器与学习率消融总输出目录: {base_output_dir}")

    # 固定物理问题、扰动范围和训练数据数量。
    # 六组实验只改变训练网络参数时使用的优化器与学习率。
    m = 1.0
    g = 9.8
    dt = 0.01
    ...
    ablation_summary = {
        ...
        "comparison_principle": (
            "All groups share the same physical problem, fixed training "
            "dataset with 10 random initial-point perturbations, absolute "
            "coordinate perturbation standard deviation, standard-normal "
            "random seed, network architecture, model initialization seed, "
            "training schedule, input normalization, and output dt scaling. "
            "Only the optimizer and learning rate change. SGD uses no "
            "momentum."
        ),
    ...
    summary_path = (
        base_output_dir
        / "optimizer_learning_rate_ablation_summary.json"
    )
    ...
    summary_plot_path = (
        base_output_dir
        / "optimizer_learning_rate_ablation_summary.png"
    )
    plot_optimizer_learning_rate_ablation_summary(
        experiment_summaries=experiment_summaries,
        save_path=summary_plot_path,
    )

    print("\n" + "=" * 72)
    print("✅ 六组优化器与学习率消融实验全部完成。")
    ...
    for item in experiment_summaries:
        print(
            ...
            f"最终参考 gap="
            f"{item['final_reference_gap_after_fixed_steps']:.4e}，"
            f"最终 residual="
            f"{item['final_reference_residual_norm_after_fixed_steps']:.4e}"
        )
